...
if(isset($_POST["veauto"]) and isset($_POST["vesavename"]) and isset($_POST["vevalue"]) and $_POST["vesavename"] != "" and $_POST["veauto"] != "" and $_POST["veauto"] != 0)
{
	...
	$inid = $mysqli->real_escape_string(@trim($_POST["veauto"]));
...
*/
	if ($stmtttt = $mysqli->prepare('Select ID From vendor_rates Where ID="'.$inid.'" '.(($_SESSION["group_id"] == 3 || $_SESSION["group_id"] == 5) ? '':'').' LIMIT 1')) { 
...
//echo $vesavename."l".$vevalue;die();
	...
	audit_log($mysqli,'vendor_rates','UPDATE',$new_value,'WHERE ID='.$inid,'','','ID');
	$sql='Update vendor_rates SET `'.$vesavename.'` = "'.$vevalue.'" WHERE ID="'.$inid.'"';
