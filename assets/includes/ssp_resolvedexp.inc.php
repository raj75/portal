                   ...
                   // $row[ $column['dt'] ] = ($isJoin) ? $column['formatter']( $data[$i][ $column['field'] ], $data[$i] ) : $column['formatter']( $data[$i][ $column['db'] ], $data[$i] );
					...
					if($isJoin){
						if(isset($_SESSION["group_id"]) and ($_SESSION["group_id"] == 1 or $_SESSION["group_id"] == 2) and $column['field']=="active_date" ){	
							$row[ $column['dt'] ] = '<td><button onclick="loadsite(\''.$data[$i][0].'\')" title="View/Edit Site Details" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></button><button onclick="deletesite(\''.$data[$i][0].'\',\'sites\')" title="Delete Site" class="btn btn-xs btn-default"><i class="fa fa-times"></i></button></td>';
						}else{
							$row[ $column['dt'] ] = '<a href="javascript:void(0);" onclick="load_details(\''.$data[$i][0].'\')"> '.$column['formatter']( $data[$i][ $column['field'] ], $data[$i] ).'</a>';
						}
					}else{
