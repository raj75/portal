...
class SSP {
            ...
            for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
                ...
                if ( isset( $column['formatter'] ) ) {
                   // $row[ $column['dt'] ] = ($isJoin) ? $column['formatter']( $data[$i][ $column['field'] ], $data[$i] ) : $column['formatter']( $data[$i][ $column['db'] ], $data[$i] );
					...
					if($isJoin and $column['dt'] != "DT_RowId"){
						//if(isset($_SESSION["group_id"]) and ($_SESSION["group_id"] == 1 or $_SESSION["group_id"] == 2) and $column['field']=="active_date" ){
						if(isset($_SESSION["group_id"]) and ($_SESSION["group_id"] == 1 or $_SESSION["group_id"] == 2) and $column['field']=="active_date" ){
							//$row[ $column['dt'] ] = '<td><button onclick="loadsite(\''.$data[$i][1].'\')" title="View/Edit Site Details" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></button><button onclick="deletesite(\''.$data[$i][1].'\',\'sites\')" title="Delete Site" class="btn btn-xs btn-default"><i class="fa fa-times"></i></button></td>';
							$row[ $column['dt'] ] = '<td><button onclick="loadsite(\''.$data[$i][3].'\')" title="View/Edit Site Details" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></button><button onclick="deletesite(\''.$data[$i][3].'\',\'sites\')" title="Delete Site" class="btn btn-xs btn-default"><i class="fa fa-times"></i></button></td>';
						}else{
							...
							if(isset($_SESSION["group_id"]) and ($_SESSION["group_id"] == 1 or $_SESSION["group_id"] == 2)){
								$tempp=$tempp.' onmouseover="showversion(\''.addslashes($data[$i][0]).'\',\''.trim($column['dbnam']).'\',\''.$column['field'].'\',\''.$ts.'\',\'list-sites\',\'assets/ajax/list-sites.php?load=true\')" id="'.$ts.'" class="showversion-link"';
							}
							//$tempp=$tempp.' onclick="load_details(\''.$data[$i][1].'\')"> '.$column['formatter']( $data[$i][ $column['field'] ], $data[$i] ).'</a>';
							$tempp=$tempp.' onclick="load_details(\''.$data[$i][3].'\')"> '.$column['formatter']( $data[$i][ $column['field'] ], $data[$i] ).'</a>
							';
							*/

							$tempp = "";

							if ($column['field'] == 'site_number') {
								$tempp='<a href="javascript:void(0);" onclick="load_details(\''.$data[$i][3].'\')"> '.$column['formatter']( $data[$i][ $column['field'] ], $data[$i] ).'</a> ';
							} else {
								$tempp=$column['formatter']( $data[$i][ $column['field'] ], $data[$i] );
							}

							$undoicon = self::undoicon($columns, $data, $data[$i]['id'], $column['field']);

							//if(isset($_SESSION["group_id"]) and ($_SESSION["group_id"] == 1 or $_SESSION["group_id"] == 2) and !empty($undoicon)){
							if(!empty($undoicon)){
								$tempp=$tempp.'<a href="javascript:void(0);" onclick="showversion(\''.addslashes($data[$i][0]).'\',\''.trim($column['dbnam']).'\',\''.$column['field'].'\',\''.$ts.'\',\'list-sites\',\'assets/ajax/list-sites.php?load=true\')" id="'.$ts.'" class="showversion-link">'.$undoicon.'</a>
								<a href="javascript:void(0);" rel="popover" data-placement="top" data-original-title="<h4>Versions</h4>" data-content="None" data-html="true" id="p'.$ts.'"></a>';
							}
							$row[ $column['dt'] ] = $tempp;
						}
					}else{
    ...
    static function filter ( $request, $columns, &$bindings, $isJoin = false )
    {
        ...
        if(isset($request['columns']) and count($request['columns'])){
          for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
              $requestColumn = $request['columns'][$i];
              $columnIdx = array_search( $requestColumn['data'], $dtColumns );//die();
              $column = $columns[ $columnIdx ];
	...
	*/
	...
	static function undoicon($columns, $data, $id, $column ) {
		global $table;
		global $mysqli;
		//print_r($data);
		//die();
