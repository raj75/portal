...
class SSP {
                ...
                if ( isset( $column['formatter'] ) ) {
                   // $row[ $column['dt'] ] = ($isJoin) ? $column['formatter']( $data[$i][ $column['field'] ], $data[$i] ) : $column['formatter']( $data[$i][ $column['db'] ], $data[$i] );
					...
					if($isJoin){
							...
							if(isset($_SESSION["group_id"]) and ($_SESSION["group_id"] == 1 or $_SESSION["group_id"] == 2)){$tempp=$tempp.' onmouseover="showversion('.$data[$i][0].',\''.trim($column['dbnam']).'\',\''.$column['field'].'\',\''.$ts.'\',\'list-sites\',\'assets/ajax/list-sites.php?load=true\')" id="'.$ts.'" class="showversion-link"';}
							$tempp=$tempp.' onclick="load_details('.$data[$i][0].')"> '.$column['formatter']( $data[$i][ $column['field'] ], $data[$i] ).'</a>';
    ...
    static function simple ( $request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '', $having = '')
    {
		...
		*/
		$resTotalLength = SSP::sql_exec( $db,
            "SELECT iso_count FROM iso.iso_count"
        );
