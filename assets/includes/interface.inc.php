...
elseif(isset($_POST["userid"]) and isset($_POST["resetdefault"]) and $_SESSION["group_id"]==1)
	echo resetdefault($mysqli,$_POST["userid"]);
else
	echo false;
...
function update_permission($mysqli,$userid='',$uper='',$aper='',$user_one=0)
{
	...
	{
		if ($stmtcheck = $mysqli->prepare('SELECT u.user_id,u.company_id FROM user u where (u.usergroups_id = 5 OR u.usergroups_id = 3) and u.user_id="'.$_userid.'" and u.company_id=(SELECT usp.company_id FROM user usp WHERE usp.user_id= "'.$user_one.'") LIMIT 1')) {

//('SELECT u.id,u.company_id FROM user u where (u.usergroups_id = 5 OR u.usergroups_id = 3) and u.id="'.$_userid.'" and u.company_id=(SELECT usp.company_id FROM user usp WHERE usp.id= "'.$user_one.'")
...
//('SELECT ug.interface FROM `usergroups` ug, user u where u.usergroups_id=ug.id and u.id = "'.$mysqli->real_escape_string($userid).'" LIMIT 1')) {
					...
					if ($stmtt->num_rows > 0) {
						...
						if($_interface == $jsondata || $mysqli->real_escape_string($_interface)==$jsondata){
							//$stmtt->free_result();
							//$stmtt->close();
							//$stmt->free_result();
							//$stmt->close();
...
function resetdefault($mysqli,$userid='')
{
	$userid=(int)@trim($userid);

	if($userid != '' and $userid != 0 and $_SESSION["group_id"] == 1){
		$mysqli->query("delete from users_interface WHERE user_id='".$userid."'");
		return true;
	}
	return false;
}
