...
//('SELECT company_name FROM company WHERE id="'.$cid.'"  LIMIT 1');
	...
	if(isset($_FILES) and isset($_FILES["fiaddfilesupload"])){

		foreach($_FILES['fiaddfilesupload']['name'] as $ky => $vl){
			$fname[] = $vl;
		}
	}
	...
	if(count($sub_query)){
		...
		if(isset($_FILES) and isset($_FILES["fiaddfilesupload"])){
			...
			$infotaget = $s3Client->doesObjectExist('datahub360', 'resources/Clients/'.strip_tags($companyname).'/focus items/');
			if(!$infotaget)
			{
				$s3Client->putObject([
					'Bucket' => 'datahub360',
					'Key'    => 'resources/Clients/'.strip_tags($companyname).'/focus items/'
				]);
			}

			foreach($_FILES['fiaddfilesupload']['name'] as $ky => $vl){
					$file_name = $vl;
					$temp_file_location = $_FILES['fiaddfilesupload']['tmp_name'][$ky];


					$s3Client->putObject([
						'Bucket' => 'datahub360',
						'Key'    => 'resources/Clients/'.strip_tags($companyname).'/focus items/'.$file_name,
...
}else if(isset($_POST["fiauto"]) and isset($_POST["fisavename"]) and isset($_POST["fivalue"]) and $_POST["fisavename"] != "" and $_POST["fiauto"] != "" and $_POST["fiauto"] != 0)
{
	$error="Error occured";
	$new_value=array();

	$masterid = $mysqli->real_escape_string(@trim($_POST["fiauto"]));
	$masavename = str_replace("_"," ",$mysqli->real_escape_string(@trim($_POST["fisavename"])));
	$mavalue = $mysqli->real_escape_string(@trim($_POST["fivalue"]));


	if($masavename == "date_added"){
	...
	audit_log($mysqli,'focus_items','UPDATE',$new_value,'WHERE id='.$masterid,'','','id');
	$sql='Update focus_items SET `'.$masavename.'` = "'.$mavalue.'" WHERE id="'.$masterid.'"';
	$stmt = $mysqli->prepare($sql);
	if($stmt){
		$stmt->execute();
		$cnc=1;
