            ...
            for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
                ...
                if ( isset( $column['formatter'] ) ) {
                   // $row[ $column['dt'] ] = ($isJoin) ? $column['formatter']( $data[$i][ $column['field'] ], $data[$i] ) : $column['formatter']( $data[$i][ $column['db'] ], $data[$i] );
					...
					if($isJoin){
						if(isset($_SESSION["group_id"]) and ($_SESSION["group_id"] == 1 or $_SESSION["group_id"] == 2) and $column['field']=="modified_by" ){	
							$row[ $column['dt'] ] = '<td><button onclick="loadsch(\''.$data[$i][0].'\')" title="View/Edit Scheduled Reports" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></button><button onclick="deletesch(\''.$data[$i][0].'\',\'TicketID:\')" title="Delete Scheduled Report" class="btn btn-xs btn-default"><i class="fa fa-times"></i></button></td>';
						}else{
