...
if(isset($_POST["comment"]) and isset($_POST["parentid"]) and isset($_POST["blogid"]) and trim($_POST["comment"]) != "" and trim($_POST["parentid"]) != "" and trim($_POST["blogid"]) != "" and trim($_POST["blogid"]) != 0)
{
	...
	$stmt = $mysqli->prepare("INSERT INTO market_news_comments (user_id,comment,parent_id,market_news_id) VALUES ('".$mysqli->real_escape_string($user_one)."', '".$mysqli->real_escape_string($comment)."', '".$mysqli->real_escape_string($parentid)."', '".$mysqli->real_escape_string($blogid)."')") ;
	$stmt->execute();
	...
	if($lastinsertID > 0) {
		...
		$stmtsk = $mysqli->prepare('SELECT id,user_id,parent_id,market_news_id,datetime FROM market_news_comments where user_id='.$lastinsertID.' LIMIT 1');

//('SELECT id,user_id,parent_id,market_news_id,datetime FROM market_news_comments where id='.$lastinsertID.' LIMIT 1');
		...
		$stmtsks = $mysqli->prepare("SELECT id FROM market_news_comments where market_news_id='".$mysqli->real_escape_string($blogid)."'");
		$stmtsks->execute();
