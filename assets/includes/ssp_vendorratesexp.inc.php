...
class SSP {
                   ...
                   // $row[ $column['dt'] ] = ($isJoin) ? $column['formatter']( $data[$i][ $column['field'] ], $data[$i] ) : $column['formatter']( $data[$i][ $column['db'] ], $data[$i] );
					...
					if($isJoin){
						if(isset($_SESSION["group_id"]) and ($_SESSION["group_id"] == 1 or $_SESSION["group_id"] == 2) and $column['field']=="active_date" ){	
							$row[ $column['dt'] ] = '<td><button onclick="veload_details(\''.$data[$i][1].'\')" title="View/Edit Site Details" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></button><button onclick="deletesite(\''.$data[$i][1].'\',\'sites\')" title="Delete Site" class="btn btn-xs btn-default"><i class="fa fa-times"></i></button></td>';
						}else{
							$tempp='<a href="javascript:void(0);"';

							if(isset($_SESSION["group_id"]) and ($_SESSION["group_id"] == 1 or $_SESSION["group_id"] == 2)){$tempp=$tempp.' onmouseover="showversion(\''.addslashes($data[$i][0]).'\',\''.trim($column['dbnam']).'\',\''.$column['field'].'\',\''.$ts.'\',\'vendor-rates-pedit\',\'assets/ajax/vendor-rates-pedit.php?load=true\')" id="'.$ts.'" class="showversion-link"';}
							$tempp=$tempp.' onclick="veload_details(\''.$data[$i][1].'\')"> '.$column['formatter']( $data[$i][ $column['field'] ], $data[$i] ).'</a>';
						...
						$tempp='<a href="javascript:void(0);"';
						$tempp=$tempp.' onmouseover="showversion(\''.addslashes($data[$i][0]).'\',\''.trim($column['dbnam']).'\',\''.$column['field'].'\',\''.$ts.'\',\'vendor-rates-pedit\',\'assets/ajax/vendor-rates-pedit.php?load=true\')" id="'.$ts.'" class="showversion-link"';
						$tempp=$tempp.' onclick="veload_details(\''.$data[$i][0].'\')"> '.$column['formatter']( $data[$i][ $column['field'] ], $data[$i] ).'</a>';

						$tempp=$tempp.'<a href="javascript:void(0);" rel="popover" data-placement="top" data-original-title="<h4>Versions</h4>" data-content="None" data-html="true" id="p'.$ts.'"></a>';
						$row[ $column['dt'] ] = $tempp;
    ...
    static function filter ( $request, $columns, &$bindings, $isJoin = false )
    {
		...
		if(isset($request['columns']) and is_array($request['columns'])){
