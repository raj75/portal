                   ...
                   // $row[ $column['dt'] ] = ($isJoin) ? $column['formatter']( $data[$i][ $column['field'] ], $data[$i] ) : $column['formatter']( $data[$i][ $column['db'] ], $data[$i] );
					...
					if($isJoin){
						if(isset($_SESSION["group_id"]) and ($_SESSION["group_id"] == 1 or $_SESSION["group_id"] == 2) and $column['field']=="active_date" ){	
							$row[ $column['dt'] ] = '<td><button onclick="sgload_details(\''.$data[$i][1].'\')" title="View/Edit Site Details" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></button><button onclick="deletesite(\''.$data[$i][1].'\',\'sites\')" title="Delete Site" class="btn btn-xs btn-default"><i class="fa fa-times"></i></button></td>';
						}else{
							$tempp='<a href="javascript:void(0);"';

							if(isset($_SESSION["group_id"]) and ($_SESSION["group_id"] == 1 or $_SESSION["group_id"] == 2)){$tempp=$tempp.' onmouseover="showversion(\''.addslashes($data[$i][0]).'\',\''.trim($column['dbnam']).'\',\''.$column['field'].'\',\''.$ts.'\',\'service-group-pedit\',\'assets/ajax/service-group-pedit.php?load=true\')" id="'.$ts.'" class="showversion-link"';}
							$tempp=$tempp.' onclick="sgload_details(\''.$data[$i][0].'\')"> '.$column['formatter']( $data[$i][ $column['field'] ], $data[$i] ).'</a>';
						...
						$tempp='<a href="javascript:void(0);"';
						$tempp=$tempp.' onmouseover="showversion(\''.addslashes($data[$i][0]).'\',\''.trim($column['dbnam']).'\',\''.$column['field'].'\',\''.$ts.'\',\'service-group-pedit\',\'assets/ajax/service-group-pedit.php?load=true\')" id="'.$ts.'" class="showversion-link"';
						$tempp=$tempp.' onclick="sgload_details(\''.$data[$i][0].'\')"> '.$column['formatter']( $data[$i][ $column['field'] ], $data[$i] ).'</a>';
