...
class SSP {
                ...
                if ( isset( $column['formatter'] ) ) {
                   // $row[ $column['dt'] ] = ($isJoin) ? $column['formatter']( $data[$i][ $column['field'] ], $data[$i] ) : $column['formatter']( $data[$i][ $column['db'] ], $data[$i] );
					...
					if($isJoin){
							...
							if(isset($_SESSION["group_id"]) and ($_SESSION["group_id"] == 1 or $_SESSION["group_id"] == 2)){$tempp=$tempp.' onmouseover="showversion('.$data[$i][0].',\''.trim($column['dbnam']).'\',\''.$column['field'].'\',\''.$ts.'\',\'start-stop-status-pedit\',\'assets/ajax/start-stop-status-pedit.php?load=true\')" id="'.$ts.'" class="showversion-link"';}
							$tempp=$tempp.' onclick="sssload_details('.$data[$i][0].')"> '.$column['formatter']( $data[$i][ $column['field'] ], $data[$i] ).'</a>';
    ...
    static function filter ( $request, $columns, &$bindings, $isJoin = false )
    {
		...
		if ( isset($request['columns']) && count($request['columns']) ) {
			...
			for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
				...
				if ( $requestColumn['searchable'] == 'true' &&
					$str != '' ) {
					if($columnIdx==3) $srch=$str;	
					else $srch='%'.$str.'%';
					
					//$binding = SSP::bind( $bindings, $srch, PDO::PARAM_STR );
							//$binding = SSP::bind( $bindings, '%'.$str.'%', PDO::PARAM_STR );
					//$columnSearch[] = ($isJoin) ? $column['db']." LIKE ".$binding : "`".$column['db']."` LIKE ".$binding;
					
					if($column['db']=="`up`.`firstname`"){
						$columnSearch[] = ($isJoin) ? " CONCAT( firstname, ' ', lastname ) LIKE '%".$str."%' " : "`".$column['db']."` LIKE '%".$str."%' ";
					}else{
						$binding = SSP::bind( $bindings, $srch, PDO::PARAM_STR );
						$columnSearch[] = ($isJoin) ? $column['db']." LIKE ".$binding : "`".$column['db']."` LIKE ".$binding;
					}
...
//print_r($where);die();
    ...
    static function simple ( $request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '', $having = '')
    {
        ...
        if($joinQuery){
            ...
            $query =  "SELECT SQL_CALC_FOUND_ROWS ".implode(", ", $col)." FROM $joinQuery $where $extraWhere $groupBy $having $order $limit";
        }else{
            $query =  "SELECT SQL_CALC_FOUND_ROWS `".implode("`, `", SSP::pluck($columns, 'db'))."` FROM `$table` $where
			 $extraWhere
			 $groupBy
       $having
			 $order
...
//die($query);
		...
		if(!isset($request['draw'])) $request['draw']=1;
        /*
         * Output
         */
        return array(
