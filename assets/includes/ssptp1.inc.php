                   ...
                   // $row[ $column['dt'] ] = ($isJoin) ? $column['formatter']( $data[$i][ $column['field'] ], $data[$i] ) : $column['formatter']( $data[$i][ $column['db'] ], $data[$i] );
					...
					if($isJoin){
						if($column['field']=="region" ){
							$row[ $column['dt'] ] = '<td><button onclick="selectsymbol(\''.$data[$i][1].'\')" title="Add" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></button></td>';
						}else{
							$row[ $column['dt'] ]=$column['formatter']( $data[$i][ $column['field'] ], $data[$i] );
						}
					}else{
						if($column['field']=="region" ){
							$row[ $column['dt'] ] = '<td class="tcenter"><span class="glyphicon glyphicon-plus blue selectsymtp1" exchgn="'.$data[$i][1].'" symbl="'.$data[$i][2].'"></span></td>';
						}else{
