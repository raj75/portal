...
if(isset($_GET["action"]) and $_GET["action"]=="region" and isset($_GET["region"]) and !empty($_GET["region"])){
	$trand=rand(20,500)."tp4t".rand(20,500);
	$regiontype=urldecode($_GET["region"]);
...
<style>
#datatable_fixed_column4-1<?php echo $trand; ?>_filter, #datatable_fixed_column4-1<?php echo $trand; ?>-4_filter,.dataTables_filter{
...
#datatable_fixed_column4-1<?php echo $trand; ?>_length,#datatable_fixed_column4-1<?php echo $trand; ?>-4_length,.dataTables_length{
float: right !important;
margin: 1% 1% !important;
}
...
#datatable_fixed_column4-1<?php echo $trand; ?>,#datatable_fixed_column4-1<?php echo $trand; ?>-4,.innerdtable{border-bottom: 1px solid #ccc !important;}}
#datatable_fixed_column4-1<?php echo $trand; ?> option,#datatable_fixed_column4-1<?php echo $trand; ?>-4 option,.innerdtable option {
  color: #555;
}
#datatable_fixed_column4-1<?php echo $trand; ?> tr.dropdown select, #datatable_fixed_column4-1<?php echo $trand; ?>-4  tr.dropdown select,.innerdtable  tr.dropdown select{font-weight: 400 !important;}
...
tr.shown td.details-control-4-4 {
}
			...
			<div class="jarviswidget jarviswidget-color-blueDark" id="wid-id-2<?php echo $trand; ?>" data-widget-editbutton="false">
				...
				<div>
					...
					<div class="widget-body no-padding symlist">
						<table id="datatable_fixed_column4-1<?php echo $trand; ?>" class="table table-striped table-bordered table-hover" width="100%">
							<thead>
								<tr class="dropdown">
...
<?php
	...
	if($regiontype == "All") $subquery=""; else $subquery=" AND Region='".$mysqli->real_escape_string($regiontype)."' ";
	$sql="SELECT DISTINCT Product,IF (clearing_code IS NULL,Region,clearing_code) AS clearing_code,Description,TradingPoint,date_code_min,date_code_max,max_date,contracts,spot_contract,spot_price,12_strip,HYPERLINK,status FROM (
SELECT Region,clearing_code,Description,TradingPoint,date_code_min,date_code_max,max_date,contracts,spot_contract,spot_price,12_strip,HYPERLINK,`GROUP`,Product,contract_type,status FROM ubm_ice.clearing_code UNION ALL
SELECT DISTINCT Region,NULL AS clearing_code,NULL AS Description,NULL AS TradingPoint,NULL AS date_code_min,NULL AS date_code_max,NULL AS max_date,NULL AS contracts,NULL AS spot_contract,NULL AS spot_price,NULL AS 12_strip,NULL AS HYPERLINK,`GROUP`,NULL AS Product,contract_type,status FROM ubm_ice.clearing_code) a WHERE `GROUP`='Natural Gas' AND contract_type='Monthly' ".$subquery." ORDER BY a.Region,a.TradingPoint";
	if($stmt = $mysqli->prepare($sql)) {
        $stmt->execute();
        $stmt->store_result();
        if($stmt->num_rows > 0) {
					$cct=1;
			$stmt->bind_result($productcode,$tpclearing_code,$tpDescription,$tpTradingPoint,$tpdate_code_min,$tpdate_code_max,$tpmax_date,$tpcontracts,$tpspot_contract,$tpspot_price,$tp12_strip,$tpHYPERLINK,$status);
			while($stmt->fetch()) {if($productcode == "") continue;
?>
							<tr>
								<td><i class="clickit glyphicon glyphicon-chevron-right" id="iid<?php echo $cct; ?>"></i></td>
...
<script type="text/javascript">
	...
	var pagefunction = function() {
		...
		/* COLUMN FILTER  */
			var otable<?php echo $trand; ?> = $("#datatable_fixed_column4-1<?php echo $trand; ?>").DataTable( {
	    ...
	    // Apply the filter
	    $("#datatable_fixed_column4-1<?php echo $trand; ?> thead th input[type=text]").on( 'keyup change', function () {
			...
			$('#datatable_fixed_column4-1<?php echo $trand; ?>').off('keyup change', '.tp5');
			$('#datatable_fixed_column4-1<?php echo $trand; ?>').on('keyup change', '.tp5', function () {
		...
		$('#datatable_fixed_column4-1<?php echo $trand; ?> tbody').off('click', '.clickit');
		$('#datatable_fixed_column4-1<?php echo $trand; ?> tbody').on('click', '.clickit', function () {
			var tthis=$(this);
			...
			parent.$("#tpchartcont4<?php echo $innertablect; ?>-4dtable").html('');
			if ( row.child.isShown() ) {
				...
				//var otable<?php echo $trand; ?>s = $("#datatable_fixed_column4-1<?php echo $trand; ?>-4", row.child());
				//otable<?php echo $trand; ?>s.detach();
				//otable<?php echo $trand; ?>s.DataTable().destroy();
				...
				var otable<?php echo $trand; ?>s = $("#datatable_fixed_column4-1<?php echo $trand; ?>-4"+randno).DataTable( {
					...
					"paging": true,
					"ajax": {
						"url": "assets/ajax/subpages/trading-platform-4-1-serverside.php?req=pjm&cc="+oarr["Clearing Code"]
					},
					"rowId": '17',
					"columnDefs": [
				...
				otable<?php echo $trand; ?>s.on( 'click', 'tr', function () {
					...
					//$("#resulti4table").remove( );
					$("#resulti4chart").remove( );
					//var tsrc="assets/ajax/subpages/trading-platform-4-1-1.php?req=innerstable&cc="+innerstablearr.join(',');
					var csrc="assets/ajax/subpages/trading-platform-4-1-1.php?req=innerschart&cc="+innerstablearr.join(',');
					rthis.parents('.innerddiv').after('<div id="resulti4chart"><iframe width="100%" height="425px" src="'+csrc+'"></iframe></div>');
					//rthis.parents('.innerddiv').after('<div id="resulti4table"></div>');
					//$("#resulti4table").load("assets/ajax/subpages/trading-platform-4-1-1.php?req=innerstable&cc="+innerstablearr.join(','), function() {
				} );
	...
	$('document').off('click', '.tp4itable');
	$('document').on('click', '.tp4itable', function () {alert("dk");
		//$(this).parents('tr').prev().find('.clickit').trigger("click");
	//tp4itable
	});
...
//}
	...
	{
		var items=[], options=[];
		$('#datatable_fixed_column4-1<?php echo $trand; ?> tbody tr td:nth-child('+indexno+')').each( function(){
