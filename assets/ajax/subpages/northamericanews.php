...
$feed=array();
require_once realpath(__DIR__ . '/../../plugins/env/autoload.php');
...
$conn = mysqli_connect($_ENV['HOST'], $_ENV['DBUSER'],$_ENV['DBPASSWORD'],$_ENV['DATABASE']);
if (!$conn) {
    printf("Connect failed: %s\n", mysqli_connect_errno());
    exit();
}

if ($ckmainstmt = mysqli_query($conn,"SELECT id,title,image,description,url,published_date,logo FROM ".$tablename." WHERE newstype='NorthAmerica' and published_date >= DATE(NOW()) - INTERVAL 7 DAY order by published_date desc LIMIT 0,63")) {
...
<?php for ( $x = 0; $x < $limit; $x++ ) {//print_r($feed[ $x ]);die();
   ...
   // $img = ($feed[ $x ]['image'])?$feed[ $x ]['image']:"");
	...
	/*$tempdesc=preg_replace("/(<img)([^<>]+src=[\'\"]{1})/s","$1 style='width:47%;height:auto;max-height:20%;' $2".$imageloc,$feed[ $x ]['desc']);

	if($feed[ $x ]['logo']==1) $rsslogo=$feed[ $x ]['logo'].".jpg"; else $rsslogo=$feed[ $x ]['logo'];

	//$tempdesc=preg_replace("/(<img[^<>]+src=[\'\"]{1}1[\'\"]{1})/s","$1 style='width:20%;height:auto;max-height:20%;' ".$imageloc,$feed[ $x ]['desc']);
	...
	$tempdesc=preg_replace("/(<img)([^<>]+src=[\'\"]{1})/s","$1 style='width:47%;height:auto;max-height:46%;' $2".$imageloc,$feed[ $x ]['desc']);

	if($feed[ $x ]['logo']==1) $rsslogo=$feed[ $x ]['logo'].".jpg"; else $rsslogo=$feed[ $x ]['logo'];

	if(!empty(@trim($rsslogo))) $tempdesc="<img src='".$imageloc."logo/".$rsslogo."' style='width: 11%;height: auto;max-height: 20%;margin-bottom: -8px;margin-top: -9px;'><hr><br>".$tempdesc;

	$tempdesc="<p>Source: <a href='".$feed[ $x ]['link']."' target='_blank'>".$feed[ $x ]['link']."</a></p>".$tempdesc;

	$tempdesc="<style>h2,h3,h4{font-weight:bold;} ul#main-image{list-style:none;}</style><p style='font-size:24px;font-weight:bold;/*margin-bottom: -5px;*/'>".$title."</p>".$tempdesc;
	...
	origColumnHeight = imageColumn.height();
		...
		function scrollColumn() {
			imageColumn.css({'top': '0'})
					   .animate({top: -origColumnHeight},180000, 'linear', scrollColumn);
	  ...
	  $(document).on("click",".naplink",function() {
		  var naattr=$( this ).attr("id");
		  var natempcontent= $( "#nadesc"+naattr ).html();
		  var natemptitle= $( "#nafgtitle"+naattr ).html();
			parent.$("#sndialog").html(natempcontent);
			//parent.$("#sndialog").dialog("option","title",natemptitle).dialog("open");
			parent.$("#sndialog").dialog("open");
			return false;
	  });
</script>
</html>
...
function cleanHtml($html)
{
    ...
    $html = preg_replace('/href="[^<>\"]+"/s', 'href="javascript:void(0);"', $html);
    $html = preg_replace("/href='[^\']+'/s", "href='javascript:void(0);'", $html);
    $html = preg_replace("/padding-bottom:[0-9\.]+\%/s", "", $html);
    $html = preg_replace("/<div[^<>]+>Get CyberSEO Lite[\s]*\([^\*]*<\/div>/s", "", $html);
