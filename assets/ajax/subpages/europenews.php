...
.desctitle{font-size: 27px;font-weight: bold;}
</style>
</head>
<body>
<?php
...
if ($ckmainstmt = mysqli_query($conn,"SELECT id,title,image,description,url,published_date,logo FROM ".$tablename." WHERE newstype='Europe' and published_date >= DATE(NOW()) - INTERVAL 7 DAY order by published_date desc LIMIT 0,63")) {
...
<?php for ( $x = 0; $x < $limit; $x++ ) {//print_r($feed[ $x ]);die();
   ...
   // $img = ($feed[ $x ]['image'])?$feed[ $x ]['image']:"");
	...
	if(isset($feed[ $x ]['image']) and !empty($feed[ $x ]['image'])) $tempdesc="<img src='".$imageloc.$feed[ $x ]['image']."' style='width:47%;height:auto;max-height:46%;'><br>".$tempdesc;
	...
	$tempdesc="<p style='font-size:24px;font-weight:bold;/*margin-bottom: -5px;*/'>".$title."</p>".$tempdesc;
	...
	origColumnHeight = imageColumn.height();
	  ...
	  $(document).on("click",".naplink",function() {
		  ...
		  var natemptitle= $( "#nafgtitle"+naattr ).html();
			...
			//parent.$("#dialog").dialog("option","title",natemptitle).dialog("open");
