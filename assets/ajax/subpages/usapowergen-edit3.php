<?php
require_once '../../includes/db_connect.php';
require_once '../../includes/functions.php';


if(!isset($_SESSION))
	sec_session_start();

if(!isset($_SESSION["user_id"]))
		die("<h5 style='padding-top:30px;' align='center'>Restricted Access!</h5>");

$user_one=$_SESSION["user_id"];

if(!isset($_SESSION['group_id']) and ($_SESSION['group_id'] != 1 or $_SESSION['group_id'] != 2 or $_SESSION['group_id'] != 3 or $_SESSION['group_id'] != 5))
	die("<h5 style='padding-top:30px;' align='center'>Restricted Access!</h5>");


function getjsondata($keyname){
  return "getjson.php?fname=".$keyname;
}
//$usenergyjson="https://".$_SERVER['HTTP_HOST'] ."/assets/ajax/subpages/files/us-energy.geojson";
//$usenergyjson="https://datahub360-powergeneration.s3.us-west-2.amazonaws.com/us-energy.geojson";
$usenergyjson=getjsondata("us-energy.geojson");
/*
$stateabbr_arr=$ulist=array();
$stmt_dsiremap = $mysqli->prepare("select DISTINCT `Plant ID` FROM usapowergeneration.Operating WHERE `Nameplate Capacity (MW)` IS NOT NULL GROUP BY `Plant ID`,`Plant Name`, `Generator ID`");
$stmt_dsiremap->execute();
$stmt_dsiremap->store_result();
if ($stmt_dsiremap->num_rows > 0) {
  $stmt_dsiremap->bind_result($ulplid);
  while($stmt_dsiremap->fetch()) {
    $ulist[]=$ulplid;
  }
}

foreach($ulist as $vvl){
  $stmt_dsiremap = $mysqli->prepare("select `Plant ID`,`Plant Name`,Technology,`Prime Mover Code`, SUM(`Nameplate Capacity (MW)`),CONCAT(MIN(startyear),'-',MAX(endyear)),`Plant State`,Latitude,Longitude FROM (select `Plant ID`,`Plant Name`, `Generator ID`,`Nameplate Capacity (MW)`,Technology,`Prime Mover Code`,`Operating Year` AS startyear,`Operating Year` AS endyear,`Plant State`,Latitude,Longitude FROM usapowergeneration.Operating WHERE `Plant ID`= ".$vvl." AND `Nameplate Capacity (MW)` IS NOT NULL GROUP BY `Plant ID`,`Plant Name`, `Generator ID`) AS Total");
  $stmt_dsiremap->execute();
  $stmt_dsiremap->store_result();
  if ($stmt_dsiremap->num_rows > 0) {
    $stmt_dsiremap->bind_result($uplid,$uplname,$utech,$upmc,$ucapacity,$uyear,$ustate,$ulat,$ulong);
    while($stmt_dsiremap->fetch()) {
      //if($stateabbr == "") continue;
      $stateabbr_arr[]=array("id"=>$uplid,"name"=>$uplname,"tech"=>$utech,"pmc"=>$upmc,"capacity"=>$ucapacity,"year"=>$uyear,"state"=>$ustate,"lat"=>$ulat,"long"=>$ulong);
    }
  }
}

//print_r($stateabbr_arr);

if(!count($stateabbr_arr)) die("Nothing to show!");
*/
?>

        <!DOCTYPE html>
            <meta charset="utf-8">
            <head>
                <meta http-equiv="Content-Security-Policy" content="upgrade-insecure-requests">
                <link rel="stylesheet" href="./files/style.css?ct=<?php echo time(); ?>" />
                <link rel="stylesheet" href="./files/leaflet.css" />
                <link href='https://fonts.googleapis.com/css?family=PT+Sans+Narrow:400,700|PT+Serif:400,400italic' rel='stylesheet' type='text/css'>
                <link rel="stylesheet" href="./files/leaflet.zoomhome.css"/>
                <link rel="stylesheet" href="./files/easy-button.css">
                <link rel="stylesheet" href="https://<?php echo $_SERVER['HTTP_HOST']; ?>/assets/plugins/fontawesome/css/fontawesome.min.css" crossorigin="anonymous">
                <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.1/jquery.min.js"></script>
                <script src="./files/leaflet.js"></script>
                <script src="./files/leaflet.zoomhome.min.js"></script>
                <script src="./files/d3.v3.js"></script>
                <script src="./files/easy-button.js"></script>
                <style>
                  .noshow{display:none !important;}
                </style>


            </head>
            <body>
                <div id="container">
                    <div id="side-bar" class="side-bar">
                            <div id="side-bar-toggle">
                                    <a href="#" class="toggle"><span class="arrow-right-hidden">&#10095;</span><span class="arrow-left">&#10094;</span></a>
                            </div>
                            <div class="links-container">
                                <div class='selector_menu'id="selector_menu_1">
                                    <select id='selector1'>
                                        <option value='All' selected='selected'>All states</option>
                                        <option value='Alabama' >Alabama</option>
                                        <option value='Alaska' >Alaska</option>
                                        <option value='Arizona' >Arizona</option>
                                        <option value='Arkansas' >Arkansas</option>
                                        <option value='California' >California</option>
                                        <option value='Colorado' >Colorado</option>
                                        <option value='Connecticut' >Connecticut</option>
                                        <option value='Delaware' >Delaware</option>
                                        <option value='Florida' >Florida</option>
                                        <option value='Georgia' >Georgia</option>
                                        <option value='Hawaii' >Hawaii</option>
                                        <option value='Idaho' >Idaho</option>
                                        <option value='Illinois' >Illinois</option>
                                        <option value='Indiana' >Indiana</option>
                                        <option value='Iowa' >Iowa</option>
                                        <option value='Kansas' >Kansas</option>
                                        <option value='Kentucky' >Kentucky</option>
                                        <option value='Louisiana' >Louisiana</option>
                                        <option value='Maine' >Maine</option>
                                        <option value='Maryland' >Maryland</option>
                                        <option value='Massachusetts' >Massachusetts</option>
                                        <option value='Michigan' >Michigan</option>
                                        <option value='Minnesota' >Minnesota</option>
                                        <option value='Mississippi' >Mississippi</option>
                                        <option value='Missouri' >Missouri</option>
                                        <option value='Montana' >Montana</option>
                                        <option value='Nebraska' >Nebraska</option>
                                        <option value='Nevada' >Nevada</option>
                                        <option value='NewHampshire' >New Hampshire</option>
                                        <option value='NewJersey' >New Jersey</option>
                                        <option value='NewMexico' >New Mexico</option>
                                        <option value='NewYork' >New York</option>
                                        <option value='NorthCarolina' >North Carolina</option>
                                        <option value='NorthDakota' >North Dakota</option>
                                        <option value='Ohio' >Ohio</option>
                                        <option value='Oklahoma' >Oklahoma</option>
                                        <option value='Oregon' >Oregon</option>
                                        <option value='Pennsylvania' >Pennsylvania</option>
                                        <option value='RhodeIsland' >Rhode Island</option>
                                        <option value='SouthCarolina' >South Carolina</option>
                                        <option value='SouthDakota' >South Dakota</option>
                                        <option value='Tennessee' >Tennessee</option>
                                        <option value='Texas' >Texas</option>
                                        <option value='Utah' >Utah</option>
                                        <option value='Vermont' >Vermont</option>
                                        <option value='Virginia' >Virginia</option>
                                        <option value='Washington' >Washington</option>
                                        <option value='WestVirginia' >West Virginia</option>
                                        <option value='Wisconsin' >Wisconsin</option>
                                        <option value='Wyoming' >Wyoming</option>
                                    </select>

                                </div>
                                <div class='selector_menu'id="selector_menu_2">
                                    <select id='selector2'>
                                        <option value='All' selected='selected'>All types</option>
                                        <option value='LowCarbon' >Low carbon</option>
                                        <option value='HighCarbon' >High carbon</option>
                                        <option value='Biomass' >Biomass</option>
                                        <option value='Coal' >Coal</option>
                                        <option value='Gas' >Gas</option>
                                        <option value='Geothermal' >Geothermal</option>
                                        <option value='Hydro' >Hydro</option>
                                        <option value='Nuclear' >Nuclear</option>
                                        <option value='Oil' >Oil</option>
                                        <option value='Other' >Other</option>
                                        <option value='Solar' >Solar</option>
                                        <option value='Storage' >Storage</option>
                                        <option value='Waste' >Waste</option>
                                        <option value='Wind' >Wind</option>
                                    </select>
                                </div>
                                <div class='selector_menu'id="selector_menu_4">
                                    <select id='selector4'>
                                        <option value='All' selected='selected'>All status</option>
                                        <option value='Operating' >Operating</option>
                                        <option value='Retired' >Retired</option>
                                        <option value='Planned' >Planned</option>
                                    </select>
                                </div>
                                <div class='selector_menu' id="selector_menu_3">
                                  <select id="basemaps">
                                      <option value="day" selected>Satellite imagery day</option>
                                      <option value="NASAGIBS_ViirsEarthAtNight2012">Satellite imagery night</option>
                                  </select>
                                </div>
                            </div>
                            <div id="title-wrapper">
                                <h1 id="title">US electricity sources</h1>
                            </div>

                            <div id="rainbow-wrapper">
                                <div class="rainbow" id="r12"></div><div class="rainbow" id="r11"></div><div class="rainbow" id="r10"></div><div class="rainbow" id="r9"></div><div class="rainbow" id="r8"></div><div class="rainbow" id="r7"></div><div class="rainbow" id="r6"></div><div class="rainbow" id="r5"></div><div class="rainbow" id="r4"></div><div class="rainbow" id="r3"></div><div class="rainbow" id="r2"></div><div class="rainbow" id="r1"></div>
                            </div>
                            <div id="subtitle-wrapper">
                                <h2 id="subtitle">All states</h2>
                                <div id="state-icon"></div>
                                <p><span class="total" id="total">1,197,676</span><span class="total" id="total-label"> MW</span></p>
                            </div>
                            <div id="chart-1">
                            </div>
                            <div id="chart-2">
                            </div>
                            <div id="key-desktop">
                                <h3 id="key-title">Capacity</h3>
                                <div id="key-wrapper">
                                    <div class="key-label"><p>6,495 MW</p></div>
                                    <div class="key-image"><img src="./files/key-desktop.png" alt="map key"/></div>
                                    <div class="key-label"><p>0 MW</p></div>
                                </div>
                            </div>
                    </div>

                        <div id="map-wrapper">
                                <div id="mapid"></div>
                        </div>
                </div>
                <script>
                // set bounds

                var southWest = new L.LatLng(28.8350, -131.5),
                northEast = new L.LatLng(45.514, -76),
                bounds = new L.LatLngBounds(southWest, northEast);

                // create map

                var map = L.map('mapid', {zoomControl: false}).fitBounds(bounds, {padding: [20, 20]})

                /*var Esri_WorldGrayCanvas = L.tileLayer('http://server.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Light_Gray_Base/MapServer/tile/{z}/{y}/{x}', {
                	attribution: 'Tiles &copy; Esri &mdash; Esri, DeLorme, NAVTEQ',
                	maxZoom: 16
                });

                var Esri_WorldImagery = L.tileLayer('http://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', {
                	attribution: 'Tiles &copy; Esri &mdash; Source: Esri, i-cubed, USDA, USGS, AEX, GeoEye, Getmapping, Aerogrid, IGN, IGP, UPR-EGP, and the GIS User Community'
                });
                var NASAGIBS_ViirsEarthAtNight2012 = L.tileLayer('http://map1.vis.earthdata.nasa.gov/wmts-webmerc/VIIRS_CityLights_2012/default/{time}/{tilematrixset}{maxZoom}/{z}/{y}/{x}.{format}', {
                	attribution: 'Imagery provided by services from the Global Imagery Browse Services (GIBS), operated by the NASA/GSFC/Earth Science Data and Information System (<a href="https://earthdata.nasa.gov">ESDIS</a>) with funding provided by NASA/HQ.',
                	bounds: [[-85.0511287776, -179.999999975], [85.0511287776, 179.999999975]],
                	minZoom: 1,
                	maxZoom: 8,
                	format: 'jpg',
                	time: '',
                	tilematrixset: 'GoogleMapsCompatible_Level'
                }).addTo(map);
                */
                var NASAGIBS_ViirsEarthAtNight2012 = L.tileLayer('http://map1.vis.earthdata.nasa.gov/wmts-webmerc/VIIRS_CityLights_2012/default/{tilematrixset}{maxZoom}/{z}/{y}/{x}.{format}', {
                	attribution: '',
                	bounds: [[-85.0511287776, -179.999999975], [85.0511287776, 179.999999975]],
                	minZoom: 1,
                	maxZoom: 8,
                	format: 'jpg',
                	time: '',
                	tilematrixset: 'GoogleMapsCompatible_Level'
                });

                var Esri_WorldImagery = L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', {
                	attribution: ''
                }).addTo(map);
                var Stamen_TonerHybrid = L.tileLayer('https://stamen-tiles-{s}.a.ssl.fastly.net/toner-hybrid/{z}/{x}/{y}{r}.{ext}', {
                	attribution: '',
                	subdomains: 'abcd',
                	minZoom: 0,
                	maxZoom: 20,
                	ext: 'png'
                }).addTo(map);

                $("#basemaps").change(function () {
                    /*if (this.value == "Esri_WorldGrayCanvas") {
                        map.removeLayer(Esri_WorldImagery)
                        .removeLayer(NASAGIBS_ViirsEarthAtNight2012)
                        .addLayer(Esri_WorldGrayCanvas);

                        //change position of logo as attribution changes

                        $("#logo-laptop").css({top: 745});
                        $("#logo-tablet").css({top: 600});
                        $("#logo-phablet").css({top: 590});
                        $("#logo-mobile").css({top: 515});
                    }
                    else if (this.value == "Esri_WorldImagery") {
                        map.removeLayer(Esri_WorldGrayCanvas)
                        .removeLayer(NASAGIBS_ViirsEarthAtNight2012)
                        .addLayer(Esri_WorldImagery);

                        //console.log("satellite day change");

                        $("#logo-laptop").css({top: 745});
                        $("#logo-tablet").css({top: 600});
                        $("#logo-phablet").css({top: 575});
                        $("#logo-mobile").css({top: 480});

                    }
                    else*/ if (this.value == "day") {
                        map.removeLayer(NASAGIBS_ViirsEarthAtNight2012)
                        .addLayer(Esri_WorldImagery)
                        .addLayer(Stamen_TonerHybrid);

                        /*$("#logo-laptop").css({top: 725});
                        $("#logo-tablet").css({top: 580});
                        $("#logo-phablet").css({top: 555});
                        $("#logo-mobile").css({top: 465});*/
                    }else if (this.value == "NASAGIBS_ViirsEarthAtNight2012") {
                        map.removeLayer(Esri_WorldImagery)
                        .removeLayer(Stamen_TonerHybrid)
                        .addLayer(NASAGIBS_ViirsEarthAtNight2012);

                        /*$("#logo-laptop").css({top: 725});
                        $("#logo-tablet").css({top: 580});
                        $("#logo-phablet").css({top: 555});
                        $("#logo-mobile").css({top: 465});*/
                    }
                });

                //disable scroll wheel zoom

                map.scrollWheelZoom.disable();

                var states = [
                    { "type": "Feature", "id": 0, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3514" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -89.599409, 48.010274 ], [ -89.488885, 48.013439 ], [ -89.522695, 47.960535 ], [ -89.613698, 47.819252 ], [ -89.728006, 47.641976 ], [ -89.842831, 47.464726 ], [ -89.957656, 47.286907 ], [ -90.131754, 47.292747 ], [ -90.305852, 47.298018 ], [ -90.479950, 47.303857 ], [ -90.654048, 47.309128 ], [ -90.857782, 47.212829 ], [ -91.060974, 47.117047 ], [ -91.264708, 47.021265 ], [ -91.468442, 46.924966 ], [ -91.592259, 46.876260 ], [ -91.716618, 46.827607 ], [ -91.840978, 46.778385 ], [ -91.964794, 46.729706 ], [ -92.011898, 46.711723 ], [ -92.274879, 46.656145 ], [ -92.264828, 46.095223 ], [ -92.296583, 46.096282 ], [ -92.543700, 45.985695 ], [ -92.756968, 45.889912 ], [ -92.899828, 45.705763 ], [ -92.689221, 45.518436 ], [ -92.765417, 45.267082 ], [ -92.766477, 44.996143 ], [ -92.796656, 44.776027 ], [ -92.505072, 44.583920 ], [ -92.385492, 44.574928 ], [ -92.062153, 44.432585 ], [ -91.949989, 44.364838 ], [ -91.879606, 44.257428 ], [ -91.627709, 44.085449 ], [ -91.289590, 43.937293 ], [ -91.257292, 43.854739 ], [ -91.254657, 43.613979 ], [ -91.228199, 43.501247 ], [ -92.540005, 43.519773 ], [ -94.001027, 43.513417 ], [ -95.359936, 43.500187 ], [ -96.452660, 43.501789 ], [ -96.439431, 44.435763 ], [ -96.560612, 45.393018 ], [ -96.735769, 45.470817 ], [ -96.834704, 45.625329 ], [ -96.780728, 45.760799 ], [ -96.556891, 45.872446 ], [ -96.539451, 46.017967 ], [ -96.538908, 46.199480 ], [ -96.601359, 46.351357 ], [ -96.685488, 46.513285 ], [ -96.733651, 46.716477 ], [ -96.745820, 46.944525 ], [ -96.779694, 46.999044 ], [ -96.820415, 47.292204 ], [ -96.824653, 47.426614 ], [ -96.844238, 47.546194 ], [ -96.893977, 47.748869 ], [ -97.015158, 47.954205 ], [ -97.131042, 48.137295 ], [ -97.148509, 48.318782 ], [ -97.161221, 48.514584 ], [ -97.127348, 48.642122 ], [ -97.120475, 48.758523 ], [ -97.214138, 48.902442 ], [ -97.228943, 49.000885 ], [ -95.158837, 48.999826 ], [ -95.156202, 49.384014 ], [ -94.817540, 49.389285 ], [ -94.640264, 48.840017 ], [ -94.329120, 48.670673 ], [ -93.630610, 48.609281 ], [ -92.609846, 48.450015 ], [ -91.639879, 48.139930 ], [ -90.830264, 48.270103 ], [ -89.599952, 48.010274 ], [ -89.599409, 48.010274 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 1, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3515" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -111.194189, 44.561156 ], [ -111.291548, 44.701406 ], [ -111.400016, 44.728924 ], [ -111.542385, 44.530486 ], [ -111.771492, 44.498214 ], [ -112.336135, 44.560640 ], [ -112.362593, 44.462222 ], [ -112.690144, 44.498731 ], [ -112.874810, 44.360083 ], [ -113.052086, 44.619912 ], [ -113.174844, 44.765433 ], [ -113.378578, 44.789773 ], [ -113.439427, 44.862792 ], [ -113.502937, 45.124223 ], [ -113.680213, 45.249073 ], [ -113.794496, 45.564997 ], [ -113.914101, 45.702585 ], [ -114.035799, 45.730103 ], [ -114.137937, 45.589336 ], [ -114.335315, 45.470274 ], [ -114.513651, 45.569234 ], [ -114.523702, 45.825369 ], [ -114.407275, 45.889912 ], [ -114.491430, 46.147080 ], [ -114.394046, 46.410087 ], [ -114.284517, 46.631805 ], [ -114.585610, 46.641339 ], [ -114.843320, 46.786318 ], [ -115.121650, 47.095369 ], [ -115.288332, 47.250398 ], [ -115.519067, 47.345121 ], [ -115.704792, 47.504930 ], [ -115.704276, 47.684842 ], [ -115.967800, 47.950484 ], [ -116.048234, 49.000369 ], [ -113.059502, 49.001402 ], [ -110.070769, 49.003004 ], [ -107.082037, 49.004580 ], [ -104.092762, 49.005640 ], [ -104.077414, 47.171565 ], [ -104.026616, 45.956601 ], [ -104.077957, 45.040610 ], [ -105.745891, 45.051178 ], [ -107.546634, 45.045881 ], [ -109.102379, 45.057018 ], [ -111.071406, 45.049602 ], [ -111.067194, 44.541597 ], [ -111.085178, 44.506147 ], [ -111.194189, 44.561156 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 2, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3516" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -96.601359, 46.351357 ], [ -96.538908, 46.199480 ], [ -96.539451, 46.017967 ], [ -98.441789, 45.963448 ], [ -100.066858, 45.965567 ], [ -102.116862, 45.961355 ], [ -104.026616, 45.956601 ], [ -104.077414, 47.171565 ], [ -104.092762, 49.005640 ], [ -100.629899, 49.003004 ], [ -97.228943, 49.000885 ], [ -97.214138, 48.902442 ], [ -97.120475, 48.758523 ], [ -97.127348, 48.642122 ], [ -97.161221, 48.514584 ], [ -97.148509, 48.318782 ], [ -97.131042, 48.137295 ], [ -97.015158, 47.954205 ], [ -96.893977, 47.748869 ], [ -96.844238, 47.546194 ], [ -96.824653, 47.426614 ], [ -96.820415, 47.292204 ], [ -96.779694, 46.999044 ], [ -96.745820, 46.944525 ], [ -96.733651, 46.716477 ], [ -96.685488, 46.513285 ], [ -96.601359, 46.351357 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 3, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3517" }, "geometry": { "type": "MultiPolygon", "coordinates": [ [ [ [ -155.936650, 19.059390 ], [ -155.908060, 19.338880 ], [ -156.073470, 19.702940 ], [ -156.023680, 19.814220 ], [ -155.850080, 19.977290 ], [ -155.919070, 20.173950 ], [ -155.861080, 20.267210 ], [ -155.785050, 20.248700 ], [ -155.402140, 20.079750 ], [ -155.224520, 19.993020 ], [ -155.062260, 19.859100 ], [ -154.807410, 19.508710 ], [ -154.831470, 19.453280 ], [ -155.222170, 19.239720 ], [ -155.542110, 19.083480 ], [ -155.688170, 18.916190 ], [ -155.936650, 19.059390 ] ] ], [ [ [ -155.995660, 20.764040 ], [ -156.079260, 20.643970 ], [ -156.414450, 20.572410 ], [ -156.586730, 20.783000 ], [ -156.701670, 20.864300 ], [ -156.710550, 20.926760 ], [ -156.612580, 21.012490 ], [ -156.257110, 20.917450 ], [ -155.995660, 20.764040 ] ] ], [ [ [ -156.758240, 21.176840 ], [ -156.789330, 21.068730 ], [ -157.325210, 21.097770 ], [ -157.250270, 21.219580 ], [ -156.758240, 21.176840 ] ] ], [ [ [ -158.126670, 21.312440 ], [ -158.253800, 21.539190 ], [ -158.292650, 21.579120 ], [ -158.025200, 21.716960 ], [ -157.941610, 21.652720 ], [ -157.652830, 21.322170 ], [ -157.707030, 21.264420 ], [ -157.778600, 21.277290 ], [ -158.126670, 21.312440 ] ] ], [ [ [ -159.800510, 22.065330 ], [ -159.748770, 22.138200 ], [ -159.596200, 22.236180 ], [ -159.365690, 22.214940 ], [ -159.345120, 21.982000 ], [ -159.463720, 21.882990 ], [ -159.800510, 22.065330 ] ] ] ] } }
                    ,
                    { "type": "Feature", "id": 4, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3518" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -111.049728, 44.488163 ], [ -111.050245, 42.001597 ], [ -114.034223, 41.993122 ], [ -117.028252, 42.000021 ], [ -117.013963, 43.797069 ], [ -116.926656, 44.081211 ], [ -117.007607, 44.211410 ], [ -117.194391, 44.279132 ], [ -117.192299, 44.438942 ], [ -117.051532, 44.665956 ], [ -116.836145, 44.863851 ], [ -116.693285, 45.186648 ], [ -116.558875, 45.444358 ], [ -116.457796, 45.574531 ], [ -116.511255, 45.726408 ], [ -116.678997, 45.807359 ], [ -116.915003, 45.999983 ], [ -116.906528, 46.177776 ], [ -116.998073, 46.330170 ], [ -117.026650, 47.722927 ], [ -117.031430, 48.999309 ], [ -116.048234, 49.000369 ], [ -115.967800, 47.950484 ], [ -115.704276, 47.684842 ], [ -115.704792, 47.504930 ], [ -115.519067, 47.345121 ], [ -115.288332, 47.250398 ], [ -115.121650, 47.095369 ], [ -114.843320, 46.786318 ], [ -114.585610, 46.641339 ], [ -114.284517, 46.631805 ], [ -114.394046, 46.410087 ], [ -114.491430, 46.147080 ], [ -114.407275, 45.889912 ], [ -114.523702, 45.825369 ], [ -114.513651, 45.569234 ], [ -114.335315, 45.470274 ], [ -114.137937, 45.589336 ], [ -114.035799, 45.730103 ], [ -113.914101, 45.702585 ], [ -113.794496, 45.564997 ], [ -113.680213, 45.249073 ], [ -113.502937, 45.124223 ], [ -113.439427, 44.862792 ], [ -113.378578, 44.789773 ], [ -113.174844, 44.765433 ], [ -113.052086, 44.619912 ], [ -112.874810, 44.360083 ], [ -112.690144, 44.498731 ], [ -112.362593, 44.462222 ], [ -112.336135, 44.560640 ], [ -111.771492, 44.498214 ], [ -111.542385, 44.530486 ], [ -111.400016, 44.728924 ], [ -111.291548, 44.701406 ], [ -111.194189, 44.561156 ], [ -111.085178, 44.506147 ], [ -111.049728, 44.488163 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 5, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3519" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -116.998073, 46.330170 ], [ -116.906528, 46.177776 ], [ -116.915003, 45.999983 ], [ -118.978236, 45.992568 ], [ -119.337000, 45.888336 ], [ -119.592050, 45.932235 ], [ -119.787309, 45.850768 ], [ -120.158268, 45.740671 ], [ -120.609636, 45.753900 ], [ -120.837193, 45.672949 ], [ -121.046741, 45.637499 ], [ -121.203889, 45.689899 ], [ -121.561077, 45.732764 ], [ -121.788091, 45.700983 ], [ -122.174915, 45.595150 ], [ -122.420456, 45.591998 ], [ -122.651708, 45.630083 ], [ -122.725786, 45.770333 ], [ -122.752244, 45.938075 ], [ -122.937969, 46.129097 ], [ -123.228494, 46.186251 ], [ -123.471373, 46.277253 ], [ -123.776704, 46.282033 ], [ -123.998676, 46.283069 ], [ -124.079635, 46.864750 ], [ -124.395670, 47.720170 ], [ -124.687210, 48.184433 ], [ -124.566101, 48.379715 ], [ -123.120000, 48.040000 ], [ -122.587138, 47.095885 ], [ -122.340021, 47.359952 ], [ -122.499831, 48.180160 ], [ -122.840000, 49.000000 ], [ -120.002696, 49.000885 ], [ -117.031430, 48.999309 ], [ -117.026650, 47.722927 ], [ -116.998073, 46.330170 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 6, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3520" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -109.045225, 36.999912 ], [ -109.043673, 31.341899 ], [ -111.023786, 31.334651 ], [ -113.305016, 32.038975 ], [ -114.814743, 32.525276 ], [ -114.721390, 32.720830 ], [ -114.589848, 32.716298 ], [ -114.479777, 32.916337 ], [ -114.655993, 33.053900 ], [ -114.691443, 33.203684 ], [ -114.743301, 33.379875 ], [ -114.548558, 33.609525 ], [ -114.469183, 34.067275 ], [ -114.165972, 34.272585 ], [ -114.354901, 34.464667 ], [ -114.484531, 34.653053 ], [ -114.567084, 34.828210 ], [ -114.622120, 34.963680 ], [ -114.642222, 35.053106 ], [ -114.580856, 35.249425 ], [ -114.630052, 35.445227 ], [ -114.650697, 35.638910 ], [ -114.650180, 35.854272 ], [ -114.739606, 35.991318 ], [ -114.671341, 36.114618 ], [ -114.460734, 36.114618 ], [ -114.269169, 36.043692 ], [ -114.132640, 36.004004 ], [ -114.066495, 36.156424 ], [ -114.023629, 36.189755 ], [ -114.030528, 36.994099 ], [ -112.417603, 37.009421 ], [ -110.496221, 37.007328 ], [ -109.045225, 36.999912 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 7, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3521" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -114.354901, 34.464667 ], [ -114.165972, 34.272585 ], [ -114.469183, 34.067275 ], [ -114.548558, 33.609525 ], [ -114.743301, 33.379875 ], [ -114.691443, 33.203684 ], [ -114.655993, 33.053900 ], [ -114.479777, 32.916337 ], [ -114.589848, 32.716298 ], [ -114.721390, 32.720830 ], [ -115.991080, 32.612583 ], [ -117.127729, 32.535327 ], [ -117.295938, 33.046225 ], [ -117.944000, 33.621236 ], [ -118.410602, 33.740909 ], [ -118.519895, 34.027782 ], [ -119.081000, 34.078000 ], [ -119.438841, 34.348477 ], [ -120.367816, 34.447226 ], [ -120.622865, 34.608611 ], [ -120.744589, 35.156821 ], [ -121.714556, 36.161721 ], [ -122.547451, 37.551817 ], [ -122.512000, 37.783611 ], [ -122.953317, 38.113798 ], [ -123.726965, 38.951473 ], [ -123.865070, 39.766927 ], [ -124.397958, 40.313018 ], [ -124.178875, 41.142244 ], [ -124.213809, 41.999478 ], [ -119.998976, 41.992605 ], [ -120.000035, 38.995398 ], [ -118.114620, 37.644421 ], [ -116.320750, 36.322047 ], [ -114.642222, 35.053106 ], [ -114.622120, 34.963680 ], [ -114.567084, 34.828210 ], [ -114.484531, 34.653053 ], [ -114.354901, 34.464667 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 8, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3522" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -102.050174, 40.000815 ], [ -102.040123, 38.459875 ], [ -102.041182, 36.991980 ], [ -103.003216, 36.995158 ], [ -104.200198, 36.996192 ], [ -105.899861, 36.997277 ], [ -107.479971, 36.999912 ], [ -109.045225, 36.999912 ], [ -109.053183, 41.001994 ], [ -108.050944, 41.002536 ], [ -107.049739, 41.002536 ], [ -105.046864, 41.003596 ], [ -104.045142, 41.004112 ], [ -102.048055, 41.004112 ], [ -102.050174, 40.033086 ], [ -102.050174, 40.000815 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 9, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3523" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -114.030528, 36.994099 ], [ -114.023629, 36.189755 ], [ -114.066495, 36.156424 ], [ -114.132640, 36.004004 ], [ -114.269169, 36.043692 ], [ -114.460734, 36.114618 ], [ -114.671341, 36.114618 ], [ -114.739606, 35.991318 ], [ -114.650180, 35.854272 ], [ -114.650697, 35.638910 ], [ -114.630052, 35.445227 ], [ -114.580856, 35.249425 ], [ -114.642222, 35.053106 ], [ -116.320750, 36.322047 ], [ -118.114620, 37.644421 ], [ -120.000035, 38.995398 ], [ -119.998976, 41.992605 ], [ -117.028252, 41.997359 ], [ -117.028252, 42.000021 ], [ -114.034223, 41.993122 ], [ -114.030528, 36.994099 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 10, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3524" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -103.001614, 33.879948 ], [ -103.001614, 31.999287 ], [ -103.929775, 31.999287 ], [ -105.730001, 31.999287 ], [ -106.630127, 31.999287 ], [ -106.619533, 31.914099 ], [ -106.507344, 31.754289 ], [ -108.240000, 31.754854 ], [ -108.241966, 31.342067 ], [ -109.043673, 31.341899 ], [ -109.045225, 36.999912 ], [ -107.479971, 36.999912 ], [ -105.899861, 36.997277 ], [ -104.200198, 36.996192 ], [ -103.003216, 36.995158 ], [ -103.001614, 36.499323 ], [ -103.001614, 33.879948 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 11, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3525" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -117.028252, 42.000021 ], [ -117.028252, 41.997359 ], [ -119.998976, 41.992605 ], [ -124.213809, 41.999478 ], [ -124.532840, 42.765990 ], [ -124.142140, 43.708380 ], [ -123.898930, 45.523410 ], [ -123.998676, 46.283069 ], [ -123.776704, 46.282033 ], [ -123.471373, 46.277253 ], [ -123.228494, 46.186251 ], [ -122.937969, 46.129097 ], [ -122.752244, 45.938075 ], [ -122.725786, 45.770333 ], [ -122.651708, 45.630083 ], [ -122.420456, 45.591998 ], [ -122.174915, 45.595150 ], [ -121.788091, 45.700983 ], [ -121.561077, 45.732764 ], [ -121.203889, 45.689899 ], [ -121.046741, 45.637499 ], [ -120.837193, 45.672949 ], [ -120.609636, 45.753900 ], [ -120.158268, 45.740671 ], [ -119.787309, 45.850768 ], [ -119.592050, 45.932235 ], [ -119.337000, 45.888336 ], [ -118.978236, 45.992568 ], [ -116.915003, 45.999983 ], [ -116.678997, 45.807359 ], [ -116.511255, 45.726408 ], [ -116.457796, 45.574531 ], [ -116.558875, 45.444358 ], [ -116.693285, 45.186648 ], [ -116.836145, 44.863851 ], [ -117.051532, 44.665956 ], [ -117.192299, 44.438942 ], [ -117.194391, 44.279132 ], [ -117.007607, 44.211410 ], [ -116.926656, 44.081211 ], [ -117.013963, 43.797069 ], [ -117.028252, 42.000021 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 12, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3526" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -109.053183, 41.001994 ], [ -109.045225, 36.999912 ], [ -110.496221, 37.007328 ], [ -112.417603, 37.009421 ], [ -114.030528, 36.994099 ], [ -114.034223, 41.993122 ], [ -111.050245, 42.001597 ], [ -111.054482, 41.027935 ], [ -109.053183, 41.001994 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 13, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3527" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -111.067194, 44.541597 ], [ -111.071406, 45.049602 ], [ -109.102379, 45.057018 ], [ -107.546634, 45.045881 ], [ -105.745891, 45.051178 ], [ -104.077957, 45.040610 ], [ -104.052558, 43.000140 ], [ -104.045142, 41.004112 ], [ -105.046864, 41.003596 ], [ -107.049739, 41.002536 ], [ -108.050944, 41.002536 ], [ -109.053183, 41.001994 ], [ -111.054482, 41.027935 ], [ -111.050245, 42.001597 ], [ -111.049728, 44.488163 ], [ -111.085178, 44.506147 ], [ -111.067194, 44.541597 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 14, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3528" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -89.662919, 36.023073 ], [ -89.673513, 35.940003 ], [ -89.775109, 35.799236 ], [ -89.950266, 35.701878 ], [ -89.988894, 35.536229 ], [ -90.147102, 35.404997 ], [ -90.134932, 35.113955 ], [ -90.249240, 35.020834 ], [ -90.268283, 34.941459 ], [ -90.446618, 34.866838 ], [ -90.450313, 34.721860 ], [ -90.584207, 34.454099 ], [ -90.699549, 34.397461 ], [ -90.876308, 34.261475 ], [ -90.982141, 34.055105 ], [ -91.200681, 33.706393 ], [ -91.223444, 33.469327 ], [ -91.108077, 33.206836 ], [ -91.156239, 33.010001 ], [ -92.001304, 33.043875 ], [ -93.094028, 33.010518 ], [ -94.059757, 33.012120 ], [ -94.002086, 33.579914 ], [ -94.233338, 33.583609 ], [ -94.427538, 33.570380 ], [ -94.479912, 33.635983 ], [ -94.451361, 34.510710 ], [ -94.430174, 35.483312 ], [ -94.628611, 36.540586 ], [ -93.412587, 36.526298 ], [ -92.307177, 36.523662 ], [ -91.251479, 36.523146 ], [ -90.112194, 36.461754 ], [ -90.029099, 36.337937 ], [ -90.141805, 36.230502 ], [ -90.253995, 36.122550 ], [ -90.315386, 36.023073 ], [ -89.662919, 36.023073 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 15, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3529" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -90.156636, 42.103735 ], [ -90.210070, 41.834914 ], [ -90.395278, 41.608417 ], [ -90.461966, 41.536457 ], [ -90.690557, 41.478786 ], [ -91.033999, 41.429565 ], [ -91.123425, 41.257585 ], [ -90.999065, 41.179812 ], [ -90.956742, 41.024757 ], [ -91.087432, 40.851719 ], [ -91.154120, 40.699842 ], [ -91.410229, 40.551143 ], [ -91.430331, 40.368596 ], [ -91.567403, 40.452208 ], [ -91.758425, 40.614110 ], [ -92.851665, 40.592432 ], [ -94.002086, 40.585017 ], [ -94.898491, 40.583440 ], [ -95.796499, 40.583957 ], [ -95.862102, 40.764928 ], [ -95.834067, 40.943806 ], [ -95.855746, 41.116302 ], [ -95.958427, 41.404708 ], [ -96.025089, 41.524287 ], [ -96.097049, 41.556559 ], [ -96.104464, 41.787811 ], [ -96.167432, 41.953434 ], [ -96.348945, 42.141821 ], [ -96.346827, 42.224374 ], [ -96.410311, 42.389480 ], [ -96.455296, 42.488957 ], [ -96.453719, 42.580502 ], [ -96.615648, 42.691632 ], [ -96.535213, 42.855679 ], [ -96.483356, 43.016005 ], [ -96.459533, 43.124500 ], [ -96.587070, 43.257308 ], [ -96.586011, 43.501247 ], [ -96.452660, 43.501789 ], [ -95.359936, 43.500187 ], [ -94.001027, 43.513417 ], [ -92.540005, 43.519773 ], [ -91.228199, 43.501247 ], [ -91.213910, 43.446754 ], [ -91.083737, 43.288004 ], [ -91.173163, 43.212324 ], [ -91.169985, 43.002233 ], [ -91.129238, 42.912833 ], [ -91.064694, 42.754083 ], [ -90.738177, 42.658275 ], [ -90.640818, 42.505364 ], [ -90.582605, 42.429168 ], [ -90.464602, 42.378370 ], [ -90.416982, 42.270418 ], [ -90.259808, 42.189983 ], [ -90.156636, 42.103735 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 16, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3530" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -94.604814, 39.139859 ], [ -94.615382, 38.068839 ], [ -94.622798, 36.999912 ], [ -95.500160, 36.998853 ], [ -97.299844, 36.997277 ], [ -99.100070, 36.995158 ], [ -100.100215, 36.994099 ], [ -101.099792, 36.993039 ], [ -102.041182, 36.991980 ], [ -102.040123, 38.459875 ], [ -102.050174, 40.000815 ], [ -102.048055, 40.000815 ], [ -100.300229, 40.000815 ], [ -99.000076, 40.000815 ], [ -96.699777, 40.001357 ], [ -95.322884, 40.001357 ], [ -95.085302, 39.868006 ], [ -94.955103, 39.870125 ], [ -94.926552, 39.725121 ], [ -95.067292, 39.539912 ], [ -94.991096, 39.444673 ], [ -94.867796, 39.234582 ], [ -94.604814, 39.139859 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 17, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3531" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -94.617501, 36.999912 ], [ -94.622798, 36.999912 ], [ -94.615382, 38.068839 ], [ -94.604814, 39.139859 ], [ -94.867796, 39.234582 ], [ -94.991096, 39.444673 ], [ -95.067292, 39.539912 ], [ -94.926552, 39.725121 ], [ -94.955103, 39.870125 ], [ -95.085302, 39.868006 ], [ -95.322884, 40.001357 ], [ -95.452540, 40.215143 ], [ -95.608112, 40.343171 ], [ -95.776371, 40.501404 ], [ -95.796499, 40.583957 ], [ -94.898491, 40.583440 ], [ -94.002086, 40.585017 ], [ -92.851665, 40.592432 ], [ -91.758425, 40.614110 ], [ -91.567403, 40.452208 ], [ -91.430331, 40.368596 ], [ -91.517638, 40.119877 ], [ -91.428212, 39.820903 ], [ -91.262589, 39.615050 ], [ -91.071567, 39.445190 ], [ -90.841375, 39.310779 ], [ -90.749287, 39.265278 ], [ -90.665701, 39.074773 ], [ -90.649810, 38.907548 ], [ -90.535528, 38.865768 ], [ -90.346599, 38.930311 ], [ -90.156094, 38.769443 ], [ -90.212731, 38.584777 ], [ -90.305335, 38.439256 ], [ -90.369879, 38.263556 ], [ -90.228079, 38.113281 ], [ -90.030158, 37.971998 ], [ -89.916909, 37.968277 ], [ -89.654987, 37.748678 ], [ -89.553908, 37.719042 ], [ -89.479287, 37.477222 ], [ -89.516339, 37.326921 ], [ -89.388285, 37.081406 ], [ -89.280333, 37.107348 ], [ -89.103057, 36.952293 ], [ -89.134270, 36.851756 ], [ -89.115227, 36.694582 ], [ -89.273977, 36.611512 ], [ -89.498356, 36.506196 ], [ -89.524272, 36.409354 ], [ -89.585121, 36.267011 ], [ -89.662919, 36.023073 ], [ -90.315386, 36.023073 ], [ -90.253995, 36.122550 ], [ -90.141805, 36.230502 ], [ -90.029099, 36.337937 ], [ -90.112194, 36.461754 ], [ -91.251479, 36.523146 ], [ -92.307177, 36.523662 ], [ -93.412587, 36.526298 ], [ -94.628611, 36.540586 ], [ -94.617501, 36.999912 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 18, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3532" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -95.452540, 40.215143 ], [ -95.322884, 40.001357 ], [ -96.699777, 40.001357 ], [ -99.000076, 40.000815 ], [ -100.300229, 40.000815 ], [ -102.048055, 40.000815 ], [ -102.050174, 40.000815 ], [ -102.050174, 40.033086 ], [ -102.048055, 41.004112 ], [ -104.045142, 41.004112 ], [ -104.052558, 43.000140 ], [ -102.099912, 43.000140 ], [ -100.599745, 43.000140 ], [ -98.594183, 43.000140 ], [ -98.335956, 42.873662 ], [ -97.968201, 42.794287 ], [ -97.881927, 42.839789 ], [ -97.644344, 42.836094 ], [ -97.287157, 42.846145 ], [ -97.028387, 42.717548 ], [ -96.754270, 42.633961 ], [ -96.708768, 42.550866 ], [ -96.623037, 42.502729 ], [ -96.455296, 42.488957 ], [ -96.410311, 42.389480 ], [ -96.346827, 42.224374 ], [ -96.348945, 42.141821 ], [ -96.167432, 41.953434 ], [ -96.104464, 41.787811 ], [ -96.097049, 41.556559 ], [ -96.025089, 41.524287 ], [ -95.958427, 41.404708 ], [ -95.855746, 41.116302 ], [ -95.834067, 40.943806 ], [ -95.862102, 40.764928 ], [ -95.796499, 40.583957 ], [ -95.776371, 40.501404 ], [ -95.608112, 40.343171 ], [ -95.452540, 40.215143 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 19, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3533" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -94.628611, 36.540586 ], [ -94.430174, 35.483312 ], [ -94.451361, 34.510710 ], [ -94.479912, 33.635983 ], [ -94.910144, 33.831785 ], [ -95.190592, 33.938161 ], [ -95.418124, 33.870414 ], [ -95.769498, 33.881007 ], [ -95.977469, 33.879431 ], [ -96.148906, 33.797937 ], [ -96.316131, 33.756131 ], [ -96.462711, 33.805327 ], [ -96.796618, 33.751351 ], [ -96.947953, 33.918059 ], [ -97.104067, 33.773598 ], [ -97.376583, 33.838142 ], [ -97.657031, 33.993713 ], [ -97.957090, 33.893694 ], [ -98.088323, 34.134480 ], [ -98.553979, 34.110657 ], [ -98.851893, 34.164633 ], [ -99.187403, 34.235559 ], [ -99.336076, 34.442963 ], [ -99.599083, 34.376300 ], [ -99.761528, 34.457794 ], [ -100.000195, 34.565229 ], [ -100.000195, 35.519848 ], [ -100.000195, 36.499323 ], [ -101.000832, 36.499323 ], [ -102.001495, 36.499323 ], [ -103.001614, 36.499323 ], [ -103.003216, 36.995158 ], [ -102.041182, 36.991980 ], [ -101.099792, 36.993039 ], [ -100.100215, 36.994099 ], [ -99.100070, 36.995158 ], [ -97.299844, 36.997277 ], [ -95.500160, 36.998853 ], [ -94.622798, 36.999912 ], [ -94.617501, 36.999912 ], [ -94.628611, 36.540586 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 20, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3534" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -96.754270, 42.633961 ], [ -97.028387, 42.717548 ], [ -97.287157, 42.846145 ], [ -97.644344, 42.836094 ], [ -97.881927, 42.839789 ], [ -97.968201, 42.794287 ], [ -98.335956, 42.873662 ], [ -98.594183, 43.000140 ], [ -100.599745, 43.000140 ], [ -102.099912, 43.000140 ], [ -104.052558, 43.000140 ], [ -104.077957, 45.040610 ], [ -104.026616, 45.956601 ], [ -102.116862, 45.961355 ], [ -100.066858, 45.965567 ], [ -98.441789, 45.963448 ], [ -96.539451, 46.017967 ], [ -96.556891, 45.872446 ], [ -96.780728, 45.760799 ], [ -96.834704, 45.625329 ], [ -96.735769, 45.470817 ], [ -96.560612, 45.393018 ], [ -96.439431, 44.435763 ], [ -96.452660, 43.501789 ], [ -96.586011, 43.501247 ], [ -96.587070, 43.257308 ], [ -96.459533, 43.124500 ], [ -96.483356, 43.016005 ], [ -96.535213, 42.855679 ], [ -96.615648, 42.691632 ], [ -96.453719, 42.580502 ], [ -96.455296, 42.488957 ], [ -96.623037, 42.502729 ], [ -96.708768, 42.550866 ], [ -96.754270, 42.633961 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 21, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3535" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -89.217670, 29.291080 ], [ -89.408387, 29.159771 ], [ -89.779280, 29.307140 ], [ -90.154630, 29.117430 ], [ -90.880225, 29.148535 ], [ -91.626785, 29.677000 ], [ -92.499060, 29.552300 ], [ -93.226370, 29.783750 ], [ -93.848420, 29.713630 ], [ -93.918474, 29.821772 ], [ -93.816878, 29.968352 ], [ -93.667120, 30.100644 ], [ -93.664484, 30.300140 ], [ -93.737503, 30.366803 ], [ -93.650195, 30.605471 ], [ -93.586168, 30.713940 ], [ -93.489844, 31.079602 ], [ -93.578236, 31.216131 ], [ -93.694120, 31.443662 ], [ -93.779309, 31.675431 ], [ -93.834861, 31.829944 ], [ -93.998908, 31.942650 ], [ -94.059757, 33.012120 ], [ -93.094028, 33.010518 ], [ -92.001304, 33.043875 ], [ -91.156239, 33.010001 ], [ -91.084797, 32.952847 ], [ -91.175799, 32.808385 ], [ -91.030820, 32.602532 ], [ -91.072084, 32.478716 ], [ -90.942971, 32.306736 ], [ -91.081618, 32.204598 ], [ -91.128179, 32.015695 ], [ -91.321320, 31.859580 ], [ -91.411288, 31.650032 ], [ -91.502290, 31.408729 ], [ -91.624531, 31.297082 ], [ -91.583784, 31.047330 ], [ -90.701667, 31.015575 ], [ -89.759219, 31.013456 ], [ -89.787796, 30.847291 ], [ -89.853941, 30.683244 ], [ -89.790457, 30.556791 ], [ -89.658682, 30.440881 ], [ -89.623232, 30.275258 ], [ -89.604706, 30.176298 ], [ -89.413735, 29.894190 ], [ -89.430000, 29.488640 ], [ -89.217670, 29.291080 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 22, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3536" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -93.578236, 31.216131 ], [ -93.489844, 31.079602 ], [ -93.586168, 30.713940 ], [ -93.650195, 30.605471 ], [ -93.737503, 30.366803 ], [ -93.664484, 30.300140 ], [ -93.667120, 30.100644 ], [ -93.816878, 29.968352 ], [ -93.918474, 29.821772 ], [ -93.848420, 29.713630 ], [ -94.690003, 29.479932 ], [ -95.600180, 28.738556 ], [ -96.593943, 28.307291 ], [ -97.140034, 27.829981 ], [ -97.370227, 27.380190 ], [ -97.379761, 26.690154 ], [ -97.330023, 26.210209 ], [ -97.140034, 25.869946 ], [ -97.530036, 25.839793 ], [ -98.240174, 26.059934 ], [ -99.020178, 26.370019 ], [ -99.300109, 26.839912 ], [ -99.520225, 27.539999 ], [ -100.110266, 28.109912 ], [ -100.455801, 28.696233 ], [ -100.957450, 29.380972 ], [ -101.662290, 29.779423 ], [ -102.479863, 29.759863 ], [ -103.110109, 28.969808 ], [ -103.939826, 29.269842 ], [ -104.456823, 29.571994 ], [ -104.705541, 30.121805 ], [ -105.037330, 30.644099 ], [ -105.631583, 31.083839 ], [ -106.142766, 31.399737 ], [ -106.507344, 31.754289 ], [ -106.619533, 31.914099 ], [ -106.630127, 31.999287 ], [ -105.730001, 31.999287 ], [ -103.929775, 31.999287 ], [ -103.001614, 31.999287 ], [ -103.001614, 33.879948 ], [ -103.001614, 36.499323 ], [ -102.001495, 36.499323 ], [ -101.000832, 36.499323 ], [ -100.000195, 36.499323 ], [ -100.000195, 35.519848 ], [ -100.000195, 34.565229 ], [ -99.761528, 34.457794 ], [ -99.599083, 34.376300 ], [ -99.336076, 34.442963 ], [ -99.187403, 34.235559 ], [ -98.851893, 34.164633 ], [ -98.553979, 34.110657 ], [ -98.088323, 34.134480 ], [ -97.957090, 33.893694 ], [ -97.657031, 33.993713 ], [ -97.376583, 33.838142 ], [ -97.104067, 33.773598 ], [ -96.947953, 33.918059 ], [ -96.796618, 33.751351 ], [ -96.462711, 33.805327 ], [ -96.316131, 33.756131 ], [ -96.148906, 33.797937 ], [ -95.977469, 33.879431 ], [ -95.769498, 33.881007 ], [ -95.418124, 33.870414 ], [ -95.190592, 33.938161 ], [ -94.910144, 33.831785 ], [ -94.479912, 33.635983 ], [ -94.427538, 33.570380 ], [ -94.233338, 33.583609 ], [ -94.002086, 33.579914 ], [ -94.059757, 33.012120 ], [ -93.998908, 31.942650 ], [ -93.834861, 31.829944 ], [ -93.779309, 31.675431 ], [ -93.694120, 31.443662 ], [ -93.578236, 31.216131 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 23, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3537" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -72.876166, 41.220559 ], [ -73.647887, 40.953239 ], [ -73.656687, 40.985070 ], [ -73.692680, 41.107310 ], [ -73.475174, 41.204669 ], [ -73.553489, 41.289857 ], [ -73.497937, 42.054513 ], [ -72.732222, 42.035987 ], [ -71.800909, 42.013250 ], [ -71.792951, 41.466617 ], [ -71.853826, 41.320036 ], [ -72.295142, 41.269755 ], [ -72.876166, 41.220559 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 24, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3513" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -69.884799, 41.922764 ], [ -69.965207, 41.636994 ], [ -70.639895, 41.475066 ], [ -71.120382, 41.494651 ], [ -71.147900, 41.647587 ], [ -71.305074, 41.762412 ], [ -71.379152, 42.024360 ], [ -71.800909, 42.013250 ], [ -72.732222, 42.035987 ], [ -73.497937, 42.054513 ], [ -73.282033, 42.743489 ], [ -72.457070, 42.727082 ], [ -71.248979, 42.718091 ], [ -71.145781, 42.816508 ], [ -70.933598, 42.884256 ], [ -70.815052, 42.865187 ], [ -70.825103, 42.334961 ], [ -70.494916, 41.804735 ], [ -70.080058, 41.779879 ], [ -70.184832, 42.144999 ], [ -69.884799, 41.922764 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 25, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3538" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -71.503511, 45.007796 ], [ -71.505087, 45.008312 ], [ -71.405093, 45.254912 ], [ -71.084820, 45.305240 ], [ -70.944165, 43.466339 ], [ -70.981760, 43.367896 ], [ -70.797611, 43.219739 ], [ -70.751025, 43.080032 ], [ -70.645734, 43.090083 ], [ -70.815052, 42.865187 ], [ -70.933598, 42.884256 ], [ -71.145781, 42.816508 ], [ -71.248979, 42.718091 ], [ -72.457070, 42.727082 ], [ -72.537505, 42.830797 ], [ -72.458647, 42.960453 ], [ -72.434307, 43.222918 ], [ -72.403611, 43.285369 ], [ -72.369763, 43.521892 ], [ -72.260209, 43.721388 ], [ -72.178199, 43.808696 ], [ -72.059136, 44.045761 ], [ -72.036373, 44.206630 ], [ -72.003041, 44.304014 ], [ -71.809901, 44.352151 ], [ -71.585522, 44.468035 ], [ -71.546377, 44.592395 ], [ -71.620455, 44.735797 ], [ -71.503511, 45.007796 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 26, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3539" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -71.147900, 41.647587 ], [ -71.120382, 41.494651 ], [ -71.853826, 41.320036 ], [ -71.792951, 41.466617 ], [ -71.800909, 42.013250 ], [ -71.379152, 42.024360 ], [ -71.305074, 41.762412 ], [ -71.147900, 41.647587 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 27, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3540" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -71.503511, 45.007796 ], [ -71.620455, 44.735797 ], [ -71.546377, 44.592395 ], [ -71.585522, 44.468035 ], [ -71.809901, 44.352151 ], [ -72.003041, 44.304014 ], [ -72.036373, 44.206630 ], [ -72.059136, 44.045761 ], [ -72.178199, 43.808696 ], [ -72.260209, 43.721388 ], [ -72.369763, 43.521892 ], [ -72.403611, 43.285369 ], [ -72.434307, 43.222918 ], [ -72.458647, 42.960453 ], [ -72.537505, 42.830797 ], [ -72.457070, 42.727082 ], [ -73.282033, 42.743489 ], [ -73.239710, 43.567935 ], [ -73.383112, 43.575351 ], [ -73.401638, 43.613436 ], [ -73.338128, 43.758440 ], [ -73.429673, 44.019846 ], [ -73.329136, 44.226732 ], [ -73.384172, 44.379152 ], [ -73.407969, 44.676007 ], [ -73.368281, 44.804604 ], [ -73.347662, 45.007253 ], [ -71.505087, 45.008312 ], [ -71.503511, 45.007796 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 28, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3541" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -85.054415, 31.108696 ], [ -85.005193, 30.990693 ], [ -87.045663, 30.984879 ], [ -87.617153, 30.927725 ], [ -87.633043, 30.851528 ], [ -87.404969, 30.608649 ], [ -87.457886, 30.411271 ], [ -87.530388, 30.274199 ], [ -88.417802, 30.384812 ], [ -88.449531, 31.911980 ], [ -88.273340, 33.510048 ], [ -88.095522, 34.805989 ], [ -88.166964, 34.999673 ], [ -86.909677, 34.999130 ], [ -85.625388, 34.985901 ], [ -85.365559, 33.744478 ], [ -85.129553, 32.750714 ], [ -84.986151, 32.437969 ], [ -84.898843, 32.259117 ], [ -85.063924, 32.083417 ], [ -85.119502, 31.765400 ], [ -85.065526, 31.577013 ], [ -85.090382, 31.400280 ], [ -85.117900, 31.236233 ], [ -85.054415, 31.108696 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 29, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3542" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -80.056539, 26.880000 ], [ -80.088015, 26.205765 ], [ -80.131560, 25.816775 ], [ -80.381030, 25.206160 ], [ -80.679799, 25.079916 ], [ -81.171913, 25.201098 ], [ -81.330121, 25.639753 ], [ -81.710072, 25.869946 ], [ -82.239781, 26.729842 ], [ -82.705150, 27.495040 ], [ -82.855260, 27.886240 ], [ -82.649885, 28.550170 ], [ -82.930000, 29.100000 ], [ -83.709590, 29.936560 ], [ -84.099797, 30.090050 ], [ -85.108820, 29.636150 ], [ -85.287840, 29.686120 ], [ -85.773100, 30.152610 ], [ -86.400070, 30.400160 ], [ -87.530388, 30.274199 ], [ -87.457886, 30.411271 ], [ -87.404969, 30.608649 ], [ -87.633043, 30.851528 ], [ -87.617153, 30.927725 ], [ -87.045663, 30.984879 ], [ -85.005193, 30.990693 ], [ -84.853316, 30.721355 ], [ -83.847900, 30.675311 ], [ -82.226009, 30.525553 ], [ -82.151931, 30.350938 ], [ -82.022817, 30.440364 ], [ -82.020156, 30.788018 ], [ -81.899517, 30.821892 ], [ -81.701597, 30.748330 ], [ -81.490473, 30.729830 ], [ -81.313710, 30.035520 ], [ -80.979832, 29.179899 ], [ -80.535585, 28.472130 ], [ -80.530040, 28.040072 ], [ -80.056539, 26.880000 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 30, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3543" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -80.865007, 32.033161 ], [ -81.336290, 31.440490 ], [ -81.490473, 30.729830 ], [ -81.701597, 30.748330 ], [ -81.899517, 30.821892 ], [ -82.020156, 30.788018 ], [ -82.022817, 30.440364 ], [ -82.151931, 30.350938 ], [ -82.226009, 30.525553 ], [ -83.847900, 30.675311 ], [ -84.853316, 30.721355 ], [ -85.005193, 30.990693 ], [ -85.054415, 31.108696 ], [ -85.117900, 31.236233 ], [ -85.090382, 31.400280 ], [ -85.065526, 31.577013 ], [ -85.119502, 31.765400 ], [ -85.063924, 32.083417 ], [ -84.898843, 32.259117 ], [ -84.986151, 32.437969 ], [ -85.129553, 32.750714 ], [ -85.365559, 33.744478 ], [ -85.625388, 34.985901 ], [ -84.854376, 34.976909 ], [ -84.320972, 34.986960 ], [ -83.076371, 34.979028 ], [ -83.185925, 34.895958 ], [ -83.346251, 34.706512 ], [ -83.076371, 34.540347 ], [ -82.902815, 34.479498 ], [ -82.716548, 34.163057 ], [ -82.597485, 33.985781 ], [ -82.249289, 33.748716 ], [ -82.180508, 33.624356 ], [ -81.943442, 33.461369 ], [ -81.827015, 33.223244 ], [ -81.507397, 33.021628 ], [ -81.435954, 32.793037 ], [ -81.376707, 32.682450 ], [ -81.411098, 32.608888 ], [ -81.224830, 32.499360 ], [ -81.126412, 32.312033 ], [ -81.127988, 32.121528 ], [ -81.036443, 32.084476 ], [ -80.865007, 32.033161 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 31, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3544" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -88.166964, 34.999673 ], [ -88.095522, 34.805989 ], [ -88.273340, 33.510048 ], [ -88.449531, 31.911980 ], [ -88.417802, 30.384812 ], [ -89.180490, 30.315980 ], [ -89.593831, 30.159994 ], [ -89.623232, 30.275258 ], [ -89.658682, 30.440881 ], [ -89.790457, 30.556791 ], [ -89.853941, 30.683244 ], [ -89.787796, 30.847291 ], [ -89.759219, 31.013456 ], [ -90.701667, 31.015575 ], [ -91.583784, 31.047330 ], [ -91.624531, 31.297082 ], [ -91.502290, 31.408729 ], [ -91.411288, 31.650032 ], [ -91.321320, 31.859580 ], [ -91.128179, 32.015695 ], [ -91.081618, 32.204598 ], [ -90.942971, 32.306736 ], [ -91.072084, 32.478716 ], [ -91.030820, 32.602532 ], [ -91.175799, 32.808385 ], [ -91.084797, 32.952847 ], [ -91.156239, 33.010001 ], [ -91.108077, 33.206836 ], [ -91.223444, 33.469327 ], [ -91.200681, 33.706393 ], [ -90.982141, 34.055105 ], [ -90.876308, 34.261475 ], [ -90.699549, 34.397461 ], [ -90.584207, 34.454099 ], [ -90.450313, 34.721860 ], [ -90.446618, 34.866838 ], [ -90.268283, 34.941459 ], [ -90.249240, 35.020834 ], [ -89.263926, 35.021351 ], [ -88.166964, 34.999673 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 32, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3545" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -78.554114, 33.861422 ], [ -79.060670, 33.493950 ], [ -79.203570, 33.158390 ], [ -80.301325, 32.509355 ], [ -80.865007, 32.033161 ], [ -81.036443, 32.084476 ], [ -81.127988, 32.121528 ], [ -81.126412, 32.312033 ], [ -81.224830, 32.499360 ], [ -81.411098, 32.608888 ], [ -81.376707, 32.682450 ], [ -81.435954, 32.793037 ], [ -81.507397, 33.021628 ], [ -81.827015, 33.223244 ], [ -81.943442, 33.461369 ], [ -82.180508, 33.624356 ], [ -82.249289, 33.748716 ], [ -82.597485, 33.985781 ], [ -82.716548, 34.163057 ], [ -82.902815, 34.479498 ], [ -83.076371, 34.540347 ], [ -83.346251, 34.706512 ], [ -83.185925, 34.895958 ], [ -83.076371, 34.979028 ], [ -82.976377, 35.008664 ], [ -82.436616, 35.180101 ], [ -81.514270, 35.171652 ], [ -81.045978, 35.125608 ], [ -81.038020, 35.037242 ], [ -80.937483, 35.103387 ], [ -80.780852, 34.934044 ], [ -80.783513, 34.817643 ], [ -79.672780, 34.807566 ], [ -78.554114, 33.861422 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 33, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3546" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -90.640818, 42.505364 ], [ -89.620054, 42.505364 ], [ -88.576552, 42.503246 ], [ -87.806599, 42.494254 ], [ -87.806599, 42.496890 ], [ -87.489641, 42.495313 ], [ -87.039307, 42.492652 ], [ -87.084808, 42.309562 ], [ -87.130335, 42.127015 ], [ -87.175836, 41.943900 ], [ -87.221337, 41.760836 ], [ -87.371638, 41.760836 ], [ -87.520854, 41.760836 ], [ -87.521371, 41.708436 ], [ -87.526151, 41.708436 ], [ -87.527210, 40.550083 ], [ -87.528269, 39.392273 ], [ -87.642035, 39.113944 ], [ -87.559999, 39.040382 ], [ -87.507082, 38.869462 ], [ -87.515040, 38.735052 ], [ -87.598653, 38.674203 ], [ -87.671129, 38.508580 ], [ -87.878558, 38.290557 ], [ -88.018808, 38.021736 ], [ -88.051080, 37.819604 ], [ -88.044207, 37.744983 ], [ -88.157430, 37.605819 ], [ -88.071725, 37.511612 ], [ -88.247399, 37.438594 ], [ -88.473871, 37.354981 ], [ -88.435242, 37.136442 ], [ -88.566475, 37.053888 ], [ -88.807778, 37.146493 ], [ -89.073963, 37.200469 ], [ -89.141685, 37.103627 ], [ -89.103057, 36.952293 ], [ -89.280333, 37.107348 ], [ -89.388285, 37.081406 ], [ -89.516339, 37.326921 ], [ -89.479287, 37.477222 ], [ -89.553908, 37.719042 ], [ -89.654987, 37.748678 ], [ -89.916909, 37.968277 ], [ -90.030158, 37.971998 ], [ -90.228079, 38.113281 ], [ -90.369879, 38.263556 ], [ -90.305335, 38.439256 ], [ -90.212731, 38.584777 ], [ -90.156094, 38.769443 ], [ -90.346599, 38.930311 ], [ -90.535528, 38.865768 ], [ -90.649810, 38.907548 ], [ -90.665701, 39.074773 ], [ -90.749287, 39.265278 ], [ -90.841375, 39.310779 ], [ -91.071567, 39.445190 ], [ -91.262589, 39.615050 ], [ -91.428212, 39.820903 ], [ -91.517638, 40.119877 ], [ -91.430331, 40.368596 ], [ -91.410229, 40.551143 ], [ -91.154120, 40.699842 ], [ -91.087432, 40.851719 ], [ -90.956742, 41.024757 ], [ -90.999065, 41.179812 ], [ -91.123425, 41.257585 ], [ -91.033999, 41.429565 ], [ -90.690557, 41.478786 ], [ -90.461966, 41.536457 ], [ -90.395278, 41.608417 ], [ -90.210070, 41.834914 ], [ -90.156636, 42.103735 ], [ -90.259808, 42.189983 ], [ -90.416982, 42.270418 ], [ -90.464602, 42.378370 ], [ -90.582605, 42.429168 ], [ -90.640818, 42.505364 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 34, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3547" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -84.806756, 41.677741 ], [ -84.809934, 40.772860 ], [ -84.817866, 39.799742 ], [ -84.823680, 39.106528 ], [ -84.881377, 39.059425 ], [ -84.799883, 38.855174 ], [ -84.843265, 38.781096 ], [ -85.011550, 38.779494 ], [ -85.167638, 38.691127 ], [ -85.404187, 38.727120 ], [ -85.425891, 38.535038 ], [ -85.566632, 38.461994 ], [ -85.698407, 38.290015 ], [ -85.839690, 38.258802 ], [ -86.059832, 37.960887 ], [ -86.262507, 38.046619 ], [ -86.325475, 38.169376 ], [ -86.499573, 37.969879 ], [ -86.610160, 37.858749 ], [ -86.825522, 37.976235 ], [ -87.055714, 37.880970 ], [ -87.131394, 37.783611 ], [ -87.439360, 37.936005 ], [ -87.653662, 37.826477 ], [ -87.911373, 37.904250 ], [ -87.921424, 37.793662 ], [ -88.051080, 37.819604 ], [ -88.018808, 38.021736 ], [ -87.878558, 38.290557 ], [ -87.671129, 38.508580 ], [ -87.598653, 38.674203 ], [ -87.515040, 38.735052 ], [ -87.507082, 38.869462 ], [ -87.559999, 39.040382 ], [ -87.642035, 39.113944 ], [ -87.528269, 39.392273 ], [ -87.527210, 40.550083 ], [ -87.526151, 41.708436 ], [ -87.521371, 41.760836 ], [ -87.221337, 41.760836 ], [ -86.918126, 41.760836 ], [ -86.824462, 41.760836 ], [ -86.824462, 41.756056 ], [ -85.748146, 41.751302 ], [ -84.807299, 41.756056 ], [ -84.806756, 41.677741 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 35, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3548" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -81.972536, 37.535952 ], [ -82.372589, 37.238037 ], [ -82.684792, 37.121094 ], [ -82.709132, 37.040143 ], [ -82.815508, 36.934826 ], [ -83.089083, 36.815763 ], [ -83.178509, 36.718379 ], [ -83.384362, 36.656497 ], [ -83.673285, 36.599859 ], [ -84.350091, 36.567045 ], [ -85.231149, 36.609910 ], [ -85.519012, 36.597741 ], [ -86.092621, 36.625775 ], [ -86.677882, 36.633733 ], [ -87.216583, 36.639004 ], [ -87.842049, 36.610970 ], [ -87.874346, 36.656497 ], [ -88.072784, 36.654378 ], [ -88.069606, 36.497204 ], [ -89.498356, 36.506196 ], [ -89.273977, 36.611512 ], [ -89.115227, 36.694582 ], [ -89.134270, 36.851756 ], [ -89.103057, 36.952293 ], [ -89.141685, 37.103627 ], [ -89.073963, 37.200469 ], [ -88.807778, 37.146493 ], [ -88.566475, 37.053888 ], [ -88.435242, 37.136442 ], [ -88.473871, 37.354981 ], [ -88.247399, 37.438594 ], [ -88.071725, 37.511612 ], [ -88.157430, 37.605819 ], [ -88.044207, 37.744983 ], [ -88.051080, 37.819604 ], [ -87.921424, 37.793662 ], [ -87.911373, 37.904250 ], [ -87.653662, 37.826477 ], [ -87.439360, 37.936005 ], [ -87.131394, 37.783611 ], [ -87.055714, 37.880970 ], [ -86.825522, 37.976235 ], [ -86.610160, 37.858749 ], [ -86.499573, 37.969879 ], [ -86.325475, 38.169376 ], [ -86.262507, 38.046619 ], [ -86.059832, 37.960887 ], [ -85.839690, 38.258802 ], [ -85.698407, 38.290015 ], [ -85.566632, 38.461994 ], [ -85.425891, 38.535038 ], [ -85.404187, 38.727120 ], [ -85.167638, 38.691127 ], [ -85.011550, 38.779494 ], [ -84.843265, 38.781096 ], [ -84.799883, 38.855174 ], [ -84.881377, 39.059425 ], [ -84.823680, 39.106528 ], [ -84.481324, 39.083248 ], [ -84.304590, 38.986923 ], [ -84.038948, 38.760994 ], [ -83.826738, 38.690068 ], [ -83.672742, 38.609117 ], [ -83.434617, 38.637151 ], [ -83.258943, 38.579480 ], [ -83.044641, 38.634515 ], [ -82.855195, 38.651440 ], [ -82.775278, 38.511216 ], [ -82.589010, 38.415433 ], [ -82.569967, 38.320194 ], [ -82.580561, 38.113281 ], [ -82.461499, 37.957167 ], [ -82.413336, 37.805315 ], [ -82.267299, 37.675659 ], [ -82.167279, 37.554478 ], [ -81.972536, 37.535952 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 36, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3549" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -75.868077, 36.551180 ], [ -75.727310, 35.550518 ], [ -76.363370, 34.808625 ], [ -77.397635, 34.512010 ], [ -78.054960, 33.925470 ], [ -78.554114, 33.861422 ], [ -79.672780, 34.807566 ], [ -80.783513, 34.817643 ], [ -80.780852, 34.934044 ], [ -80.937483, 35.103387 ], [ -81.038020, 35.037242 ], [ -81.045978, 35.125608 ], [ -81.514270, 35.171652 ], [ -82.436616, 35.180101 ], [ -82.976377, 35.008664 ], [ -83.076371, 34.979028 ], [ -84.320972, 34.986960 ], [ -84.298751, 35.198627 ], [ -84.087084, 35.261595 ], [ -84.017760, 35.369004 ], [ -83.875960, 35.490211 ], [ -83.673285, 35.516670 ], [ -83.438338, 35.562687 ], [ -83.209722, 35.648961 ], [ -83.110787, 35.737328 ], [ -82.920282, 35.817220 ], [ -82.925579, 35.889722 ], [ -82.674225, 36.025192 ], [ -82.593248, 35.937342 ], [ -82.223890, 36.125728 ], [ -82.051394, 36.106143 ], [ -81.897399, 36.273910 ], [ -81.693664, 36.317293 ], [ -81.704775, 36.460152 ], [ -81.679376, 36.585571 ], [ -79.992399, 36.542188 ], [ -77.999549, 36.537434 ], [ -76.941215, 36.545883 ], [ -75.868077, 36.551180 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 37, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3550" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -82.210661, 38.579480 ], [ -82.340834, 38.440832 ], [ -82.589010, 38.415433 ], [ -82.775278, 38.511216 ], [ -82.855195, 38.651440 ], [ -83.044641, 38.634515 ], [ -83.258943, 38.579480 ], [ -83.434617, 38.637151 ], [ -83.672742, 38.609117 ], [ -83.826738, 38.690068 ], [ -84.038948, 38.760994 ], [ -84.304590, 38.986923 ], [ -84.481324, 39.083248 ], [ -84.823680, 39.106528 ], [ -84.817866, 39.799742 ], [ -84.809934, 40.772860 ], [ -84.806756, 41.677741 ], [ -84.295056, 41.685156 ], [ -83.839967, 41.685156 ], [ -83.462678, 41.694148 ], [ -83.142000, 41.975681 ], [ -83.121898, 41.949739 ], [ -83.029810, 41.832796 ], [ -82.866306, 41.752878 ], [ -82.690089, 41.675105 ], [ -82.439278, 41.675105 ], [ -82.213323, 41.778820 ], [ -81.974138, 41.888890 ], [ -81.760870, 41.986791 ], [ -81.507397, 42.103735 ], [ -81.277747, 42.209026 ], [ -81.027969, 42.247137 ], [ -80.682434, 42.299511 ], [ -80.520506, 42.324394 ], [ -80.516268, 42.324910 ], [ -80.516268, 42.324394 ], [ -80.518930, 40.641111 ], [ -80.657552, 40.590830 ], [ -80.615229, 40.463835 ], [ -80.661789, 40.233643 ], [ -80.764987, 39.973297 ], [ -80.861829, 39.757393 ], [ -80.878753, 39.654221 ], [ -81.150752, 39.426147 ], [ -81.266094, 39.377442 ], [ -81.401047, 39.349407 ], [ -81.522228, 39.371628 ], [ -81.745005, 39.199649 ], [ -81.785752, 39.019221 ], [ -81.816964, 38.921836 ], [ -81.905874, 38.882149 ], [ -81.918560, 38.993796 ], [ -82.054030, 39.018161 ], [ -82.194797, 38.801198 ], [ -82.210661, 38.579480 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 38, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3551" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -81.679376, 36.585571 ], [ -81.704775, 36.460152 ], [ -81.693664, 36.317293 ], [ -81.897399, 36.273910 ], [ -82.051394, 36.106143 ], [ -82.223890, 36.125728 ], [ -82.593248, 35.937342 ], [ -82.674225, 36.025192 ], [ -82.925579, 35.889722 ], [ -82.920282, 35.817220 ], [ -83.110787, 35.737328 ], [ -83.209722, 35.648961 ], [ -83.438338, 35.562687 ], [ -83.673285, 35.516670 ], [ -83.875960, 35.490211 ], [ -84.017760, 35.369004 ], [ -84.087084, 35.261595 ], [ -84.298751, 35.198627 ], [ -84.320972, 34.986960 ], [ -84.854376, 34.976909 ], [ -85.625388, 34.985901 ], [ -86.909677, 34.999130 ], [ -88.166964, 34.999673 ], [ -89.263926, 35.021351 ], [ -90.249240, 35.020834 ], [ -90.134932, 35.113955 ], [ -90.147102, 35.404997 ], [ -89.988894, 35.536229 ], [ -89.950266, 35.701878 ], [ -89.775109, 35.799236 ], [ -89.673513, 35.940003 ], [ -89.662919, 36.023073 ], [ -89.585121, 36.267011 ], [ -89.524272, 36.409354 ], [ -89.498356, 36.506196 ], [ -88.069606, 36.497204 ], [ -88.072784, 36.654378 ], [ -87.874346, 36.656497 ], [ -87.842049, 36.610970 ], [ -87.216583, 36.639004 ], [ -86.677882, 36.633733 ], [ -86.092621, 36.625775 ], [ -85.519012, 36.597741 ], [ -85.231149, 36.609910 ], [ -84.350091, 36.567045 ], [ -83.673285, 36.599859 ], [ -82.186322, 36.565985 ], [ -81.679376, 36.585571 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 39, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3552" }, "geometry": { "type": "MultiPolygon", "coordinates": [ [ [ [ -75.377539, 38.015383 ], [ -75.940035, 37.216875 ], [ -76.031063, 37.256563 ], [ -75.722050, 37.937052 ], [ -75.609850, 38.000032 ], [ -75.377539, 38.015383 ] ] ], [ [ [ -78.345625, 39.405504 ], [ -77.834984, 39.134562 ], [ -77.722795, 39.322432 ], [ -77.576216, 39.288561 ], [ -77.443407, 39.213423 ], [ -77.516968, 39.105986 ], [ -77.305819, 39.045679 ], [ -77.119551, 38.934008 ], [ -77.036480, 38.848276 ], [ -77.041235, 38.789545 ], [ -77.059217, 38.708593 ], [ -77.229622, 38.614416 ], [ -77.343388, 38.391611 ], [ -77.210579, 38.337118 ], [ -77.048107, 38.380503 ], [ -76.989895, 38.239733 ], [ -76.301619, 37.917948 ], [ -76.258739, 36.966402 ], [ -75.971800, 36.897261 ], [ -75.868076, 36.551180 ], [ -76.941216, 36.545885 ], [ -77.999548, 36.537436 ], [ -79.992397, 36.542189 ], [ -81.679375, 36.585573 ], [ -82.186321, 36.565985 ], [ -83.673285, 36.599859 ], [ -83.384361, 36.656496 ], [ -83.178509, 36.718382 ], [ -83.089083, 36.815763 ], [ -82.815508, 36.934828 ], [ -82.709131, 37.040144 ], [ -82.684792, 37.121096 ], [ -82.372589, 37.238039 ], [ -81.972536, 37.535952 ], [ -81.928095, 37.366091 ], [ -81.815388, 37.275606 ], [ -81.663511, 37.195173 ], [ -81.348129, 37.315836 ], [ -81.228008, 37.245455 ], [ -80.854929, 37.329065 ], [ -80.833252, 37.418493 ], [ -80.720003, 37.383043 ], [ -80.596186, 37.456060 ], [ -80.456995, 37.441774 ], [ -80.298244, 37.519030 ], [ -80.276567, 37.610575 ], [ -80.292974, 37.728060 ], [ -80.157505, 37.901098 ], [ -79.964363, 38.031790 ], [ -79.915142, 38.179426 ], [ -79.743679, 38.357222 ], [ -79.647923, 38.574728 ], [ -79.515089, 38.497446 ], [ -79.366390, 38.426027 ], [ -79.222988, 38.464632 ], [ -79.174826, 38.555658 ], [ -79.076408, 38.680017 ], [ -78.965277, 38.821817 ], [ -78.892801, 38.780039 ], [ -78.744619, 38.909151 ], [ -78.548843, 39.039839 ], [ -78.424484, 39.139318 ], [ -78.345625, 39.405504 ] ] ] ] } }
                    ,
                    { "type": "Feature", "id": 40, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3553" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -86.481563, 45.323177 ], [ -86.264600, 45.226878 ], [ -86.401672, 45.133214 ], [ -86.536082, 45.041670 ], [ -86.714934, 44.846410 ], [ -86.822344, 44.591335 ], [ -86.918668, 44.361685 ], [ -87.032434, 44.091288 ], [ -87.083232, 43.890190 ], [ -87.141962, 43.657878 ], [ -87.145140, 43.571088 ], [ -87.155191, 43.326632 ], [ -87.113928, 43.030294 ], [ -87.079537, 42.782118 ], [ -87.039307, 42.492652 ], [ -87.489641, 42.495313 ], [ -87.806599, 42.496890 ], [ -87.806599, 42.494254 ], [ -88.576552, 42.503246 ], [ -89.620054, 42.505364 ], [ -90.640818, 42.505364 ], [ -90.738177, 42.658275 ], [ -91.064694, 42.754083 ], [ -91.129238, 42.912833 ], [ -91.169985, 43.002233 ], [ -91.173163, 43.212324 ], [ -91.083737, 43.288004 ], [ -91.213910, 43.446754 ], [ -91.228199, 43.501247 ], [ -91.254657, 43.613979 ], [ -91.257292, 43.854739 ], [ -91.289590, 43.937293 ], [ -91.627709, 44.085449 ], [ -91.879606, 44.257428 ], [ -91.949989, 44.364838 ], [ -92.062153, 44.432585 ], [ -92.385492, 44.574928 ], [ -92.505072, 44.583920 ], [ -92.796656, 44.776027 ], [ -92.766477, 44.996143 ], [ -92.765417, 45.267082 ], [ -92.689221, 45.518436 ], [ -92.899828, 45.705763 ], [ -92.756968, 45.889912 ], [ -92.543700, 45.985695 ], [ -92.296583, 46.096282 ], [ -92.264828, 46.095223 ], [ -92.274879, 46.656145 ], [ -92.011898, 46.711723 ], [ -91.964794, 46.729706 ], [ -91.840978, 46.778385 ], [ -91.716618, 46.827607 ], [ -91.592259, 46.876260 ], [ -91.468442, 46.924966 ], [ -91.264708, 47.021265 ], [ -91.060974, 47.117047 ], [ -90.857782, 47.212829 ], [ -90.654048, 47.309128 ], [ -90.479950, 47.303857 ], [ -90.305852, 47.298018 ], [ -90.131754, 47.292747 ], [ -89.957656, 47.286907 ], [ -90.014810, 47.199600 ], [ -90.071448, 47.111750 ], [ -90.128059, 47.023900 ], [ -90.185213, 46.936076 ], [ -90.241825, 46.848226 ], [ -90.298979, 46.760919 ], [ -90.355590, 46.673069 ], [ -90.395278, 46.611677 ], [ -90.410626, 46.584185 ], [ -90.396880, 46.576253 ], [ -90.334971, 46.596872 ], [ -90.333912, 46.593694 ], [ -90.176738, 46.560879 ], [ -90.096304, 46.380994 ], [ -89.221060, 46.202116 ], [ -88.644274, 46.022204 ], [ -88.361707, 46.020628 ], [ -88.166964, 46.007916 ], [ -88.111929, 45.843352 ], [ -87.875406, 45.779842 ], [ -87.787013, 45.639618 ], [ -87.847888, 45.559183 ], [ -87.893389, 45.396713 ], [ -87.673248, 45.387747 ], [ -87.747326, 45.226878 ], [ -87.613975, 45.108875 ], [ -87.612915, 45.110451 ], [ -87.459462, 45.068128 ], [ -87.419775, 45.184012 ], [ -87.244617, 45.289846 ], [ -87.115504, 45.452291 ], [ -86.942465, 45.452291 ], [ -86.774207, 45.452291 ], [ -86.481563, 45.323177 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 41, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3554" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -77.722796, 39.322432 ], [ -77.834985, 39.134562 ], [ -78.345626, 39.405502 ], [ -78.424484, 39.139317 ], [ -78.548843, 39.039840 ], [ -78.744620, 38.909150 ], [ -78.892802, 38.780036 ], [ -78.965278, 38.821817 ], [ -79.076408, 38.680017 ], [ -79.174826, 38.555657 ], [ -79.222988, 38.464629 ], [ -79.366390, 38.426027 ], [ -79.515089, 38.497444 ], [ -79.647924, 38.574726 ], [ -79.743680, 38.357220 ], [ -79.915142, 38.179427 ], [ -79.964364, 38.031787 ], [ -80.157505, 37.901098 ], [ -80.292975, 37.728059 ], [ -80.276567, 37.610573 ], [ -80.298246, 37.519028 ], [ -80.456996, 37.441772 ], [ -80.596186, 37.456060 ], [ -80.720003, 37.383041 ], [ -80.833252, 37.418492 ], [ -80.854930, 37.329065 ], [ -81.228008, 37.245453 ], [ -81.348130, 37.315836 ], [ -81.663511, 37.195172 ], [ -81.815388, 37.275606 ], [ -81.928094, 37.366092 ], [ -81.972536, 37.535952 ], [ -82.167279, 37.554478 ], [ -82.267299, 37.675659 ], [ -82.413336, 37.805315 ], [ -82.461499, 37.957167 ], [ -82.580561, 38.113281 ], [ -82.569967, 38.320194 ], [ -82.589010, 38.415433 ], [ -82.340834, 38.440832 ], [ -82.210661, 38.579480 ], [ -82.194797, 38.801198 ], [ -82.054030, 39.018161 ], [ -81.918560, 38.993796 ], [ -81.905874, 38.882149 ], [ -81.816964, 38.921836 ], [ -81.785752, 39.019221 ], [ -81.745005, 39.199649 ], [ -81.522228, 39.371628 ], [ -81.401047, 39.349407 ], [ -81.266094, 39.377442 ], [ -81.150752, 39.426147 ], [ -80.878753, 39.654221 ], [ -80.861829, 39.757393 ], [ -80.764987, 39.973297 ], [ -80.661789, 40.233643 ], [ -80.615229, 40.463835 ], [ -80.657552, 40.590830 ], [ -80.518930, 40.641111 ], [ -80.518930, 39.720883 ], [ -79.477521, 39.720883 ], [ -79.485995, 39.213421 ], [ -79.332516, 39.302847 ], [ -79.161080, 39.418214 ], [ -78.963159, 39.457902 ], [ -78.829291, 39.562650 ], [ -78.534012, 39.522446 ], [ -78.425001, 39.596524 ], [ -78.232403, 39.672204 ], [ -77.923352, 39.592829 ], [ -77.801654, 39.449944 ], [ -77.722796, 39.322432 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 42, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3555" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -75.319867, 38.959948 ], [ -75.071835, 38.782032 ], [ -75.048385, 38.448765 ], [ -75.714624, 38.449307 ], [ -75.787642, 39.723545 ], [ -75.710903, 39.802377 ], [ -75.620960, 39.847361 ], [ -75.405573, 39.795504 ], [ -75.554272, 39.691247 ], [ -75.527813, 39.498649 ], [ -75.319867, 38.959948 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 43, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3556" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -76.911579, 38.878454 ], [ -77.041235, 38.789545 ], [ -77.036481, 38.848275 ], [ -77.119551, 38.934006 ], [ -77.038600, 38.982169 ], [ -76.911579, 38.878454 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 44, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3557" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -75.048385, 38.448765 ], [ -75.056730, 38.404120 ], [ -75.377538, 38.015380 ], [ -75.609850, 38.000032 ], [ -75.722050, 37.937050 ], [ -76.232870, 38.319215 ], [ -76.350000, 39.150000 ], [ -76.542725, 38.717615 ], [ -76.329330, 38.083260 ], [ -76.989895, 38.239733 ], [ -77.048108, 38.380500 ], [ -77.210579, 38.337118 ], [ -77.343387, 38.391610 ], [ -77.229622, 38.614413 ], [ -77.059219, 38.708594 ], [ -77.041235, 38.789545 ], [ -76.911579, 38.878454 ], [ -77.038600, 38.982169 ], [ -77.119551, 38.934006 ], [ -77.305818, 39.045679 ], [ -77.516968, 39.105985 ], [ -77.443407, 39.213421 ], [ -77.576215, 39.288558 ], [ -77.722796, 39.322432 ], [ -77.801654, 39.449944 ], [ -77.923352, 39.592829 ], [ -78.232403, 39.672204 ], [ -78.425001, 39.596524 ], [ -78.534012, 39.522446 ], [ -78.829291, 39.562650 ], [ -78.963159, 39.457902 ], [ -79.161080, 39.418214 ], [ -79.332516, 39.302847 ], [ -79.485995, 39.213421 ], [ -79.477521, 39.720883 ], [ -78.549903, 39.719824 ], [ -78.232919, 39.721400 ], [ -77.523299, 39.725663 ], [ -76.668183, 39.720883 ], [ -75.787642, 39.723545 ], [ -75.714624, 38.449307 ], [ -75.048385, 38.448765 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 45, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3558" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -73.912796, 40.960187 ], [ -73.952325, 40.750750 ], [ -74.256754, 40.473370 ], [ -73.962440, 40.427630 ], [ -74.178438, 39.709256 ], [ -74.906042, 38.939303 ], [ -74.980663, 39.196497 ], [ -75.200262, 39.248354 ], [ -75.527813, 39.498649 ], [ -75.554272, 39.691247 ], [ -75.405573, 39.795504 ], [ -75.200805, 39.887049 ], [ -75.128820, 39.949500 ], [ -74.891754, 40.081791 ], [ -74.763183, 40.190777 ], [ -75.078022, 40.449547 ], [ -75.095488, 40.555380 ], [ -75.203957, 40.586619 ], [ -75.199203, 40.747461 ], [ -75.082259, 40.869702 ], [ -75.135718, 40.999875 ], [ -74.975909, 41.087725 ], [ -74.801268, 41.311561 ], [ -74.679028, 41.355486 ], [ -73.912796, 40.960187 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 46, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3559" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -74.679028, 41.355486 ], [ -74.840439, 41.426386 ], [ -75.010816, 41.495710 ], [ -75.075386, 41.641231 ], [ -75.048928, 41.751302 ], [ -75.167990, 41.841787 ], [ -75.385471, 41.998961 ], [ -76.743837, 42.000537 ], [ -78.200648, 42.000021 ], [ -79.760087, 42.000021 ], [ -79.760087, 42.500068 ], [ -79.773257, 42.546894 ], [ -78.939362, 42.863611 ], [ -78.919777, 42.965207 ], [ -79.010000, 43.270000 ], [ -79.171674, 43.466339 ], [ -79.002330, 43.527188 ], [ -78.845699, 43.583283 ], [ -78.720280, 43.625089 ], [ -76.820034, 43.628784 ], [ -76.696760, 43.784899 ], [ -76.586147, 43.924063 ], [ -76.500000, 44.018459 ], [ -76.375000, 44.096310 ], [ -75.318265, 44.816231 ], [ -74.866898, 45.000380 ], [ -73.347662, 45.007253 ], [ -73.368281, 44.804604 ], [ -73.407969, 44.676007 ], [ -73.384172, 44.379152 ], [ -73.329136, 44.226732 ], [ -73.429673, 44.019846 ], [ -73.338128, 43.758440 ], [ -73.401638, 43.613436 ], [ -73.383112, 43.575351 ], [ -73.239710, 43.567935 ], [ -73.282033, 42.743489 ], [ -73.497937, 42.054513 ], [ -73.553489, 41.289857 ], [ -73.475174, 41.204669 ], [ -73.692680, 41.107310 ], [ -73.656687, 40.985070 ], [ -73.647887, 40.954769 ], [ -73.710000, 40.931102 ], [ -72.241166, 41.119480 ], [ -71.944828, 40.930034 ], [ -73.345001, 40.630001 ], [ -73.982120, 40.627882 ], [ -73.952325, 40.750750 ], [ -73.912796, 40.960187 ], [ -74.679028, 41.355486 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 47, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3560" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -74.891754, 40.081791 ], [ -75.128820, 39.949500 ], [ -75.200805, 39.887049 ], [ -75.405573, 39.795504 ], [ -75.620960, 39.847361 ], [ -75.710903, 39.802377 ], [ -75.787642, 39.723545 ], [ -76.668183, 39.720883 ], [ -77.523299, 39.725663 ], [ -78.232919, 39.721400 ], [ -78.549903, 39.719824 ], [ -79.477521, 39.720883 ], [ -80.518930, 39.720883 ], [ -80.518930, 40.641111 ], [ -80.516268, 42.324394 ], [ -80.516268, 42.324910 ], [ -80.247448, 42.366200 ], [ -79.772638, 42.546275 ], [ -79.760087, 42.500068 ], [ -79.760087, 42.000021 ], [ -78.200648, 42.000021 ], [ -76.743837, 42.000537 ], [ -75.385471, 41.998961 ], [ -75.167990, 41.841787 ], [ -75.048928, 41.751302 ], [ -75.075386, 41.641231 ], [ -75.010816, 41.495710 ], [ -74.840439, 41.426386 ], [ -74.679028, 41.355486 ], [ -74.801268, 41.311561 ], [ -74.975909, 41.087725 ], [ -75.135718, 40.999875 ], [ -75.082259, 40.869702 ], [ -75.199203, 40.747461 ], [ -75.203957, 40.586619 ], [ -75.095488, 40.555380 ], [ -75.078022, 40.449547 ], [ -74.763183, 40.190777 ], [ -74.891754, 40.081791 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 48, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3561" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -67.137344, 45.137452 ], [ -66.964660, 44.809700 ], [ -68.032520, 44.325200 ], [ -69.060000, 43.980000 ], [ -70.116170, 43.684050 ], [ -70.645734, 43.090083 ], [ -70.751025, 43.080032 ], [ -70.797611, 43.219739 ], [ -70.981760, 43.367896 ], [ -70.944165, 43.466339 ], [ -71.084820, 45.305240 ], [ -70.660023, 45.460223 ], [ -70.304954, 45.914795 ], [ -70.000140, 46.693171 ], [ -69.237086, 47.447776 ], [ -68.904781, 47.184795 ], [ -68.234305, 47.354629 ], [ -67.790353, 47.066249 ], [ -67.791412, 45.702585 ], [ -67.137344, 45.137452 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 49, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3562" }, "geometry": { "type": "Polygon", "coordinates": [ [ [ -90.395278, 46.611677 ], [ -90.355590, 46.673069 ], [ -90.298979, 46.760919 ], [ -90.241825, 46.848226 ], [ -90.185213, 46.936076 ], [ -90.128059, 47.023900 ], [ -90.071448, 47.111750 ], [ -90.014810, 47.199600 ], [ -89.957656, 47.286907 ], [ -89.842831, 47.464726 ], [ -89.728006, 47.641976 ], [ -89.613698, 47.819252 ], [ -89.522695, 47.960535 ], [ -89.488885, 48.015263 ], [ -89.272917, 48.019808 ], [ -89.185610, 48.047326 ], [ -88.378114, 48.302918 ], [ -87.494396, 47.961595 ], [ -87.208108, 47.848372 ], [ -86.921846, 47.735123 ], [ -86.672068, 47.636162 ], [ -86.495335, 47.566838 ], [ -86.428647, 47.539838 ], [ -86.234447, 47.459946 ], [ -86.040247, 47.380054 ], [ -85.846563, 47.300136 ], [ -85.652363, 47.220219 ], [ -85.458163, 47.139810 ], [ -85.263963, 47.059893 ], [ -85.070280, 46.980001 ], [ -84.876080, 46.900083 ], [ -84.826858, 46.766732 ], [ -84.779238, 46.637102 ], [ -84.543749, 46.538684 ], [ -84.543232, 46.538684 ], [ -84.604900, 46.439600 ], [ -84.336700, 46.408770 ], [ -84.142120, 46.512226 ], [ -84.128374, 46.483649 ], [ -84.115144, 46.370917 ], [ -84.091851, 46.275419 ], [ -83.890765, 46.116927 ], [ -83.763228, 46.108969 ], [ -83.669048, 46.122741 ], [ -83.616131, 46.116927 ], [ -83.469551, 45.994686 ], [ -83.592851, 45.816894 ], [ -83.397049, 45.729044 ], [ -83.179052, 45.632719 ], [ -82.919223, 45.517920 ], [ -82.760473, 45.447510 ], [ -82.550925, 45.347517 ], [ -82.515475, 45.204114 ], [ -82.485296, 45.083993 ], [ -82.446667, 44.915708 ], [ -82.407523, 44.743729 ], [ -82.368352, 44.572809 ], [ -82.326546, 44.391296 ], [ -82.281044, 44.192342 ], [ -82.240840, 44.015608 ], [ -82.196373, 43.822442 ], [ -82.137642, 43.571088 ], [ -82.190559, 43.474272 ], [ -82.417031, 43.017607 ], [ -82.429743, 42.980038 ], [ -82.900180, 42.430227 ], [ -83.119779, 42.079912 ], [ -83.128771, 42.068802 ], [ -83.142000, 41.975681 ], [ -83.462678, 41.694148 ], [ -83.839967, 41.685156 ], [ -84.295056, 41.685156 ], [ -84.806756, 41.677741 ], [ -84.807299, 41.756056 ], [ -85.748146, 41.751302 ], [ -86.824462, 41.756056 ], [ -86.824462, 41.760836 ], [ -86.918126, 41.760836 ], [ -87.221337, 41.760836 ], [ -87.175836, 41.943900 ], [ -87.130335, 42.127015 ], [ -87.084808, 42.309562 ], [ -87.039307, 42.492652 ], [ -87.079537, 42.782118 ], [ -87.113928, 43.030294 ], [ -87.155191, 43.326632 ], [ -87.145140, 43.571088 ], [ -87.141962, 43.657878 ], [ -87.083232, 43.890190 ], [ -87.032434, 44.091288 ], [ -86.918668, 44.361685 ], [ -86.822344, 44.591335 ], [ -86.714934, 44.846410 ], [ -86.536082, 45.041670 ], [ -86.401672, 45.133214 ], [ -86.264600, 45.226878 ], [ -86.481563, 45.323177 ], [ -86.774207, 45.452291 ], [ -86.942465, 45.452291 ], [ -87.115504, 45.452291 ], [ -87.244617, 45.289846 ], [ -87.419775, 45.184012 ], [ -87.459462, 45.068128 ], [ -87.612915, 45.110451 ], [ -87.613975, 45.108875 ], [ -87.747326, 45.226878 ], [ -87.673248, 45.387747 ], [ -87.893389, 45.396713 ], [ -87.847888, 45.559183 ], [ -87.787013, 45.639618 ], [ -87.875406, 45.779842 ], [ -88.111929, 45.843352 ], [ -88.166964, 46.007916 ], [ -88.361707, 46.020628 ], [ -88.644274, 46.022204 ], [ -89.221060, 46.202116 ], [ -90.096304, 46.380994 ], [ -90.176738, 46.560879 ], [ -90.333912, 46.593694 ], [ -90.334971, 46.596872 ], [ -90.396880, 46.576253 ], [ -90.410626, 46.584185 ], [ -90.395278, 46.611677 ] ] ] } }
                    ,
                    { "type": "Feature", "id": 50, "properties": { "scalerank": 2, "featurecla": "Admin-1 scale rank", "adm1_code": "USA-3563" }, "geometry": { "type": "MultiPolygon", "coordinates": [ [ [ [ -153.228729, 57.968968 ], [ -152.564791, 57.901427 ], [ -152.141147, 57.591059 ], [ -153.006314, 57.115842 ], [ -154.005090, 56.734677 ], [ -154.516403, 56.992749 ], [ -154.670993, 57.461196 ], [ -153.762780, 57.816575 ], [ -153.228729, 57.968968 ] ] ], [ [ [ -166.467792, 60.384170 ], [ -165.674430, 60.293607 ], [ -165.579164, 59.909987 ], [ -166.192770, 59.754441 ], [ -166.848337, 59.941406 ], [ -167.455277, 60.213069 ], [ -166.467792, 60.384170 ] ] ], [ [ [ -171.731657, 63.782515 ], [ -171.114434, 63.592191 ], [ -170.491112, 63.694975 ], [ -169.682505, 63.431116 ], [ -168.689439, 63.297506 ], [ -168.771941, 63.188598 ], [ -169.529440, 62.976931 ], [ -170.290556, 63.194438 ], [ -170.671386, 63.375822 ], [ -171.553063, 63.317789 ], [ -171.791111, 63.405846 ], [ -171.731657, 63.782515 ] ] ], [ [ [ -140.985988, 69.711998 ], [ -140.985988, 69.711998 ], [ -140.992499, 66.000029 ], [ -140.997770, 60.306397 ], [ -140.012998, 60.276838 ], [ -139.039000, 60.000007 ], [ -138.340878, 59.562101 ], [ -137.452508, 58.904984 ], [ -136.479725, 59.463864 ], [ -135.475833, 59.787772 ], [ -134.944987, 59.270543 ], [ -134.271100, 58.861110 ], [ -133.355549, 58.410285 ], [ -132.730421, 57.692887 ], [ -131.707822, 56.552104 ], [ -130.007770, 55.915812 ], [ -129.979994, 55.284998 ], [ -130.536110, 54.802753 ], [ -131.085818, 55.178906 ], [ -131.967211, 55.497776 ], [ -132.250011, 56.369996 ], [ -133.539181, 57.178887 ], [ -134.078063, 58.123068 ], [ -135.038211, 58.187715 ], [ -136.628062, 58.212209 ], [ -137.800006, 58.499995 ], [ -139.867787, 59.537762 ], [ -140.825274, 59.727517 ], [ -142.574444, 60.084447 ], [ -143.958881, 59.999180 ], [ -145.925557, 60.458610 ], [ -147.114374, 60.884656 ], [ -148.224306, 60.672989 ], [ -148.018066, 59.978329 ], [ -148.570823, 59.914173 ], [ -149.727858, 59.705658 ], [ -150.608243, 59.368211 ], [ -151.716393, 59.155821 ], [ -151.859433, 59.744984 ], [ -151.409719, 60.725803 ], [ -150.346941, 61.033588 ], [ -150.621111, 61.284425 ], [ -151.895839, 60.727198 ], [ -152.578330, 60.061657 ], [ -154.019172, 59.350279 ], [ -153.287511, 58.864728 ], [ -154.232492, 58.146374 ], [ -155.307491, 57.727795 ], [ -156.308335, 57.422774 ], [ -156.556097, 56.979985 ], [ -158.117217, 56.463608 ], [ -158.433321, 55.994154 ], [ -159.603327, 55.566686 ], [ -160.289720, 55.643581 ], [ -161.223048, 55.364735 ], [ -162.237766, 55.024187 ], [ -163.069447, 54.689737 ], [ -164.785569, 54.404173 ], [ -164.942226, 54.572225 ], [ -163.848340, 55.039431 ], [ -162.870001, 55.348043 ], [ -161.804175, 55.894986 ], [ -160.563605, 56.008055 ], [ -160.070560, 56.418055 ], [ -158.684443, 57.016675 ], [ -158.461097, 57.216921 ], [ -157.722770, 57.570001 ], [ -157.550274, 58.328326 ], [ -157.041675, 58.918885 ], [ -158.194731, 58.615802 ], [ -158.517218, 58.787781 ], [ -159.058606, 58.424186 ], [ -159.711667, 58.931390 ], [ -159.981289, 58.572549 ], [ -160.355271, 59.071123 ], [ -161.355003, 58.670838 ], [ -161.968894, 58.671665 ], [ -162.054987, 59.266925 ], [ -161.874171, 59.633621 ], [ -162.518059, 59.989724 ], [ -163.818341, 59.798056 ], [ -164.662218, 60.267484 ], [ -165.346388, 60.507496 ], [ -165.350832, 61.073895 ], [ -166.121379, 61.500019 ], [ -165.734452, 62.074997 ], [ -164.919179, 62.633076 ], [ -164.562508, 63.146378 ], [ -163.753332, 63.219449 ], [ -163.067224, 63.059459 ], [ -162.260555, 63.541936 ], [ -161.534450, 63.455817 ], [ -160.772507, 63.766108 ], [ -160.958335, 64.222799 ], [ -161.518068, 64.402788 ], [ -160.777778, 64.788604 ], [ -161.391926, 64.777235 ], [ -162.453050, 64.559445 ], [ -162.757786, 64.338605 ], [ -163.546394, 64.559160 ], [ -164.960830, 64.446945 ], [ -166.425288, 64.686672 ], [ -166.845004, 65.088896 ], [ -168.110560, 65.669997 ], [ -166.705271, 66.088318 ], [ -164.474710, 66.576660 ], [ -163.652512, 66.576660 ], [ -163.788602, 66.077207 ], [ -161.677774, 66.116120 ], [ -162.489715, 66.735565 ], [ -163.719717, 67.116395 ], [ -164.430991, 67.616338 ], [ -165.390287, 68.042772 ], [ -166.764441, 68.358877 ], [ -166.204707, 68.883031 ], [ -164.430811, 68.915535 ], [ -163.168614, 69.371115 ], [ -162.930566, 69.858062 ], [ -161.908897, 70.333330 ], [ -160.934797, 70.447690 ], [ -159.039176, 70.891642 ], [ -158.119723, 70.824721 ], [ -156.580825, 71.357764 ], [ -155.067790, 71.147776 ], [ -154.344165, 70.696409 ], [ -153.900006, 70.889989 ], [ -152.210006, 70.829992 ], [ -152.270002, 70.600006 ], [ -150.739992, 70.430017 ], [ -149.720003, 70.530010 ], [ -147.613362, 70.214035 ], [ -145.689990, 70.120010 ], [ -144.920011, 69.989992 ], [ -143.589446, 70.152514 ], [ -142.072510, 69.851938 ], [ -140.985988, 69.711998 ] ] ] ] } }

                ]

                //

                var group = L.layerGroup();

                // set up type of fuel state

                var varState = "All";

                // set polygon style

                var myPolygonStyle = {
                    "color": "#999999",
                    "weight": 3,
                    "opacity": 0.9,
                    "fillOpacity": 0.22
                };

                //markers

                function allMarkers (data) {
                    var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                    promise.then(function(data) {
                    var markers = L.geoJSON(data, {
                        pointToLayer: function (feature, latlng) {
                            return L.circleMarker(latlng);
                        },
                            onEachFeature: onEachFeature,
                            style: style
                    });
                    group.addLayer(markers);
                    });
                }

                // call first set of markers

                allMarkers();

                group.addTo(map);

                // refilter options for states

                var refilterLayers = {
                    All: function (varState) {
                        function allMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data) {
                            var markers = L.geoJSON(data, {
                                filter: function (feature, layer) {
                                    if (varState == "All") {
                                        return(feature.properties["Fuel type"] == "Biomass" || "Coal" || "Gas" || "Geothermal" || "Hydro" || "Nuclear" || "Oil" || "Other" || "Solar" || "Storage"  || "Waste" || "Wind" );
                                    }
                                    else if (varState == "LowCarbon") {
                                        return(feature.properties["LowCarbon"] == "Yes");
                                    }
                                    else if (varState == "HighCarbon") {
                                        return(feature.properties["LowCarbon"] == "No");
                                    }
                                    else if (varState == "Biomass") {
                                        return(feature.properties["Fuel type"] == "Biomass");
                                    }
                                    else if (varState == "Coal") {
                                        return(feature.properties["Fuel type"] == "Coal");
                                    }
                                    else if (varState == "Gas") {
                                        return(feature.properties["Fuel type"] == "Gas");
                                    }
                                    else if (varState == "Geothermal") {
                                        return(feature.properties["Fuel type"] == "Geothermal");
                                    }
                                    else if (varState == "Hydro") {
                                        return(feature.properties["Fuel type"] == "Hydro");
                                    }
                                    else if (varState == "Nuclear") {
                                        return(feature.properties["Fuel type"] == "Nuclear");
                                    }
                                    else if (varState == "Oil") {
                                        return(feature.properties["Fuel type"] == "Oil");
                                    }
                                    else if (varState == "Other") {
                                        return(feature.properties["Fuel type"] == "Other");
                                    }
                                    else if (varState == "Solar") {
                                        return(feature.properties["Fuel type"] == "Solar");
                                    }
                                    else if (varState == "Storage") {
                                        return(feature.properties["Fuel type"] == "Storage");
                                    }
                                    else if (varState == "Waste") {
                                        return(feature.properties["Fuel type"] == "Waste");
                                    }
                                    else if (varState == "Wind") {
                                        return(feature.properties["Fuel type"] == "Wind");
                                    }
                                },
                                pointToLayer: function (feature, latlng) {
                                    return L.circleMarker(latlng);
                                },
                                    onEachFeature: onEachFeature,
                                    style: style
                            });
                            group.addLayer(markers);
                            });

                            ////console.log("refilterLayersAll");

                        }

                        group.clearLayers();
                        allMarkers();
                        map.fitBounds(bounds, {padding: [20, 20]});
                        group.addTo(map);
                    },
                    Alabama: function (varState) {
                        function alabamaMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "AL");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "AL" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "AL" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "AL" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "AL" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "AL" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "AL" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "AL" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "AL" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "AL" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "AL" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "AL" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "AL" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "AL" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "AL" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function alabamaPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3541");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        alabamaPolygon();
                        alabamaMarkers();
                        map.setView([32.31, -89], 6);
                        group.addTo(map);

                        ////console.log("refilterLayersAlabama");
                    },
                    Alaska: function (varState) {
                        function alaskaMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "AK");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "AK" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "AK" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "AK" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "AK" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "AK" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "AK" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "AK" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "AK" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "AK" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "AK" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "AK" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "AK" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "AK" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "AK" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function alaskaPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3563");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        alaskaPolygon();
                        alaskaMarkers();
                        map.setView([64.2, -159], 4);
                        group.addTo(map);

                        ////console.log("refilterLayersAlaska");
                    },
                    Arizona: function (varState) {
                        function arizonaMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "AZ");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "AZ" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "AZ" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "AZ" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "AZ" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "AZ" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "AZ" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "AZ" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "AZ" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "AZ" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "AZ" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "AZ" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "AZ" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "AZ" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "AZ" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function arizonaPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3520");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        arizonaPolygon();
                        arizonaMarkers();
                        map.setView([34.048, -114.5], 6);
                        group.addTo(map);

                        ////console.log("refilterLayersArizona");
                    },
                    Arkansas: function (varState) {
                        function arkansasMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "AR");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "AR" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "AR" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "AR" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "AR" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "AR" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "AR" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "AR" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "AR" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "AR" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "AR" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "AR" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "AR" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "AR" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "AR" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function arkansasPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3528");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        arkansasPolygon();
                        arkansasMarkers();
                        map.setView([34.5, -94], 6);
                        group.addTo(map);

                        ////console.log("refilterLayersArkansas");
                    },
                    California: function (varState) {
                        function californiaMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "CA");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "CA" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "CA" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "CA" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "CA" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "CA" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "CA" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "CA" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "CA" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "CA" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "CA" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "CA" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "CA" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "CA" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "CA" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function californiaPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3521");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        californiaPolygon();
                        californiaMarkers();
                        map.setView([36.778, -123], 5);
                        group.addTo(map);

                        //console.log("refilterLayersCalifornia");
                    },
                    Colorado: function (varState) {
                        function coloradoMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "CO");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "CO" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "CO" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "CO" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "CO" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "CO" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "CO" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "CO" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "CO" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "CO" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "CO" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "CO" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "CO" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "CO" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "CO" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function coloradoPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3522");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        coloradoPolygon();
                        coloradoMarkers();
                        map.setView([39.55, -108], 6);
                        group.addTo(map);

                        //console.log("refilterLayersColorado");
                    },
                    Connecticut: function (varState) {
                        function connecticutMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "CT");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "CT" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "CT" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "CT" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "CT" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "CT" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "CT" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "CT" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "CT" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "CT" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "CT" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "CT" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "CT" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "CT" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "CT" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function connecticutPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3537");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        connecticutPolygon();
                        connecticutMarkers();
                        map.setView([41.5, -73.8], 7);
                        group.addTo(map);

                        //console.log("refilterLayersConnecticut");
                    },
                    Delaware: function (varState) {
                        function delawareMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "DE");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "DE" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "DE" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "DE" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "DE" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "DE" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "DE" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "DE" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "DE" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "DE" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "DE" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "DE" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "DE" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "DE" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "DE" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function delawarePolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3555");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        delawarePolygon();
                        delawareMarkers();
                        map.setView([38.910, -77], 7);
                        group.addTo(map);

                        //console.log("refilterLayersDelaware");
                    },
                    Florida: function (varState) {
                        function floridaMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "FL");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "FL" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "FL" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "FL" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "FL" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "FL" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "FL" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "FL" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "FL" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "FL" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "FL" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "FL" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "FL" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "FL" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "FL" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function floridaPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3542");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        floridaPolygon();
                        floridaMarkers();
                        map.setView([27.664, -86], 6);
                        group.addTo(map);

                        //console.log("refilterLayersFlorida");
                    },
                    Georgia: function (varState) {
                        function georgiaMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "GA");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "GA" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "GA" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "GA" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "GA" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "GA" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "GA" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "GA" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "GA" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "GA" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "GA" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "GA" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "GA" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "GA" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "GA" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function georgiaPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3543");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        georgiaPolygon();
                        georgiaMarkers();
                        map.setView([32.165, -86], 6);
                        group.addTo(map);

                        //console.log("refilterLayersGeorgia");
                    },
                    Hawaii: function (varState) {
                        function hawaiiMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "HI");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "HI" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "HI" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "HI" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "HI" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "HI" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "HI" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "HI" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "HI" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "HI" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "HI" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "HI" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "HI" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "HI" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "HI" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function hawaiiPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3517");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        hawaiiPolygon();
                        hawaiiMarkers();
                        map.setView([20, -158.5], 7);
                        group.addTo(map);

                        //console.log("refilterLayersHawaii");
                    },
                    Idaho: function (varState) {
                        function idahoMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "ID");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "ID" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "ID" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "ID" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "ID" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "ID" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "ID" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "ID" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "ID" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "ID" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "ID" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "ID" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "ID" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "ID" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "ID" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function idahoPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3518");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        idahoPolygon();
                        idahoMarkers();
                        map.setView([45.068, -120], 5);
                        group.addTo(map);

                        //console.log("refilterLayersIdaho");
                    },
                    Illinois: function (varState) {
                        function illinoisMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "IL");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "IL" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "IL" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "IL" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "IL" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "IL" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "IL" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "IL" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "IL" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "IL" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "IL" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "IL" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "IL" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "IL" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "IL" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function illinoisPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3546");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        illinoisPolygon();
                        illinoisMarkers();
                        map.setView([40, -91], 6);
                        group.addTo(map);

                        //console.log("refilterLayersIllinois");
                    },
                    Indiana: function (varState) {
                        function indianaMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "IN");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "IN" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "IN" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "IN" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "IN" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "IN" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "IN" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "IN" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "IN" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "IN" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "IN" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "IN" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "IN" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "IN" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "IN" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function indianaPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3547");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        indianaPolygon();
                        indianaMarkers();
                        map.setView([40, -88.5], 6);
                        group.addTo(map);

                        //console.log("refilterLayersIndiana");
                    },
                    Iowa: function (varState) {
                        function iowaMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "IA");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "IA" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "IA" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "IA" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "IA" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "IA" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "IA" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "IA" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "IA" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "IA" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "IA" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "IA" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "IA" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "IA" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "IA" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function iowaPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3529");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        iowaPolygon();
                        iowaMarkers();
                        map.setView([41.878, -95], 6);
                        group.addTo(map);

                        //console.log("refilterLayersIowa");
                    },
                    Kansas: function (varState) {
                        function kansasMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "KS");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "KS" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "KS" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "KS" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "KS" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "KS" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "KS" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "KS" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "KS" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "KS" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "KS" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "KS" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "KS" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "KS" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "KS" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function kansasPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3530");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        kansasPolygon();
                        kansasMarkers();
                        map.setView([38.9, -100.8], 6);
                        group.addTo(map);

                        //console.log("refilterLayersKansas");
                    },
                    Kentucky: function (varState) {
                        function kentuckyMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "KY");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "KY" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "KY" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "KY" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "KY" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "KY" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "KY" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "KY" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "KY" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "KY" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "KY" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "KY" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "KY" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "KY" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "KY" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function kentuckyPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3548");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        kentuckyPolygon();
                        kentuckyMarkers();
                        map.setView([37.839, -88.5], 6);
                        group.addTo(map);

                        //console.log("refilterLayersKentucky");
                    },
                    Louisiana: function (varState) {
                        function louisianaMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "LA");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "LA" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "LA" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "LA" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "LA" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "LA" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "LA" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "LA" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "LA" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "LA" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "LA" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "LA" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "LA" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "LA" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "LA" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function louisianaPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3535");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        louisianaPolygon();
                        louisianaMarkers();
                        map.setView([30.984, -94], 6);
                        group.addTo(map);

                        //console.log("refilterLayersLouisiana");
                    },
                    Maine: function (varState) {
                        function maineMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "ME");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "ME" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "ME" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "ME" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "ME" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "ME" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "ME" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "ME" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "ME" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "ME" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "ME" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "ME" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "ME" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "ME" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "ME" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function mainePolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3561");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        mainePolygon();
                        maineMarkers();
                        map.setView([45.253, -72], 6);
                        group.addTo(map);

                        //console.log("refilterLayersMaine");
                    },
                    Maryland: function (varState) {
                        function marylandMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "MD");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "MD" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "MD" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "MD" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "MD" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "MD" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "MD" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "MD" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "MD" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "MD" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "MD" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "MD" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "MD" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "MD" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "MD" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function marylandPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3557");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        marylandPolygon();
                        marylandMarkers();
                        map.setView([39.045, -78], 7);
                        group.addTo(map);

                        //console.log("refilterLayersMaryland");
                    },
                    Massachusetts: function (varState) {
                        function massachusettsMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "MA");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "MA" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "MA" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "MA" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "MA" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "MA" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "MA" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "MA" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "MA" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "MA" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "MA" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "MA" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "MA" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "MA" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "MA" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function massachusettsPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3513");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        massachusettsPolygon();
                        massachusettsMarkers();
                        map.setView([42.2, -72.4], 7);
                        group.addTo(map);

                        //console.log("refilterLayersMassachusetts");
                    },
                    Michigan: function (varState) {
                        function michiganMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "MI");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "MI" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "MI" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "MI" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "MI" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "MI" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "MI" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "MI" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "MI" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "MI" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "MI" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "MI" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "MI" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "MI" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "MI" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function michiganPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3562");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        michiganPolygon();
                        michiganMarkers();
                        map.setView([44.314, -89], 5);
                        group.addTo(map);

                        //console.log("refilterLayersMichigan");
                    },
                    Minnesota: function (varState) {
                        function minnesotaMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "MN");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "MN" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "MN" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "MN" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "MN" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "MN" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "MN" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "MN" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "MN" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "MN" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "MN" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "MN" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "MN" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "MN" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "MN" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function minnesotaPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3514");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        minnesotaPolygon();
                        minnesotaMarkers();
                        map.setView([46.729, -96], 5);
                        group.addTo(map);

                        //console.log("refilterLayersMinnesota");
                    },
                    Mississippi: function (varState) {
                        function mississippiMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "MS");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "MS" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "MS" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "MS" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "MS" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "MS" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "MS" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "MS" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "MS" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "MS" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "MS" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "MS" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "MS" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "MS" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "MS" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function mississippiPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3544");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        mississippiPolygon();
                        mississippiMarkers();
                        map.setView([32.6, -92], 6);
                        group.addTo(map);

                        //console.log("refilterLayersMississippi");
                    },
                    Missouri: function (varState) {
                        function missouriMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "MO");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "MO" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "MO" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "MO" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "MO" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "MO" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "MO" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "MO" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "MO" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "MO" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "MO" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "MO" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "MO" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "MO" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "MO" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function missouriPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3531");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        missouriPolygon();
                        missouriMarkers();
                        map.setView([37.964, -95], 6);
                        group.addTo(map);

                        //console.log("refilterLayersMissouri");
                    },
                    Montana: function (varState) {
                        function montanaMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "MT");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "MT" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "MT" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "MT" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "MT" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "MT" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "MT" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "MT" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "MT" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "MT" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "MT" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "MT" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "MT" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "MT" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "MT" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function montanaPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3515");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        montanaPolygon();
                        montanaMarkers();
                        map.setView([46.879, -113], 5);
                        group.addTo(map);

                        //console.log("refilterLayersMontana");
                    },
                    Nebraska: function (varState) {
                        function nebraskaMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "NE");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "NE" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "NE" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "NE" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "NE" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "NE" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "NE" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "NE" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "NE" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "NE" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "NE" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "NE" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "NE" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "NE" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "NE" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function nebraskaPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3532");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        nebraskaPolygon();
                        nebraskaMarkers();
                        map.setView([41.4925, -102.5], 6);
                        group.addTo(map);

                        //console.log("refilterLayersNebraska");
                    },
                    Nevada: function (varState) {
                        function nevadaMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "NV");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "NV" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "NV" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "NV" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "NV" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "NV" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "NV" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "NV" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "NV" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "NV" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "NV" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "NV" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "NV" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "NV" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "NV" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function nevadaPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3523");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        nevadaPolygon();
                        nevadaMarkers();
                        map.setView([38.8026, -122], 5);
                        group.addTo(map);

                        //console.log("refilterLayersNevada");
                    },
                    NewHampshire: function (varState) {
                        function newHampshireMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "NH");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "NH" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "NH" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "NH" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "NH" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "NH" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "NH" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "NH" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "NH" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "NH" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "NH" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "NH" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "NH" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "NH" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "NH" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function newHampshirePolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3538");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        newHampshirePolygon();
                        newHampshireMarkers();
                        map.setView([43.8, -73], 7);
                        group.addTo(map);

                        //console.log("refilterLayersNewHampshire");
                    },
                    NewJersey: function (varState) {
                        function newJerseyMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "NJ");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "NJ" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "NJ" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "NJ" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "NJ" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "NJ" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "NJ" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "NJ" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "NJ" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "NJ" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "NJ" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "NJ" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "NJ" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "NJ" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "NJ" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function newJerseyPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3558");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        newJerseyPolygon();
                        newJerseyMarkers();
                        map.setView([40.0583, -76], 7);
                        group.addTo(map);

                        //console.log("refilterLayersNewJersey");
                    },
                    NewMexico: function (varState) {
                        function newMexicoMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "NM");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "NM" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "NM" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "NM" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "NM" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "NM" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "NM" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "NM" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "NM" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "NM" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "NM" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "NM" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "NM" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "NM" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "NM" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function newMexicoPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3524");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        newMexicoPolygon();
                        newMexicoMarkers();
                        map.setView([34.5199, -110], 5);
                        group.addTo(map);

                        //console.log("refilterLayersNewMexico");
                    },
                    NewYork: function (varState) {
                        function newYorkMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "NY");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "NY" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "NY" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "NY" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "NY" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "NY" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "NY" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "NY" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "NY" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "NY" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "NY" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "NY" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "NY" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "NY" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "NY" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function newYorkPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3559");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        newYorkPolygon();
                        newYorkMarkers();
                        map.setView([43, -77], 6);
                        group.addTo(map);

                        //console.log("refilterLayersNewYork");
                    },
                    NorthCarolina: function (varState) {
                        function northCarolinaMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "NC");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "NC" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "NC" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "NC" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "NC" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "NC" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "NC" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "NC" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "NC" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "NC" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "NC" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "NC" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "NC" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "NC" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "NC" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function northCarolinaPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3549");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        northCarolinaPolygon();
                        northCarolinaMarkers();
                        map.setView([35.7595, -82], 6);
                        group.addTo(map);

                        //console.log("refilterLayersNorthCarolina");
                    },
                    NorthDakota: function (varState) {
                        function northDakotaMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "ND");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "ND" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "ND" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "ND" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "ND" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "ND" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "ND" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "ND" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "ND" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "ND" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "ND" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "ND" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "ND" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "ND" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "ND" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function northDakotaPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3516");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        northDakotaPolygon();
                        northDakotaMarkers();
                        map.setView([47.5514, -105.5], 5);
                        group.addTo(map);

                        //console.log("refilterLayersNorthDakota");
                    },
                    Ohio: function (varState) {
                        function ohioMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "OH");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "OH" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "OH" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "OH" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "OH" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "OH" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "OH" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "OH" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "OH" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "OH" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "OH" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "OH" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "OH" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "OH" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "OH" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function ohioPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3550");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        ohioPolygon();
                        ohioMarkers();
                        map.setView([40.4172, -84], 6);
                        group.addTo(map);

                        //console.log("refilterLayersOhio");
                    },
                    Oklahoma: function (varState) {
                        function oklahomaMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "OK");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "OK" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "OK" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "OK" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "OK" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "OK" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "OK" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "OK" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "OK" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "OK" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "OK" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "OK" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "OK" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "OK" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "OK" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function oklahomaPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3533");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        oklahomaPolygon();
                        oklahomaMarkers();
                        map.setView([35.0077, -100], 6);
                        group.addTo(map);

                        //console.log("refilterLayersOklahoma");
                    },
                    Oregon: function (varState) {
                        function oregonMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "OR");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "OR" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "OR" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "OR" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "OR" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "OR" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "OR" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "OR" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "OR" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "OR" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "OR" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "OR" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "OR" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "OR" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "OR" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function oregonPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3525");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        oregonPolygon();
                        oregonMarkers();
                        map.setView([43.8041, -125], 5);
                        group.addTo(map);

                        //console.log("refilterLayersOregon");
                    },
                    Pennsylvania: function (varState) {
                        function pennsylvaniaMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "PA");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "PA" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "PA" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "PA" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "PA" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "PA" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "PA" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "PA" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "PA" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "PA" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "PA" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "PA" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "PA" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "PA" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "PA" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function pennsylvaniaPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3560");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        pennsylvaniaPolygon();
                        pennsylvaniaMarkers();
                        map.setView([41.2033, -80], 6);
                        group.addTo(map);

                        //console.log("refilterLayersPennsylvania");
                    },
                    RhodeIsland: function (varState) {
                        function rhodeIslandMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "RI");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "RI" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "RI" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "RI" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "RI" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "RI" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "RI" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "RI" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "RI" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "RI" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "RI" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "RI" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "RI" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "RI" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "RI" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function rhodeIslandPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3539");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        rhodeIslandPolygon();
                        rhodeIslandMarkers();
                        map.setView([41.5800, -72.5], 7);
                        group.addTo(map);

                        //console.log("refilterLayersRhodeIsland");
                    },
                    SouthCarolina: function (varState) {
                        function southCarolinaMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "SC");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "SC" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "SC" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "SC" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "SC" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "SC" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "SC" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "SC" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "SC" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "SC" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "SC" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "SC" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "SC" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "SC" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "SC" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function southCarolinaPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3545");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        southCarolinaPolygon();
                        southCarolinaMarkers();
                        map.setView([33.8360, -83.5], 6);
                        group.addTo(map);

                        //console.log("refilterLayersSouthCarolina");
                    },
                    SouthDakota: function (varState) {
                        function southDakotaMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "SD");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "SD" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "SD" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "SD" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "SD" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "SD" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "SD" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "SD" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "SD" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "SD" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "SD" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "SD" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "SD" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "SD" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "SD" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function southDakotaPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3534");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        southDakotaPolygon();
                        southDakotaMarkers();
                        map.setView([43.9695, -103], 5);
                        group.addTo(map);

                        //console.log("refilterLayersSouthDakota");
                    },
                    Tennessee: function (varState) {
                        function tennesseeMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "TN");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "TN" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "TN" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "TN" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "TN" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "TN" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "TN" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "TN" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "TN" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "TN" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "TN" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "TN" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "TN" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "TN" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "TN" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function tennesseePolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3551");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        tennesseePolygon();
                        tennesseeMarkers();
                        map.setView([35.5174, -88], 6);
                        group.addTo(map);

                        //console.log("refilterLayersTennessee");
                    },
                    Texas: function (varState) {
                        function texasMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "TX");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "TX" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "TX" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "TX" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "TX" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "TX" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "TX" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "TX" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "TX" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "TX" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "TX" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "TX" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "TX" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "TX" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "TX" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function texasPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3536");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        texasPolygon();
                        texasMarkers();
                        map.setView([31.968, -103], 5);
                        group.addTo(map);

                        //console.log("refilterLayersTexas");
                    },
                    Utah: function (varState) {
                        function utahMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "UT");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "UT" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "UT" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "UT" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "UT" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "UT" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "UT" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "UT" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "UT" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "UT" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "UT" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "UT" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "UT" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "UT" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "UT" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function utahPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3526");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        utahPolygon();
                        utahMarkers();
                        map.setView([39.3209, -114.5], 6);
                        group.addTo(map);

                        //console.log("refilterLayersUtah");
                    },
                    Vermont: function (varState) {
                        function vermontMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "VT");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "VT" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "VT" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "VT" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "VT" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "VT" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "VT" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "VT" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "VT" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "VT" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "VT" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "VT" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "VT" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "VT" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "VT" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function vermontPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3540");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        vermontPolygon();
                        vermontMarkers();
                        map.setView([43.7, -74], 7);
                        group.addTo(map);

                        //console.log("refilterLayersVermont");
                    },
                    Virginia: function (varState) {
                        function virginiaMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "VA");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "VA" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "VA" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "VA" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "VA" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "VA" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "VA" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "VA" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "VA" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "VA" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "VA" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "VA" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "VA" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "VA" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "VA" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function virginiaPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3552");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        virginiaPolygon();
                        virginiaMarkers();
                        map.setView([37.4315, -81.5], 6);
                        group.addTo(map);

                        //console.log("refilterLayersVirginia");
                    },
                    Washington: function (varState) {
                        function washingtonMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "WA");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "WA" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "WA" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "WA" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "WA" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "WA" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "WA" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "WA" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "WA" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "WA" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "WA" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "WA" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "WA" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "WA" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "WA" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function washingtonPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3519");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        washingtonPolygon();
                        washingtonMarkers();
                        map.setView([47.7510, -123], 6);
                        group.addTo(map);

                        //console.log("refilterLayersWashington");
                    },
                    WestVirginia: function (varState) {
                        function westVirginiaMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "WV");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "WV" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "WV" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "WV" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "WV" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "WV" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "WV" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "WV" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "WV" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "WV" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "WV" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "WV" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "WV" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "WV" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "WV" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function westVirginiaPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3554");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        westVirginiaPolygon();
                        westVirginiaMarkers();
                        map.setView([38.5976, -83], 6);
                        group.addTo(map);

                        //console.log("refilterLayersWestVirginia");
                    },
                    Wisconsin: function (varState) {
                        function wisconsinMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "WI");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "WI" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "WI" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "WI" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "WI" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "WI" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "WI" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "WI" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "WI" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "WI" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "WI" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "WI" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "WI" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "WI" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "WI" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function wisconsinPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3553");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        wisconsinPolygon();
                        wisconsinMarkers();
                        map.setView([44.7844, -92], 6);
                        group.addTo(map);

                        //console.log("refilterLayersWisconsin");
                    },
                    Wyoming: function (varState) {
                        function wyomingMarkers (data) {
                            var promise = $.getJSON("<?php echo $usenergyjson; ?>");
                            promise.then(function(data){
                                var markers = L.geoJSON(data, {
                                    filter: function (feature, layer) {
                                        if (varState == "All") {
                                            return (feature.properties.State === "WY");
                                        }
                                        else if (varState == "LowCarbon") {
                                            return(feature.properties.State === "WY" && feature.properties["LowCarbon"] == "Yes");
                                        }
                                        else if (varState == "HighCarbon") {
                                            return(feature.properties.State === "WY" && feature.properties["LowCarbon"] == "No");
                                        }
                                        else if (varState == "Biomass") {
                                            return(feature.properties.State === "WY" && feature.properties["Fuel type"] == "Biomass")
                                        }
                                        else if (varState == "Coal") {
                                            return(feature.properties.State === "WY" && feature.properties["Fuel type"] == "Coal");
                                        }
                                        else if (varState == "Gas") {
                                            return(feature.properties.State === "WY" && feature.properties["Fuel type"] == "Gas");
                                        }
                                        else if (varState == "Geothermal") {
                                            return(feature.properties.State === "WY" && feature.properties["Fuel type"] == "Geothermal");
                                        }
                                        else if (varState == "Hydro") {
                                            return(feature.properties.State === "WY" && feature.properties["Fuel type"] == "Hydro");
                                        }
                                        else if (varState == "Nuclear") {
                                            return(feature.properties.State === "WY" && feature.properties["Fuel type"] == "Nuclear");
                                        }
                                        else if (varState == "Oil") {
                                            return(feature.properties.State === "WY" && feature.properties["Fuel type"] == "Oil");
                                        }
                                        else if (varState == "Other") {
                                            return(feature.properties.State === "WY" && feature.properties["Fuel type"] == "Other");
                                        }
                                        else if (varState == "Solar") {
                                            return(feature.properties.State === "WY" && feature.properties["Fuel type"] == "Solar");
                                        }
                                        else if (varState == "Storage") {
                                            return(feature.properties.State === "WY" && feature.properties["Fuel type"] == "Storage");
                                        }
                                        else if (varState == "Waste") {
                                            return(feature.properties.State === "WY" && feature.properties["Fuel type"] == "Waste");
                                        }
                                        else if (varState == "Wind") {
                                            return(feature.properties.State === "WY" && feature.properties["Fuel type"] == "Wind");
                                        }
                                    },
                                    pointToLayer: function (feature, latlng) {
                                        return L.circleMarker(latlng);
                                    },
                                    onEachFeature: onEachFeature,
                                    style: style
                                });
                                group.addLayer(markers);
                            });
                        }

                        function wyomingPolygon () {
                            var polygon = L.geoJSON(states, {
                                filter: function (feature) {
                                    return (feature.properties["adm1_code"] === "USA-3527");
                                },
                                style: myPolygonStyle
                            });
                            group.addLayer(polygon);
                        }

                        group.clearLayers();
                        wyomingPolygon();
                        wyomingMarkers();
                        map.setView([43.0759, -111], 5);
                        group.addTo(map);

                        //console.log("refilterLayersWyoming");
                    }

                }

                // find radius of marker

                function getRadius(d) {
                    return d > 6400  ? 46.1 :
                            d > 3200  ? 32.9 :
                            d > 1600  ? 23.6 :
                            d > 800  ? 17.0 :
                            d > 400  ? 12.3 :
                            d > 200 ? 8.9 :
                            d > 100  ? 6.6 :
                            d > 50  ? 5.0 :
                            d > 25  ? 3.8 :
                                    3.5;
                }

                //old scale - new one makes variance in data easier to see


                // function getRadius(d) {
                //     return d > 6400  ? 35.5 :
                //             d > 3200  ? 31.5 :
                //             d > 1600  ? 27.5 :
                //             d > 800  ? 23.5 :
                //             d > 400  ? 19.5 :
                //             d > 200 ? 15.5 :
                //             d > 100  ? 11.5 :
                //             d > 50  ? 7.5 :
                //                     3.5;
                // }

                // colors to be used

                var colors = {
                    "Coal": "#333333",
                    "Gas": "#216184",
                    "Solar": "#EFC530",
                    "Nuclear": "#A14A7B",
                    "Oil": "#673b9b",
                    "Hydro": "#2cb0c1",
                    "Wind": "#136400",
                    "Biomass": "#A7B734",
                    "Waste": "#d67b36",
                    "Storage": "#e58888",
                    "Geothermal": "#C7432B",
                    "Other": "#7c5641"
                }

                // style of markers

                function style(feature) {
                    return {
                        fillColor: colors[feature.properties["Fuel type"]],
                        weight: 0.4,
                        opacity: 0.37,
                        color: '#f3f3f3',
                        fillOpacity: 0.73,
                		radius: getRadius(feature.properties["Capacity (MW)"])
                    };
                }

                // add pop up

                function onEachFeature(feature, layer) {
                	// does this feature have a property named popupContent?
                	if (feature.properties) {
                		layer.bindPopup('<h1 style= color:'+ colors[feature.properties["Fuel type"]] +';><b>'+feature.properties["Plant Name"]+'</h1>Capacity: </b>'+feature.properties["Capacity label"]+' MW<br /><b>Type: </b>'+feature.properties["Fuel label"]+'<br /><b>State: </b>'+feature.properties["State label"]+'<br /><b>Year opened: </b>'+feature.properties["Year opened"], {closeButton: false, offset: L.point(0, -20)});
                        layer.on('mouseover', function() { layer.openPopup(); });
                        layer.on('mouseout', function() { layer.closePopup(); });
                	};
                }

                // add zoomHome plugin

                var zoomHome = L.Control.zoomHome({position: 'topright'});
                zoomHome.addTo(map);


                //group.clearLayers();
                //newJerseyPolygon();
                //newJerseyMarkers();

                function zoomto(zlat,zlong){
                  map.setView([zlat,zlong], 18);
                  group.addTo(map);
                }
                // states dropdown menu

                $('#selector1').change(function(){

                    var functionName = $(this).val()

                    varState = $("#selector2").val()
                    var sel4 = $("#selector4").val()

                    refilterLayers[functionName](varState,sel4);

                    var newSubtitle = $("#selector1 option:selected").text()

                    $("#subtitle").fadeOut(200, function() {
                        $(this).text(newSubtitle).fadeIn(200);
                    });

                    updateTotal(functionName);

                });

                // fuel type dropdown menu

                $('#selector2').change(function() {

                    varState = $(this).val()

                    var functionName = $("#selector1").val()
                    var sel4 = $("#selector4").val()

                    refilterLayers[functionName](varState,sel4);

                });


                // Status

                $('#selector4').change(function() {

                    var sel4 = $(this).val()

                    var functionName = $("#selector1").val()
                    var varState = $("#selector2").val()

                    refilterLayers[functionName](varState,sel4);

                });

                // reset dropdown on window reload

                $(document).ready(function () {
                    $("select").each(function () {
                        $(this).val($(this).find('option[selected]').val());
                    });
                })
/////////////////////////////////////
/////////////////////////////////////
/////////////////////////////////////
/////////////////////////////////////
/////////////////////////////////////
                // update total as state changes
/////////////////////////////////////
                var totalsArray = {
                    All: "1,197,676",
                    Alabama: "31,565",
                    Alaska: "3,012",
                    Arizona: "32,980",
                    Arkansas: "16,339",
                    California: "86,563",
                    Colorado: "18,124",
                    Connecticut: "9,937",
                    Delaware: "3,682",
                    Florida: "69,879",
                    Georgia: "39,512",
                    Hawaii: "3,500",
                    Idaho: "5,231",
                    Illinois: "50,813",
                    Indiana: "28,704",
                    Iowa: "19,088",
                    Kansas: "17,077",
                    Kentucky: "23,827",
                    Louisiana: "28,618",
                    Maine: "5,240",
                    Maryland: "15,078",
                    Massachusetts: "15,853",
                    Michigan: "32,175",
                    Minnesota: "18,064",
                    Mississippi: "18,107",
                    Missouri: "23,884",
                    Montana: "6,419",
                    Nebraska: "9,234",
                    Nevada: "13,241",
                    NewHampshire: "4,675",
                    NewJersey: "22,038",
                    NewMexico: "9,675",
                    NewYork: "44,118",
                    NorthCarolina: "35,528",
                    NorthDakota: "8,626",
                    Ohio: "31,715",
                    Oklahoma: "28,307",
                    Oregon: "17,257",
                    Pennsylvania: "48,390",
                    RhodeIsland: "2,130",
                    SouthCarolina: "24,738",
                    SouthDakota: "4,397",
                    Tennessee: "24,175",
                    Texas: "129,784",
                    Utah: "9,952",
                    Vermont: "813",
                    Virginia: "29,198",
                    Washington: "31,826",
                    WestVirginia: "15,996",
                    Wisconsin: "19,111",
                    Wyoming: "9,483"
                }
                totalsArray=[];
                function updateTotal (functionName) {

                    var myTotal = totalsArray[functionName]

                    //console.log(myTotal);

                    $(".total").fadeOut(200, function() {
                        $("#total").text(myTotal);
                        $(".total").fadeIn(200);
                    });
                }


                // horizontal bar chart

                var margin = {top: 50, right: (parseInt(d3.select("#chart-1").style("width"))/13 + 10), bottom: 10, left: (parseInt(d3.select("#chart-1").style("width"))/5 + 30)},
                    width = parseInt(d3.select("#chart-1").style("width")) - margin.left - margin.right,
                    height = 360 - margin.top - margin.bottom;


                var y = d3.scale.ordinal().rangeRoundBands([0, height * 0.95], .3);

                var x = d3.scale.linear().range([0, width]);

                var xAxis = d3.svg.axis()
                .scale(x)
                .orient("top")
                .ticks(3);

                var yAxis = d3.svg.axis()
                .scale(y)
                .orient("left");

                var colorScale = d3.scale.ordinal()
                .domain(["Biomass", "Coal", "Gas", "Geothermal", "Hydro", "Nuclear", "Oil", "Other", "Solar", "Storage", "Waste", "Wind"])
                .range(["#A7B734", "#333333" , "#216184", "#C7432B", "#2cb0c1", "#A14A7B", "#673b9b", "#7c5641", "#EFC530", "#e58888", "#d67b36", "#136400"]);

                var commaFormat = d3.format(',');


                var svg = d3.select("#chart-1").append("svg")
                .attr("width", width + margin.left + margin.right)
                .attr("height", height + margin.top + margin.bottom)
                .attr("id", "svg-1")
                .append("g")
                .attr("transform",
                      "translate(" + margin.left + "," + margin.top + ")");

                var div = d3.select("#chart-1")
                .append("div")  // declare the tooltip div
                .attr("class", "tooltip")              // apply the 'tooltip' class
                .style("opacity", 0);                  // set the opacity to nil

                // add x axis label

                svg.append("g")
                    .attr("class", "label")
                    .append("text")
                    .attr("x", width)
                    .attr("y", -35)
                    .style("text-anchor", "end")
                    .text("Capacity (MW)");

                var state = "";

                initialDraw ("All");

                // set up initial transition

                function initialDraw (state) {

                    //d3.csv("https://datahub360-powergeneration.s3.us-west-2.amazonaws.com/us-energy-totals.csv", function(error, data) {
                    d3.csv("<?php echo getjsondata("us-energy-totals.csv"); ?>", function(error, data) {

                            data.forEach(function(d) {
                                d.Type = d.Type;
                                d[state] = +d[state];
                            });

                            y.domain(data.sort(function(a,b){return b[state]-a[state];}).map(function(d) { return d.Type; }));
                            x.domain([0, d3.max(data, function(d) { return d[state] * 1.1;})]);

                            svg.append("g")
                            .attr("class", "x axis")
                            .transition()
                            .duration(1000)
                            .call(xAxis)
                            .selectAll("text")
                            .style("text-anchor", "middle")
                            .attr("dx", "0em")
                            .attr("dy", "-.25em");

                            svg.append("g")
                            .attr("class", "y axis")
                            .transition()
                            .duration(1000)
                            .call(yAxis)
                            .selectAll("text")
                            .attr("dx", "0em")
                            .attr("dy", "0.2em")
                            .style("text-anchor", "end");

                            // now deal with bars

                            var bar = svg.selectAll(".bar").data(data);

                            var barExit = bar.exit().remove();

                            var barEnter = bar.enter()
                                .append("g")
                                .attr("class", "bar");

                            var barRects = barEnter.append("rect")
                                .attr("rx", 4)
                                .attr("y", function(d) { return y(d.Type); })
                                .attr("width", function(d) { return x(0); })
                                .attr("x", function(d) { return x(0); })
                                .attr("height", y.rangeBand() * 0.78)
                                .style("fill",function(d) {return colorScale(d.Type);})
                                .on("mouseover", function(d) {
                                    div.transition()
                                        .duration(500)
                                        .style("opacity", 0);
                                    div.transition()
                                        .duration(200)
                                        .style("opacity", 1);
                                    div	.html("<b>Capacity: </b>" + commaFormat(d[state]) + " MW")
                                        .style("left", (d3.event.pageX) + "px")
                                        .style("top", (d3.event.pageY - 28) + "px");
                                    })
                                    .on("mouseout", function(d) {
                                    div.transition()
                                        .duration(500)
                                        .style("opacity", 0);
                                });


                            var barRectUpdate = bar.select("rect")
                                .transition()
                                .duration(1800)
                                .attr("y", function(d) { return y(d.Type); })
                                .attr("x", function(d) { return x(0); })
                                .attr("height", y.rangeBand() * 0.78)
                                .attr("width", function (d) { return x(d[state])})
                                .style("fill",function(d) {return colorScale(d.Type);});

                    })

                }

                // link chart to change with state dropdown

                d3.select("#selector1").on("change", selectState)

                function selectState() {

                    var state = this.options[this.selectedIndex].value

                    draw(state);

                    //console.log("update bar chart")

                    drawChart2(state);

                    //console.log("update bar chart 2")

                }

                function draw (state) {

                    //d3.csv("https://datahub360-powergeneration.s3.us-west-2.amazonaws.com/us-energy-totals.csv", function(error, data) {
                    d3.csv("<?php echo getjsondata("us-energy-totals.csv"); ?>", function(error, data) {


                            data.forEach(function(d) {
                                d.Type = d.Type;
                                d[state] = +d[state];
                            });

                            y.domain(data.sort(function(a,b){return b[state]-a[state];}).map(function(d) { return d.Type; }));
                            x.domain([0, d3.max(data, function(d) { return d[state] * 1.1;})]);

                            // remove and call axes

                            svg.select(".y.axis").remove();
                            svg.select(".x.axis").remove();

                            svg.append("g")
                            .attr("class", "x axis")
                            .transition()
                            .duration(1000)
                            .call(xAxis)
                            .selectAll("text")
                            .style("text-anchor", "middle")
                            .attr("dx", "0em")
                            .attr("dy", "-.25em");

                            svg.append("g")
                            .attr("class", "y axis")
                            .transition()
                            .duration(1000)
                            .call(yAxis)
                            .selectAll("text")
                            .attr("dx", "0em")
                            .attr("dy", "0.2em")
                            .style("text-anchor", "end");

                            // now deal with bars

                            var bar = svg.selectAll(".bar").data(data);

                            var barExit = bar.exit().remove();

                            var barEnter = bar.enter()
                                .append("g")
                                .attr("class", "bar");

                            var barRects = barEnter.append("rect")
                                .attr("rx", 4)
                                .attr("y", function(d) { return y(d.Type); })
                                .attr("width", function (d) { return x(d[state])})
                                .attr("x", function(d) { return x(0); })
                                .attr("height", y.rangeBand() * 0.78)
                                .style("fill",function(d) {return colorScale(d.Type);});

                            var barRectUpdate = bar.select("rect")
                                .transition()
                                .duration(900)
                                .attr("y", function(d) { return y(d.Type); })
                                .attr("x", function(d) { return x(0); })
                                .attr("height", y.rangeBand() * 0.78)
                                .attr("width", function (d) { return x(d[state])})
                                .style("fill",function(d) {return colorScale(d.Type);});

                            // ensure that tooltip changes with data

                            var tooltipUpdate = bar.select("rect")
                                .on("mouseover", function(d) {
                                    div.transition()
                                        .duration(500)
                                        .style("opacity", 0);
                                    div.transition()
                                        .duration(200)
                                        .style("opacity", 1);
                                    div	.html("<span id='#capacity'><b>Capacity: </b></span>" + commaFormat(d[state]) + " MW")
                                        .style("left", (d3.event.pageX) + "px")
                                        .style("top", (d3.event.pageY - 28) + "px");
                                    })
                                .on("mouseout", function(d) {
                                    div.transition()
                                        .duration(500)
                                        .style("opacity", 0);
                                });

                    })

                }

                // redraw d3 graphs on window resize - currently not working particularly well but at least with this version the width of the svg itself updates

                $(window).on("resize", function () {

                    var width = parseInt(d3.select("#chart-1").style("width")) - margin.left - margin.right;

                    d3.select("#chart-1").select("#svg-1").attr("width", width + margin.left + margin.right);

                    d3.select("#chart-2").select("#svg-2").attr("width", width + margin.left + margin.right);

                });

                // second chart, a single normalised horizontal bar chart

                function drawChart2 (state) {

                    // remove any previous svg

                    d3.selectAll('#chart-2 svg').remove();

                    //Width and height
                    var margin = {top: 30, right: (parseInt(d3.select("#chart-2").style("width"))/13 + 10), bottom: 20, left: 30},
                        width = parseInt(d3.select("#chart-2").style("width")) - margin.left - margin.right,
                        height = 80 - margin.top - margin.bottom;

                    //Set up stack method
                    var stack = d3.layout.stack();

                    var yScale = d3.scale.ordinal();
                    var xScale = d3.scale.linear().range([0, width]);

                    //Colors correspond to index of stacked data

                    var colors = d3.scale.linear()
                        .domain([0, 1 ])
                        .range(["#2cb0c1", "#333333"]);


                    //d3.csv("https://datahub360-powergeneration.s3.us-west-2.amazonaws.com/LowCarbonPercentage.csv", function(data) {
                    d3.csv("<?php echo getjsondata("LowCarbonPercentage.csv"); ?>", function(data) {

                        // filter data depending on state

                        filteredData = data.filter(function(row) {
                            return row['State'] == state
                        })

                        //console.log(filteredData);

                        //Create a new array to hold restructured dataset
                        var dataset = [

                            { type: "LowCarbon",
                            details: []
                            },
                            { type: "HighCarbon",
                            details: []
                            }
                        ];
                        //Loop once for each row in data
                        for (var i = 0; i < dataset.length; i++) {

                            for (var j = 0; j < filteredData.length; j++) {
                                if (dataset[i].type == "LowCarbon") {
                                    dataset[i].details.push({
                                        State: filteredData[j].State,
                                        Amount: filteredData[j].LowCarbon
                                    });
                                }
                                else if (dataset[i].type == "HighCarbon") {
                                    dataset[i].details.push({
                                        State: filteredData[j].State,
                                        Amount: filteredData[j].HighCarbon
                                    });
                                }
                            }
                        };


                        // Create array with each color group for a color legend
                        series = dataset.map(function (d) {
                                return d.type;
                            });

                        dataset = dataset.map(function (d) {
                                return d.details.map(function (o, i) {
                                    // Structure it so that your numeric
                                    // axis (the stacked amount) is y
                                    return {
                                        y: +o.Amount,
                                        x: o.State
                                    };
                                });
                            }),

                    //console.log(dataset);

                    //Data, stacked
                    stack(dataset);

                    var dataset = dataset.map(function (group) {
                        return group.map(function (d) {
                            // Invert the x and y values, and y0 becomes x0
                            return {
                                x: +d.y,
                                y: d.x,
                                x0: +d.y0
                            };
                        });
                    });

                    //Set up scales

                    // Array of state names for domain
                    var lowCarbonByState = dataset[0].map(function (d) {
                            return d.y;
                        });

                    yScale.domain(lowCarbonByState)
                        .rangeRoundBands([0, height], 0);

                    xScale.domain([0,
                        d3.max(dataset, function(group) {
                            return d3.max(group, function(d) {
                                return d.x + d.x0 ;
                            });
                        })
                    ])
                    .range([0, width]);

                    //Create SVG element
                    var svg = d3.select("#chart-2")
                                .append("svg")
                                .attr("id", "svg-2")
                                .attr("width", width + margin.left + margin.right )
                                .attr("height", height + margin.bottom + margin.top);

                    var div = d3.select("#chart-2")
                        .append("div")  // declare the tooltip div
                        .attr("class", "tooltip")              // apply the 'tooltip' class
                        .style("opacity", 0);// set the opacity to nil

                    var formatAsPercentage = d3.format("%");

                    var formatAsPercentage2 = d3.format(".1%");

                    var xAxis = d3.svg.axis()
                            .scale(xScale)
                            .orient('bottom')
                            .ticks(2)
                            .tickFormat(formatAsPercentage);

                    var yAxis = d3.svg.axis()
                            .scale(yScale)
                            .orient('left')
                            .tickValues([]);

                    // Add a group for each row of data

                    var groups = svg.selectAll("g")
                        .data(dataset)
                        .enter()
                        .append("g")
                        .style("fill", function(d, i) {
                            return colors(i);
                        });

                    // Add a rect for each data value

                    var rects = groups.selectAll("rect")
                        .data(function(d) { return d; })
                        .enter()
                        .append("rect")
                        .attr("rx", 4)
                        .attr("class", "bar")
                        .attr("x", function(d) {
                            return xScale(d.x0) + margin.left;
                        })
                        .attr("width", function(d) {
                            return xScale(d.x);
                        })
                        .attr("y", function(d) { return (y(d.State) + margin.top); })
                        .attr("height", y.rangeBand() * 0.85)
                        .on("mouseover", function() { tooltip.style("display", null); })
                        .on("mouseout", function() { tooltip.style("display", "none"); })
                        .on("mouseover", function(d) {
                            div.transition()
                                .duration(500)
                                .style("opacity", 0);
                            div.transition()
                                .duration(200)
                                .style("opacity", 1);
                            div	.html(formatAsPercentage2(d.x))
                                .style("left", (d3.event.pageX) + "px")
                                .style("top", (d3.event.pageY - 28) + "px");
                            })
                            .on("mouseout", function(d) {
                            div.transition()
                                .duration(500)
                                .style("opacity", 0);
                        });

                    svg.append('g')
                            .attr('class', 'axis')
                            .attr('transform', 'translate(' + margin.left + ',' + (margin.top + height) + ')')
                            .call(xAxis);

                    svg.append('g')
                        .attr('class', 'axis')
                        .attr('transform', 'translate(' + margin.left + ',' + margin.top + ')')
                        .call(yAxis);

                    svg.append("text")
                        .attr("class", "label-2")
                        .attr('x', margin.left + 5)
                        .attr('y', 20)
                        .style("fill", "#2cb0c1")
                        .text("Low carbon");

                    svg.append("text")
                        .attr("class", "label-2")
                        .attr('x', width + margin.left - 5)
                        .attr('y', 20)
                        .style("text-anchor", "end")
                        .style("fill", "#898989")
                        .text("High carbon");

                    });

                }

                // timeout seems to fix issue of it sometimes rendering improperly initially

                setTimeout(drawChart2("All"), 700);

                // jquery for toggle button

                $(".toggle").click(function() {
                    $('.side-bar').toggleClass('side-open');
                    $('.arrow-right-hidden').toggleClass('arrow-right');
                    $('.arrow-left').toggleClass('arrow-left-hidden');
                });
                $( document ).ready(function() {
                    //$("#basemaps select").val("day").change();
                });
                </script>
            </body>
        </html>
