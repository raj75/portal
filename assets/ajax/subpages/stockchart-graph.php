...
$user_one=$_SESSION['user_id'];
	...
	$symarrdash=$symdatesarr=array();
	if ($fwstmtb = $mysqli->prepare("SELECT fw.`name`,fwd.`symbol`, MAX(fwd.date) FROM financial_widget_data fwd left join financial_widget fw ON fwd.`symbol`=fw.`symbol`  WHERE (fwd.widget='financial' or fwd.widget='Financial') and (fw.widget='financial' or fw.widget='Financial') GROUP BY fwd.`symbol` ORDER BY  fw.`name`")) {
		$fwstmtb->execute();
		$fwstmtb->store_result();
		if ($fwstmtb->num_rows > 0) {
			$fwstmtb->bind_result($fwbname,$fwbsymbol,$fwbdate);
			while($fwstmtb->fetch()){
				if(empty($firstsymbol)) $firstsymbol=$fwbsymbol;
				$symdatesarr[$fwbsymbol]=array($fwbname,$fwbdate);
			}
		}
	}else{
	...
	if(!count($symdatesarr)) die("Nothing to show!");
	ksort($symdatesarr);

	foreach($symdatesarr as $kky=>$vvl){
		if ($fwstmt = $mysqli->prepare("SELECT  fwd.`symbol`,fwd.close,fwd.open,fwd.`change`,fwd.changePercent,fwd.volume, fwd.date FROM financial_widget_data fwd  WHERE (fwd.widget='financial' or fwd.widget='Financial') AND fwd.date='".$vvl[1]."' AND fwd.`symbol`='".$kky."' LIMIT 1")) {
			$fwstmt->execute();
			$fwstmt->store_result();
			if ($fwstmt->num_rows > 0) {
				$fwstmt->bind_result($fwsymbol,$fwlast_price,$fwprice,$fwprice_change,$fwprice_change_pct,$fwvolume,$fwdate);
				while($fwstmt->fetch()){
...
$symboltable=$datatable="financial_widget_data";
...
$symname=$sympchange=$symlastprice=$sympricechangepct=$updated_date="";
//if ($cmsymbol = mysqli_query($mysqli,"SELECT `name`,`symbol`,price,last_price,price_change,price_change_pct,volume,updated_date FROM ".$symboltable." WHERE (widget='financial' or widget='Financial') and replace(symbol , ' ','')='".mysqli_real_escape_string($mysqli,$symbol)."' LIMIT 1")) {

if ($cmsymbol = mysqli_query($mysqli,"SELECT  fwd.`symbol`,fwd.close,fwd.open,fwd.`change`,fwd.changePercent,fwd.volume,fwd.date FROM financial_widget_data fwd  WHERE fwd.source='FMP' AND (fwd.widget='financial' or fwd.widget='Financial') and replace(fwd.`symbol` , ' ','')='".mysqli_real_escape_string($mysqli,$symbol)."'  AND fwd.date='".$symdatesarr[$symbol][1]."' LIMIT 1")) {
	if (mysqli_num_rows($cmsymbol) > 0) {
		$cmrow=mysqli_fetch_assoc($cmsymbol);
		if(empty($cmrow["change"])) $cmrow["change"]=0;
		if(empty($cmrow["close"])) $cmrow["close"]=0;
		if(empty($cmrow["changePercent"])) $cmrow["changePercent"]=0;
		$symname=$symdatesarr[$symbol][0];
		$sympchange=round($cmrow["change"],2);
		$symlastprice=round($cmrow["close"],2);
		$sympricechangepct=round($cmrow["changePercent"],2);
		$updated_date=$cmrow["date"];
			/*"name"=>$cmrow["name"],
			"symbol"=>$cmrow["symbol"],
			"price"=>$cmrow["price"],
...
//die("SELECT `open`,`high`,low,close,adjusted_close,volume,date FROM ".$datatable." WHERE (widget='financial' or widget='Financial') and TRIM(symbol)='".mysqli_real_escape_string($mysqli,$symbol)."' ORDER BY `date`");

if ($cksymbol = mysqli_query($mysqli,"SELECT `open`,`high`,low,close,adjusted_close,volume,date FROM ".$datatable." WHERE (widget='financial' or widget='Financial') and TRIM(symbol)='".mysqli_real_escape_string($mysqli,$symbol)."' ORDER BY `date`")) {
	if (mysqli_num_rows($cksymbol) > 0) {
		while($ckrow=mysqli_fetch_assoc($cksymbol)){
			$symarr[]=array(
				"Date"=>$ckrow["date"],
				"Open"=>round($ckrow["open"],2),
				"High"=>round($ckrow["high"],2),
				"Low"=>round($ckrow["low"],2),
				"Close"=>round($ckrow["close"],2),
				"Volume"=>$ckrow["volume"],
				"Adj Close"=>round($ckrow["adjusted_close"],2)
			);
		}
	}
}
...
.nopadding{margin:0;padding:0;/*position:fixed;*/position:sticky;bottom:0;width:100%;}
			...
			*/			
			...
			xAxis.zoomToDates(
			  ...
			  new Date(newMinDateStr)			  			  
						...
						.symbollist .marbt5{margin-bottom: 6px;cursor:pointer;}
...
<?php
if(count($symarrdash)){
	foreach($symarrdash as $ky => $vl){ $vl["price_change"]=(int)$vl["price_change"];
...
?>
			<div class="row marbt5 <?php if($firstsymbol==$vl["symbol"]){echo 'selected'; } ?>" id="<?php echo @trim($vl["symbol"]); ?>">
...
<script>
$( document ).ready(function() {
    //$('#loadgraph').attr('src', 'assets/ajax/subpages/stockchart-graph.php?ct=<?php echo time(); ?>&symbol=<?php echo rawurlencode ($firstsymbol); ?>');
	$( ".marbt5" ).removeClass( "selected" );
	var phpsymbol = "#<?php echo $symbol;?>";
	$('div[id="<?php echo $symbol;?>"]').addClass( "selected" );
	//$( "#<?php echo $symbol;?>" ).addClass( "selected" );
});
...
$('.marbt5').on('click', function() {
	...
	$(parent.document).find("#loadgraph").attr('src', 'assets/ajax/subpages/stockchart-graph.php?ct=<?php echo time(); ?>&symbol='+encodeURIComponent($(this).attr("id"))+'');
  //$('#loadgraph').attr('src', 'assets/ajax/subpages/stockchart-graph.php?ct=<?php echo time(); ?>&symbol='+encodeURIComponent($(this).attr("id"))+'');
});
</script>
