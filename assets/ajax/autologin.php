...
$stmtat = $mysqli->prepare("SELECT up.accuvio_user,up.accuvio_pass,up.capturis_user,up.capturis_pass,c.ubm_type FROM user up,company c WHERE up.user_id= '".$user_one."' and c.company_id=up.company_id LIMIT 1");
if(!$stmtat){
...
if ($stmtat->num_rows > 0) {
	$stmtat->bind_result($at_accuvio_user,$at_accuvio_pass,$at_capturis_user,$at_capturis_pass,$at_ubm);
	$stmtat->fetch();
...
<HTML>
<HEAD>
<TITLE><?php if($w=="Capturis") echo "UBM";else echo $w; ?> Auto Login</TITLE>
...
$(document).ready(function(){
    ...
    function loginForm() {
		<?php
		if(($w=="Capturis" and @trim($at_capturis_user) != "" and @trim($at_capturis_pass) != "") or ($w=="Accuvio" and @trim($at_accuvio_user) != "" and @trim($at_accuvio_pass) != "")){?>
			$('.rbDecorated').click();
		<?php }else{?>
...
<BODY>
    ...
    <form <?php if($w=="Capturis" and @trim($at_capturis_user) != "" and @trim($at_capturis_pass) != ""){
		...
		<?php
		if($w=="Capturis" and @trim($at_capturis_user) != "" and @trim($at_capturis_pass) != ""){
