...
if ($contstmt = $mysqli->prepare("SELECT e.id,e.`sender`,e.subject,e.contentType,e.content,e.sentDateTime,e.attachments,e.`toRecipients`,e.external_attachments FROM fax.`emails` e where e.id='".$uid."' ".$ssubsql." LIMIT 1")) {
	$contstmt->execute();
...
//echo $s3folderpath.'/'.$afilename_tmp[1];
		$infotarget = $s3Client->doesObjectExist('datahub360-correspondence', "fax@vervantis.com/".$filename);
		if(!empty($filename) and $infotarget)
		{
			$headers = $s3Client->headObject(array(
				  "Bucket" => 'datahub360-correspondence',
				  "Key" => "fax@vervantis.com/".$filename
				));
