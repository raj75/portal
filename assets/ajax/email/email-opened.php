...
if($_SESSION["group_id"] != 1 and $_SESSION["group_id"] != 2) $ssubsql=" and c.company_id=".$cname."";
else $ssubsql="";

//$ssubsql="";
/*if ($contstmt = $mysqli->prepare("SELECT id,from,from,subject,text_plain,text_html,date_recv,attachments FROM libremail.`messages` crs, company c where '".$fname."' = crs.folderpath and crs.id=".$uid." ".$ssubsql." LIMIT 1")) {*/
//if ($contstmt = $mysqli->prepare("SELECT lm.id,lm.`from`,lm.subject,lm.text_plain,lm.text_html,lm.date_recv,lm.attachments,la.`email` FROM libremail.`messages` lm,libremail.`accounts` la where lm.account_id=la.id and lm.id=".$uid." ".$ssubsql." LIMIT 1")) {
//if ($contstmt = $mysqli->prepare("SELECT id,`sender`,subject,contentType,content,sentDateTime,attachments,`toRecipients` FROM email.`emails` where id=".$uid." ".$ssubsql." LIMIT 1")) {

//if ($contstmt = $mysqli->prepare("SELECT id,`sender`,subject,contentType,content,sentDateTime,attachments,`toRecipients` FROM email.`emails` where id='".$uid."' ".$ssubsql." LIMIT 1")) {
if ($contstmt = $mysqli->prepare("SELECT e.id,e.`sender`,e.subject,e.contentType,e.content,e.sentDateTime,e.attachments,e.`toRecipients`,e.external_attachments FROM email.`emails` e, vervantis.company c where e.id='".$uid."' and e.folderpath LIKE CONCAT(c.email_folder_path,'%') ".$ssubsql." LIMIT 1")) {
	$contstmt->execute();
...
<h2 class="email-open-header">
	<?php echo htmlentities($cont_subject);?>
</h2>
...
//echo $s3folderpath.'/'.$afilename_tmp[1];
		$infotarget = $s3Client->doesObjectExist('datahub360-correspondence', "operations@vervantis.com/".$filename);
		if(!empty($filename) and $infotarget)
		{
			$headers = $s3Client->headObject(array(
				  "Bucket" => 'datahub360-correspondence',
				  "Key" => "operations@vervantis.com/".$filename
				));
      ...
      <li class="files">
        <a href="javascript:void(0);" class="files defaultcursor">
          ...
          <span class="details"><span class="fsize"><?php if(isset($headarr["ContentLength"])) echo formatSizeUnits($headarr["ContentLength"]); ?></span><span class="glyphicon glyphicon-download downloadf" onclick="openInNewTab('<?php echo $fileurl; ?>')" title="Download"></span></span>
...
<div id="dialognew" title="Preview"></div>
<style>.noshow{height: 12%;opacity: 0.8;position: absolute;right: 2%;top: 0;width: 12%;z-index: 9999;} #dialognew{overflow:hidden !important;text-align:center;}</style>
...
<?php if(isset($_SESSION) and ($_SESSION["group_id"] == 3 or $_SESSION["group_id"] == 5 or $_SESSION["group_id"] == 2 or $_SESSION["group_id"] == 1)){ ?>
		open: function (event, ui) {
			if($("#refreshiframe").length == 0) {
				$(this).parent().find('.ui-dialog-titlebar').append('<a href="javascript:void(0);" onclick="refreshiframe()" id="refreshiframe"><span class="glyphicon glyphicon-refresh" style="vertical-align: bottom;"></span></a>');
			}
			   $('#download-d-btn')
...
function refreshiframe(){
	$( '#googleload' ).attr( 'src', function ( i, val ) { return val; });
}
