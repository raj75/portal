...
$tablename="rss";
$imageloc="https://datahub360-public.s3-us-west-2.amazonaws.com/streaming+news/";
...
if ($ckmainstmt = mysqli_query($conn,"SELECT id,title,image,description,url,published_date FROM ".$tablename." WHERE newstype='NorthAmerica' order by published_date desc LIMIT 0,63")) {
	if (mysqli_num_rows($ckmainstmt) > 0) {
		while ($row = mysqli_fetch_assoc($ckmainstmt)) {
			$feed[]=array(
			'image'  => $row['image'],
			'title' => $row['title'],
...
<?php for ( $x = 0; $x < $limit; $x++ ) {
   ...
   // $img = ($feed[ $x ]['image'])?$feed[ $x ]['image']:"");
	...
	$tempdesc=preg_replace("/<img([^<>]+)>/s","",$feed[ $x ]['desc']);
		 ...
		 <div class="fg_story" style="border-color: rgb(88, 95, 105); display: block; opacity: 1;">
			...
			<?php } ?>
				<div class="fg_storytext">
					<div>
						<span class="fg_title bcolor" id="nafgtitle<?php echo $x; ?>" style="color:#000000;"><?php echo (strlen($title) >80?substr($title, 0, 80)."...":$title); ?></span>
	...
	origColumnHeight = imageColumn.height();
		...
		function scrollColumn() {
			imageColumn.css({'top': '0'})
					   .animate({top: -origColumnHeight},70000, 'linear', scrollColumn);
	  ...
	  $(".naplink").click(function() {
		  ...
		  var natemptitle= $( "#nafgtitle"+naattr ).html();
			...
			parent.$("#dialog").dialog("option","title","North America News: "+natemptitle).dialog("open");
			return false;
	  });
</script> 
</html>