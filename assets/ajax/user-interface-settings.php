...
//SELECT id,email,usergroups_id,gender,status FROM user';
	...
	$sql='SELECT user_id,email,usergroups_id,gender,status FROM user where company_id=(SELECT company_id FROM user WHERE user_id= "'.$user_one.'")';
...
<script type="text/javascript">
	...
	loadScript("assets/plugins/datatables1.11.3/datatables.min.js", function(){
		loadScript("assets/js/jquery.nestable.js", pagefunction)
	});
...
function loadusermenu(userid) {
	$('#response').html('');
    $('#response').load('assets/ajax/custom-user-menu.php?editmenu=true&userid='+userid);
}
</script>
