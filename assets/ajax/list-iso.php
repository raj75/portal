	...
	<thead>
		...
		<tr class="dropdown">
			<th class="hasinput d-1">
				<select class="form-control isodrp iso1">
					<option value="">Filter ISO</option>
<?php
	if ($stmt_iso = $mysqli->prepare('SELECT distinct(ISO) FROM iso.iso_filters where ISO IS NOT NULL ORDER BY ISO')) {
        $stmt_iso->execute();
        $stmt_iso->store_result();
        if ($stmt_iso->num_rows > 0) {
			$stmt_iso->bind_result($diso);
			while($stmt_iso->fetch()) {
				echo '<option value="'.$diso.'">'.$diso.'</option>';
...
?>
			...
			<th class="hasinput">
				<select class="form-control isodrp iso2">
					<option value="">Filter NODE ID</option>
<?php
	if ($stmt_iso = $mysqli->prepare('SELECT distinct(NODE_ID) FROM iso.iso_filters where NODE_ID is not null ORDER BY NODE_ID')) {
        $stmt_iso->execute();
        $stmt_iso->store_result();
        if ($stmt_iso->num_rows > 0) {
			...
			while($stmt_iso->fetch()) {
				if($dnodeid == "") continue;
				echo '<option value="'.$dnodeid.'">'.$dnodeid.'</option>';
...
?>
			...
			<th class="hasinput">
				<input class="dtdisplay datepicker " data-dateformat="mm/dd/yy" type="text" placeholder="Filter OPR DT">
			</th>
			<th class="hasinput">
				<select class="form-control isodrp">
					<option value="">Filter OPR HR</option>
			...
			<th class="hasinput">
				<input type="text" class="form-control" placeholder="Filter MCC" />
			</th>
			<th class="hasinput">
				<input type="text" class="form-control" placeholder="Filter MCE" />
			</th>
			<th class="hasinput">
				<input type="text" class="form-control" placeholder="Filter MCL" />
			</th>
		</tr>
		<tr>
