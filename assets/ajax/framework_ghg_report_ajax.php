...
$columns = array(
   ...
   array( 'db' => 'CO2_Pounds',     'dt' => 'CO2_Pounds',  'formatter' => function( $d, $row ) {return number_format($d, 2, '.', ',');} ),
   array( 'db' => 'CH4_Pounds',     'dt' => 'CH4_Pounds',  'formatter' => function( $d, $row ) {return number_format($d, 2, '.', ',');} ),
   array( 'db' => 'N2O_Pounds',     'dt' => 'N2O_Pounds',  'formatter' => function( $d, $row ) {return number_format($d, 2, '.', ',');} ),
   array( 'db' => 'CO2-e_Pounds',     'dt' => 'CO2-e_Pounds',  'formatter' => function( $d, $row ) {return number_format($d, 2, '.', ',');} ),
   array( 'db' => 'NOx-Annual_Pounds',     'dt' => 'NOx-Annual_Pounds',  'formatter' => function( $d, $row ) {return number_format($d, 2, '.', ',');} ),
   array( 'db' => 'NOx-Ozone_Season_Pounds',     'dt' => 'NOx-Ozone_Season_Pounds',  'formatter' => function( $d, $row ) {return number_format($d, 2, '.', ',');} ),
   array( 'db' => 'SO2_Pounds',     'dt' => 'SO2_Pounds',  'formatter' => function( $d, $row ) {return number_format($d, 2, '.', ',');} ),
   array( 'db' => 'CalPeriod',     'dt' => 'CalPeriod',  'formatter' => function( $d, $row ) {return $d;} ),
   array( 'db' => 'ClientID',     'dt' => 'ClientID',  'formatter' => function( $d, $row ) {return $d;} ),
   array( 'db' => 'scope1year',     'dt' => 'scope1year',  'formatter' => function( $d, $row ) {return $d;} ),
   array( 'db' => 'scope2year',     'dt' => 'scope2year',  'formatter' => function( $d, $row ) {return $d;} ),
...
$_POST['columns'][17]['search']['value'] = "";
$_POST['columns'][17]['searchable'] = false;
...
// ---------scope1year filter--------------
$scope1year= $_POST['columns'][19]['search']['value'];

if ( isset($scope1year) AND strlen($scope1year) > 1 ) {
	$scope1year = mysqli_real_escape_string($mysqli,$scope1year); 
	$scope1year_qry = " AND j.Year='$scope1year' ";
}
$_POST['columns'][19]['search']['value'] = "";
$_POST['columns'][19]['searchable'] = false;

// ---------scope2year filter--------------
$scope2year= $_POST['columns'][20]['search']['value'];

if ( isset($scope2year) AND strlen($scope2year) > 1 ) {
	$scope2year = mysqli_real_escape_string($mysqli,$scope2year); 
	$scope2year_qry = " AND j.Year='$scope2year' ";
}
$_POST['columns'][20]['search']['value'] = "";
$_POST['columns'][20]['searchable'] = false;
...
SELECT
    ...
    ROUND(SUM((a.CalBaseCost + a.CalUsageCost) * f.Allocation), 2) AS Cost,
		...
		a.CalPeriod,
		a.ClientID,
		j.Year as scope1year,
		j.Year as scope2year
FROM
	ubm_database.tblCostUsage  a
LEFT JOIN
    ubm_database.tblMeters AS c ON a.MeterID = c.MeterID	
