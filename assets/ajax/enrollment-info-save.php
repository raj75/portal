...
Editor::inst( $db, $table, 'ID' )
	->fields(
		Field::inst( 'Commodity' )->validator( Validate::notEmpty( ValidateOptions::inst()
		...
		Field::inst( 'Utility' )->validator( Validate::notEmpty( ValidateOptions::inst()
                ->message( 'Utility is required' ) 
            ) )
			->setFormatter( function ( $val, $data, $opts ) {
                 return $val;
            } ),		
		Field::inst( 'ISO' )->validator( Validate::notEmpty( ValidateOptions::inst()
                ->message( 'ISO is required' ) 
            ) )
			->setFormatter( function ( $val, $data, $opts ) {
                 return strip_tags($val);
            } ),
		Field::inst( '`Market status`' )->validator( Validate::notEmpty( ValidateOptions::inst()
                ->message( 'Market status is required' ) 
            ) )
			->setFormatter( function ( $val, $data, $opts ) {
                 return strip_tags($val);
            } ),
		Field::inst( '`Account, POD ID, ESIID`' )->validator( Validate::notEmpty( ValidateOptions::inst()
                ->message( 'Account, POD ID, ESIID is required' ) 
            ) )
			->setFormatter( function ( $val, $data, $opts ) {
                 return strip_tags($val);
            } ),
		Field::inst( 'Prefix' )->validator( Validate::notEmpty( ValidateOptions::inst()
                ->message( 'Prefix is required' ) 
            ) )
			->setFormatter( function ( $val, $data, $opts ) {
                 return strip_tags($val);
            } ),
		Field::inst( '`Name Key`' )->validator( Validate::notEmpty( ValidateOptions::inst()
                ->message( 'Name Key is required' ) 
            ) )
			->setFormatter( function ( $val, $data, $opts ) {
                 return strip_tags($val);
            } ),
		Field::inst( '`Meter Number`' )->validator( Validate::notEmpty( ValidateOptions::inst()
                ->message( 'Meter Number is required' ) 
            ) )
			->setFormatter( function ( $val, $data, $opts ) {
                 return strip_tags($val);
            } ),
		Field::inst( '`LOA Required`' )->validator( Validate::notEmpty( ValidateOptions::inst()
                ->message( 'LOA Required is required' ) 
            ) )
			->setFormatter( function ( $val, $data, $opts ) {
                 return strip_tags($val);
            } ),
		Field::inst( '`Billing options`' )->validator( Validate::notEmpty( ValidateOptions::inst()
                ->message( 'Billing options is required' ) 
            ) )
			->setFormatter( function ( $val, $data, $opts ) {
                 return strip_tags($val);
            } ),
		
		Field::inst( '`Purchase of Receivables (POR)`' )
		->setFormatter( function ( $val, $data, $opts ) {
                 return strip_tags($val);
            } ),
		Field::inst( '`Consolidated billing option`' )
		->setFormatter( function ( $val, $data, $opts ) {
                 return strip_tags($val);
            } ),
		Field::inst( '`Lead Time for Enrolls/Drops`' )
		->setFormatter( function ( $val, $data, $opts ) {
                 return strip_tags($val);
            } ),
		Field::inst( '`Special Enrollment Requirements`' )
		->setFormatter( function ( $val, $data, $opts ) {
                return strip_tags($val);
            } ),
		Field::inst( '`Days for ES response`' )
		->setFormatter( function ( $val, $data, $opts ) {
                 return strip_tags($val);
            } ),
		Field::inst( 'Comments' )
