...
<?php if($_SESSION["group_id"] == 1 or $_SESSION["group_id"] == 2){?>
				<button class="btn-primary" align="right" id="new-saving-analysis" style="height: 30px !important;width: auto !important;">Add New Saving Analysis</button>
<?php } ?>
			</div>
			<div id="satable"></div>
		</article>
...
<!-- end widget grid -->
<?php if($_SESSION["group_id"] == 1 or $_SESSION["group_id"] == 2){?>
<section id="savinganalysisdetails"></section>
<?php } 
if($_SESSION["group_id"] == 1 or $_SESSION["group_id"] == 2 or $_SESSION["group_id"] == 3 or $_SESSION["group_id"] == 5){?>
<div id="saresponse"></div>
<div id="satopdialog"></div>
<?php } ?>
<script type="text/javascript">
$(document).ready(function(){
<?php if($_SESSION["group_id"] == 1 or $_SESSION["group_id"] == 2){?>
	$("#new-saving-analysis").click(function(){
		//$('#datatable_fixed_column').DataTable().ajax.reload();
		//otable.ajax.reload();
		$("#dialog-message").remove();
		$('#saresponse').html('');
		$('#saresponse').load('assets/ajax/saving_analysis_add.php?action=add');
	});
<?php }
if($_SESSION["group_id"] == 1 or $_SESSION["group_id"] == 2 or $_SESSION["group_id"] == 3 or $_SESSION["group_id"] == 5){?>
	$('#satable').load("assets/ajax/saving_analysis_pedit.php?ct=Math.random()<?php if(isset($_GET["type"]) and $_GET["type"]=="unread"){echo "&type=unread";} echo $subquery; ?>");
<?php } ?>
	...
	$('#hidedemo').change(function () {
		...
		$('#satable').load("assets/ajax/saving_analysis_pedit.php?ct=Math.random()<?php if(isset($_GET["type"]) and $_GET["type"]=="unread"){echo "&type=unread"; } ?>&showdemo="+showdemo);
	});
	<?php } ?>
});
</script>