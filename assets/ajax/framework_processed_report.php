...
if(checkpermission($mysqli,120)==false) die("Permission Denied! Please contact Vervantis.");
...
<style>
	...
	*/
	...
	#fpr_datatable{border-bottom: 1px solid #ccc !important;}
	#fpr_datatable .isodrp{width:auto !important;}
	...
	#fpr_datatable tbody tr {cursor: pointer;}
	/*
	#fpr_datatable tbody tr td:nth-child(11) {
	...
	<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
		<h1 class="page-title txt-color-blueDark">
			<i class="fa fa-table"></i>
				Reports <span>> Processed Invoices Report</span>
		</h1>
					...
					<div class="row myrow">
						...
						<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" >
							...
							<div class="" style="padding-left:15px; float:left">
								...
								<button onclick="navigateurl('assets/ajax/framework_processed_report.php','Accounts Report')" type="button" class="btn btn-default" id="reset_query">Reset</button> 
							</div>
						</div>
						...
						<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" >
							...
							<div class="debug_qry" style="padding-left:15px; float:left; border:1px solid #e9e9e9;">
								<textarea class="qry_area" rows="5" cols="150"></textarea>
							</div>
						</div>
						</div>
		...
		<section id="widget-grid2" class="sitestable m-top45 ">
				...
				<div class="jarviswidget jarviswidget-color-blueDark" id="wid-id-3" data-widget-editbutton="false">
					<header>
						...
						<h2>Processed Invoices Report </h2>
					</header>
					...
					<div>
						...
						<div class="widget-body no-padding" id="adhoc-datatable-load">
							...
							<table id="fpr_datatable" class="display hover" style="width:100%">
								<thead>
									...
									<tr>
										...
										<th class="hasinput">
											<input type="text" class="form-control" placeholder="Total Due" />
										</th>
										<th class="hasinput">
											<input type="text" class="form-control" placeholder="Late Fee" />
										</th>
										<th class="hasinput">
										...
										<th class="hasinput">
											<input type="text" class="form-control" placeholder="Receipt Date" />
										</th>
										<th class="hasinput">
											<input type="text" class="form-control" placeholder="Entry Date" />
										</th>
										<th class="hasinput">
											<input type="text" class="form-control" placeholder="Consolidation Notification Date" />
										</th>
										<th class="hasinput">
											<input type="text" class="form-control" placeholder="Consolidation Received Date" />
										</th>
										<th class="hasinput">
											<input type="text" class="form-control" placeholder="Vendor Payment Date" />
										</th>
										<th class="hasinput">
											<input type="text" class="form-control" placeholder="Vendor Payment Clear Date" />
										</th>
										<th class="hasinput">
											<input type="text" class="form-control" placeholder="Invoice Deleted Date" />
										</th>
										<th class="hasinput">
											<input type="text" class="form-control" placeholder="Invoice Updated" />
										</th>
										<th class="hasinput">
											<input type="text" class="form-control" placeholder="Final Bill" />
										</th>
									...
									<tr>
										...
										<th>Invoice&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Deleted Date</th>
										<th>Invoice Updated</th>
										<th>Final Bill</th>
									</tr>
...
<script type="text/javascript">
	...
	var pagefunction = function() {
		...
		fpr_table = new DataTable('#fpr_datatable', {
			ajax: 'assets/ajax/framework_processed_report_ajax.php',
			...
			columns: [
				...
				{ data: 'LateFee' },
				{ data: 'Period' },
				{ data: 'DueDate' , name: 'DueDate' , width: '250px', class: 'mywid' },
				{ data: 'InvoiceDate' },
				{ data: 'ReceiptDate' },
				{ data: 'EntryDate' },
				...
				{ data: 'InvoiceDeletedDate' },
				// show only admin (group id 1)
				{ data: 'InvoiceUpdated' },
				{ data: 'FinalBill' },
			...
			*/
			...
			initComplete: function (settings, json) {
				...
				<?php if ($_SESSION["group_id"] == 1 OR $_SESSION["group_id"] == 2) { ?>
				fpr_table.column( 0 ).visible( true );				
				<?php } ?>
		...
		.on("draw.dt", function (e, dt, type, indexes) {
				...
				var grp_tr = $("#fpr_datatable tbody").find('tr:first');
	...
	$(document).ready(function(){
		...
		*/
		  ...
		  $('#create_query').on('click', function () {
			...
			//fpr_table.column(3).search( start_end_date ).draw(); // due date
			fpr_table.column(11).search( start_end_date ); // due date
			...
			if (vendor_id.length > 0) {
				fpr_table.column(2).search( vendor_id ); // vendor
			} else {
				fpr_table.column(2).search(""); // vendor
			}
			
			var account_id = $('#account').val();
			// check if account set 
			if (account_id.length > 0) {
				fpr_table.column(3).search( account_id ); // account
			} else {
				fpr_table.column(3).search(""); // account
			}
			...
			if (client_id.length > 0) {
				fpr_table.column(0).search( client_id ); // account
			} else {
				fpr_table.column(0).search(""); // account
			}
			<?php } ?>
			...
			if (country_ar.length > 0) {
				fpr_table.column(7).search( country_ar ); // country
			} else {
				fpr_table.column(7).search(""); // country
			}
			
			var state_ar = $('#state').val();
			// check if state set 
			if (state_ar.length > 0) {
				fpr_table.column(6).search( state_ar ); // state
			} else {
				fpr_table.column(6).search(""); // state
			}
			...
			// check if site set 
			if (site_id.length > 0) {
				fpr_table.column(5).search( site_id ); // site
			} else {
				fpr_table.column(5).search(""); // site
			}
		...
		*/
		...
		// inputs
		// Apply the filter
		$(document.body).on('keyup', '#fpr_datatable_wrapper thead th input[type=text]' ,function(){
		...
		$('#group_by').on('change', function (e) {
			...
			if (group_option == 'none') {
				fpr_table.rowGroup().disable().draw();
			} else {
				...
				if (group_option == 'VendorName') {
					...
					order_ind = 6;
				} else if (group_option == 'State') {
					order_ind = 7;
				} 
				
				fpr_table.order([order_ind, 'asc']).rowGroup().dataSrc($(this).val()).draw();
				//table.rowGroup().dataSrc($(this).data('column'));
				//fpr_table.rowGroup().dataSrc($(this).val()).draw();
			}
		});
		
		// hide no group tr
		fpr_table.on( 'draw', function () {
			// your code here
			//alert('draw');
			var grp_tr = $("#fpr_datatable tbody").find('tr:first');
			if (grp_tr.text() == 'No group') {
