	...
	<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
		<h1 class="page-title txt-color-blueDark">
			<i class="fa fa-table fa-fw "></i> 
				Charts
			<span>> Crude Oil > NYMEX Henry Hub </span>
		</h1>
	</div>
</div>
<style>
...
<div class="container fullwidth">
  <div class="embed-responsive embed-responsive-16by9 back-white">
	<p>&nbsp;</p>
<?php
	if ($stmt = $mysqli->prepare("SELECT _month, _year FROM cme_tmp WHERE energy_type='CO' and date(CONCAT(_year,'-',month(str_to_date(_month,'%b')),'-01')) >= DATE_ADD(date_format(now(), '%Y-%m-01'), INTERVAL 1 MONTH) group by _month,_year ORDER BY id")) { 
		$stmt->execute();
		$stmt->store_result();
		if ($stmt->num_rows > 0) {
			$ct=0;
...
function changedata(data)
{
	if(data != ""){
		$(".cm-date").html("");
		$(".cm-date").load("assets/ajax/chart-pedit.php?etype=CO"+data);
	}
}
</script>
	<div class="row">
...
<iframe class="embed-responsive-item" id="COiframe" frameborder="0" width="100%" height="975px"></iframe>
<?php
		}else{ ?>
