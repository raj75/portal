...
if(checkpermission($mysqli,132)==false) die("Permission Denied! Please contact Vervantis.");
...
//----------years-----------
$year_options = "";
$qry_ser = $mysqli2->query("SELECT DISTINCT `Year` FROM (SELECT YEAR(NOW()) AS `Year` UNION ALL SELECT `Year` FROM ubm_database.tblClientYears a WHERE a.ClientID=10)  b ORDER BY b.`Year` DESC");
		if ($qry_ser->num_rows > 0) {
			while($row_ser=$qry_ser->fetch_assoc()) {
				...
				$year_options .= "<option $selected>$year_name</option>";
			}
		}
	...
	<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
		<h1 class="page-title txt-color-blueDark">
			<i class="fa fa-table"></i>
				Reports <span>> Cost Usage YOY Report</span>
		</h1>
					...
					<div class="row" style="padding-left:15px; padding-bottom:15px;">
						...
						<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" >
							...
							<div class="" style="padding-left:15px; float:left">
								...
								<button onclick="navigateurl('assets/ajax/framework_yoy_report.php','Accounts Report')" type="button" class="btn btn-default" id="reset_query">Reset</button> 
		...
		*/
		  ...
		  $('#create_query').on('click', function () {
			...
			*/
			...
			//fghg_table.column(3).search( start_end_date ).draw(); // due date
			fghg_table.column(13).search( start_end_date ); // entry date
			...
			*/
			...
			if (account_id.length > 0) {
				fghg_table.column(7).search( account_id ); // account
			} else {
				fghg_table.column(7).search(""); // account
			}
			...
			if (country_ar.length > 0) {
				fghg_table.column(5).search( country_ar ); // country
			} else {
				fghg_table.column(5).search(""); // country
			}
			*/
			...
			if (state_ar.length > 0) {
				fghg_table.column(4).search( state_ar ); // state
			} else {
				fghg_table.column(4).search(""); // state
			}
			...
			// check if site set 
			if (site_id.length > 0) {
				fghg_table.column(0).search( site_id ); // site
			} else {
				fghg_table.column(0).search(""); // site
			}
			...
			if (show_meter == 1) {				
				...
				//var column_datatable = fghg_table.column(columnIdx); 
