								...
								<div class="row">
									<section class="col col-6">Company
										<label class="select"> <i class="icon-append fa fa-sitemap"></i>
											<select name="sscid" id="sscid" placeholder="Read" class="">
											<?php
...
<?php } ?>
								...
								<div class="row">
									...
									<section class="col col-6">Billing City
										<label class="input">
											<input type="text" name="ssbillcity" id="ssbillcity" placeholder="Billing City" value="<?php echo $cdBillingCity; ?>">
										</label>
									</section>
								</div>
								...
								<div class="row">
									...
									<section class="col col-6">Billing State
										<label class="input">
											<input type="text" name="ssbillstate" id="ssbillstate" placeholder="Billing State" value="<?php echo $cdBillingState; ?>">
										</label>
									</section>
								</div>
								...
								<div class="row">
									...
									<section class="col col-6">Billing Zip
										<label class="input">
											<input type="text" name="ssbillzip" id="ssbillzip" placeholder="Billing Zip" value="<?php echo $cdBillingZipcode; ?>">
										</label>
									</section>
								</div>
							</fieldset>
							...
							<fieldset>
								...
								<div class="row">
									<section class="col col-3 center">Utility Service Type <i style="color:red;">*</i>
										<label class="input">
									...
									<section class="col col-3 center">Vendor Name <i style="color:red;">*</i>
										<label class="input">
									...
									<section class="col col-3 center">Account <i style="color:red;">*</i>
										<label class="input">
...
<script type="text/javascript">
  ...
  document.head.append(script);
				  ...
				  /*$("#sns-submit").click(function (e) {
					...
					$("#stop-service-submit").on("click", function(e) {
                      ...
                      e.stopPropagation();
						...
						if(Array.from($('.ssutsertp').get(), e => e.value).join('@@').replace(/@/g, '') !="" && Array.from($('.ssvenname').get(), e => e.value).join('@@').replace(/@/g, '') !="" && Array.from($('.ssaccount').get(), e => e.value).join('@@').replace(/@/g, '') !="" && $("#ssturnoffdate").val() != ""){
							if (myDropz.getQueuedFiles().length > 0)
							{
								...
								formData.append('billing_city', $("#ssbillcity").val());
								formData.append('site_contact_fax', $("#sssitecontfax").val());
								formData.append('billing_state', $("#ssbillstate").val());
								formData.append('billing_zip', $("#ssbillzip").val());
								...
								if(Array.from($('.ssmeter').get(), e => e.value).join('@@').replace(/@/g, '') !=""){
								...
								myDropz.on("successmultiple", function(file, result) {
									if (result != false)
									{
										...
										{
											Swal.fire("At least one entry mandatory in:","Utility Service Type, Vendor Name, Account and Turn Off Date", "warning");
										}else{
	...
	//pagefunction();
$(document).on('change', '#sscid', function() {
	var formData = new FormData();
	formData.append('cid', $("#sscid").val());
	$.ajax({
		type: 'post',
		...
		success: function (result) {
			if (result != false)
			{
				...
				if(results.error == "")
				{
					...
					$("#ssentityname").val(results.EntityName);
					$("#ssfedtaxidno").val(results.TaxID);
					$("#ssbilladdr1").val(results.BillingAddress1);
					$("#ssbilladdr2").val(results.BillingAddress2);
					//$("#").val(results.BillingAddress3);
					//$("#snscid").val(results.companyID);
					$("#ssbillcity").val(results.BillingCity);
					$("#ssbillstate").val(results.BillingState);
					$("#ssbillzip").val(results.BillingZipcode);
...
$( document ).ready(function() {
  $("#sscid option:first").attr('selected','selected').change();;
});
</script>
<?php
/*	}else
