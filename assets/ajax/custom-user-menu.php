...
<section id="widget-gridss">
			...
			<div class="jarviswidget jarviswidget-color-blueDark padding10 cumenu" id="wid-id-00" data-widget-fullscreenbutton="false" data-widget-editbutton="false">
				<header>
				...
				<div>
					...
					<div class="widget-body">
					   <menu id="nestable-menu">
							...
							<button type="button" data-action="undo-it">Undo Changes</button>
							<button type="button" data-action="reset-default">Reset To Default</button> |
							<button type="button" data-action="save-all">Save</button>
							<button type="button" data-action="cancel-it" title="Save changes before close">Close</button>
						</menu>
				...
				if (action === 'reset-default') {
					if(confirm("Are you sure to reset to default?")){
						$.ajax({
							...
							data: "userid="+userid+"&resetdefault=true",
							async: true,
							success: function(rstatus){
								if(rstatus == true){
									parent.$('#response').html('');
									parent.$('#response').load('assets/ajax/custom-user-menu.php?editmenu=true&userid='+userid);
								}else
...
if(($_SESSION["group_id"]==1 or $_SESSION["group_id"]==5) and isset($_GET["editmenu"]) and 1==2){?>
<style>
