...
if(checkpermission($mysqli,119)==false) die("Permission Denied! Please contact Vervantis.");
...
<style>
	...
	*/
	...
	#fvvr_datatable{border-bottom: 1px solid #ccc !important;}
	#fvvr_datatable .isodrp{width:auto !important;}
	...
	#fvvr_datatable tbody tr {cursor: pointer;}
	/*
	#fvvr_datatable tbody tr td:nth-child(11) {
	...
	*/
	...
	#fvvr_datatable_wrapper .select2-container {/*height:15px !important;*/}
	#fvvr_datatable_wrapper input.form-control {height:40px !important;}
	...
	<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
		<h1 class="page-title txt-color-blueDark">
			<i class="fa fa-table"></i>
				Reports <span>> Vendor Report</span>
		</h1>
					...
					<div class="row myrow">
							...
							<div class="row myrow">			
								...
								<div class="myfilter col-xs-12 col-sm-12 col-md-4 col-lg-4">
									...
									<select name="group_by" id="group_by" class="select2 select2group">
										...
										<option value='VendorState'>Vendor State</option>
										<option value='ServiceTypeName'>Service Type</option>
									</select>
								</div>
								...
								<div class="myfilter col-xs-12 col-sm-12 col-md-4 col-lg-4">
									<label><b>Vendor Status</b></label>
									<br>
									<select name="vendorstatus" id="vendorstatus" class="select2 select2height">
						...
						<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" >
							...
							<div class="" style="padding-left:15px; float:left">
								...
								<button onclick="navigateurl('assets/ajax/framework_vendors_report.php','Accounts Report')" type="button" class="btn btn-default" id="reset_query">Reset</button> 
							</div>
						</div>
				...
				<div class="jarviswidget jarviswidget-color-blueDark" id="wid-id-3" data-widget-editbutton="false">
					...
					<div>
						...
						<div class="widget-body no-padding" id="adhoc-datatable-load">
							...
							<table id="fvvr_datatable" class="display hover" style="width:100%">
								<thead>
									...
									<tr>										
										...
										<th class="hasinput">
											<input type="text" class="form-control" placeholder="Vendor Address1" />
										</th>
										<th class="hasinput">
											<input type="text" class="form-control" placeholder="Vendor Address2" />
										</th>
										<th class="hasinput">
											<input type="text" class="form-control" placeholder="Vendor Address3" />
										</th>
										<th class="hasinput">
										...
										<th class="hasinput">
											<input type="text" class="form-control" placeholder="Vendor Phone" />
										</th>
										<th class="hasinput">
											<input type="text" class="form-control" placeholder="Vendor Fax" />
										</th>
										<th class="hasinput">
											<input type="text" class="form-control" placeholder="Vendor Email" />
										</th>
										<th class="hasinput">
											<!--<input type="text" class="form-control" placeholder="Vendor Status" />-->
											<select name="vendorstatuscol" id="vendorstatuscol" class="select2-- select2col">
												<option value=''>All</option>
												<option value='active'>Active</option>
												<option value='inactive'>Inactive</option>
											</select>
									...
									<tr>										
										...
										<th>Site Country</th>
										<th>Vendor Name</th>
										<th>Vendor Address1</th>
										<th>Vendor Address2</th>
										<th>Vendor Address3</th>
										<th>Vendor City</th>
										<th>Vendor State</th>
										<th>Vendor Zip</th>
...
<script type="text/javascript">
	...
	var pagefunction = function() {
		...
		fvvr_table = new DataTable('#fvvr_datatable', {
			ajax: 'assets/ajax/framework_vendors_report_ajax.php',
	...
	$(document).ready(function(){
		...
		*/
		  ...
		  $('#create_query').on('click', function () {
			...
			if (vendor_id.length > 0) {
				fvvr_table.column(4).search( vendor_id ); // vendor
			} else {
				fvvr_table.column(4).search(""); // vendor
			}
			...
			if (client_id.length > 0) {
				fvvr_table.column(0).search( client_id ); // client
			} else {
				fvvr_table.column(0).search(""); // client
			}
			<?php } ?>
			...
			if (country_ar.length > 0) {
				fvvr_table.column(11).search( country_ar ); // country
			} else {
				fvvr_table.column(11).search(""); // country
			}
			
			var state_ar = $('#state').val();
			// check if state set 
			if (state_ar.length > 0) {
				fvvr_table.column(9).search( state_ar ); // state
			} else {
				fvvr_table.column(9).search(""); // state
			}
			
			var service_id = $('#service_type').val();
			
			if (service_id.length > 0) {
				fvvr_table.column(1).search( service_id ); // vendor
			} else {
				fvvr_table.column(1).search(""); // vendor
			}
			
			var vendorstatus = getFilterStatus('vendorstatus');
			// check if vendorstatus set 
			if (vendorstatus.length > 0) {
				fvvr_table.column(15).search( vendorstatus ); // vendorstatus
			} else {
				fvvr_table.column(15).search(""); // vendorstatus
			}
		...
		//fvvr_table.column(17).visible(false);   // To hide
		...
		*/
		...
		// inputs
		// Apply the filter
		$(document.body).on('keyup', '#fvvr_datatable_wrapper thead th input[type=text]' ,function(){
		...
		$( "#vendorstatuscol" ).on( "change", function() {
		...
		$('#group_by').on('change', function (e) {
			...
			if (group_option == 'none') {
				fvvr_table.rowGroup().disable().draw();
			} else {
				...
				if (group_option == 'VendorName') {
					order_ind = 4;
				} else if (group_option == 'VendorState') {
					order_ind = 9;
				} else if (group_option == 'ServiceTypeName') {
					order_ind = 1;
				} 
				
				fvvr_table.order([order_ind, 'asc']).rowGroup().dataSrc($(this).val()).draw();
				
				//table.rowGroup().dataSrc($(this).data('column'));
				//fvvr_table.rowGroup().dataSrc($(this).val()).draw();
			}
		});
		...
		// hide no group tr
		fvvr_table.on( 'draw', function () {
			// your code here
			
			var grp_tr = $("#fvvr_datatable tbody").find('tr:first');
