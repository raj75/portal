...
<table id="datatable_fixed_column" class="table table-striped table-bordered table-hover" width="100%">
	<thead>
		<tr class="dropdown">
			<th class="hasinput hidden">
				<input type="text" class="form-control" placeholder="Filter ID" />
			</th>
			<th class="hasinput">
				<select class="form-control resolvedexpdrp" id="resolvedexpdrp">
					<option value="">Filter Company Name</option>
					<?php
						if(isset($_SESSION['group_id']) and ($_SESSION['group_id'] == 1 or $_SESSION['group_id'] == 2)){
							if ($stmt = $mysqli->prepare('SELECT DISTINCT `Customer Description` from exceptions ORDER BY `Customer Description`')){

								$stmt->execute();
								$stmt->store_result();
								if ($stmt->num_rows > 0) {
									$stmt->bind_result($__company);
									while($stmt->fetch()){
										echo "<option value='".$__company."'>".$__company."</option>";
									}
								}
							}else{
			...
			<th class="hasinput">
				<input type="text" class="form-control" placeholder="Filter Service" />
			</th>
			<th class="hasinput">
				<input type="text" class="form-control" placeholder="Filter Resolution" />
			</th>
			<th class="hasinput">
				<input type="text" class="form-control" placeholder="Filter DocID" />
			</th>
			<th class="hasinput">
				<input type="text" class="form-control" placeholder="Filter EST Date" />
			</th>
		</tr>
		<tr>
			<th data-hide="phone" class="hidden">ID</th>
			<th>Company Name</th>
			<th>Site #</th>
			<th>Site Name</th>
			<th>Vendor Name</th>
	...
	var pagefunction = function() {
		...
		*/
        ...
        "buttons": [
			...
			"autoWidth" : true,
			"ajax": "assets/ajax/server_processing_resolvedexp-test.php"
	    });
<?php
if($_SESSION["group_id"] != 1 and $_SESSION["group_id"] != 2){
?>
		otable.columns( [0,1] ).visible( false );
<?php }else{ ?>
...
<?php } ?>
	    ...
	    $("#datatable_fixed_column .resolvedexpdrp").on( 'keyup change', function () {
	        otable
				.column( $(this).parent().index()+':visible' )
	            .search(this.value)
	            .draw();
