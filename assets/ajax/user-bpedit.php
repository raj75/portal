...
require_once '../includes/functions.php';
	if($_SESSION["group_id"] == 1 || $_SESSION["group_id"] == 2)
		$sql='SELECT firstname,lastname,user_id,email,usergroups_id,gender,status FROM user';

//SELECT u.id,u.email,u.usergroups_id,u.gender,u.status FROM user u';

	elseif($_SESSION["group_id"] == 5)
		$sql='SELECT firstname,lastname,user_id,email,usergroups_id,gender,status FROM user where (usergroups_id = 5 OR usergroups_id = 3) and company_id=(SELECT usp.company_id FROM user usp WHERE usp.user_id= "'.$user_one.'")';
...
//SELECT id,email,usergroups_id,gender,status FROM user where (usergroups_id = 5 OR usergroups_id = 3) and company_id=(SELECT usp.company_id FROM user usp WHERE usp.id= "'.$user_one.'")';
	...
	if ($stmt = $mysqli->prepare($sql)) {
        ...
        $stmt->store_result();
        if ($stmt->num_rows > 0) {
			$stmt->bind_result($fname,$lname,$id,$Email,$Usergroups,$Gender,$Status);
			while($stmt->fetch()) {
				$ts=$id.rand(650,900);
			?>
				<tr>
...
<?php } ?>
						<td><?php echo $Email; ?>
						<?php if($_SESSION["group_id"] == 1 || $_SESSION["group_id"] == 2){?>
							<?php --$ts;?><a href="javascript:void(0);" onclick="showversion('<?php echo $id; ?>','<?php echo 'user'; ?>','<?php echo 'email'; ?>','<?php echo $ts;?>')" id="<?php echo $ts;?>"> <i class="icon-prepend fa fa-question-circle"></i></a><a href="javascript:void(0);" rel="popover" data-placement="top" data-original-title="<h4>Versions</h4>" data-content="None" data-html="true" id="p<?php echo $ts;?>">&nbsp;</a>
						<?php } ?>
						</td>
						<td><?php echo $fname; ?>
						<?php if($_SESSION["group_id"] == 1 || $_SESSION["group_id"] == 2){?>
							<?php --$ts;?><a href="javascript:void(0);" onclick="showversion('<?php echo $id; ?>','<?php echo 'user'; ?>','<?php echo 'firstname'; ?>','<?php echo $ts;?>')" id="<?php echo $ts;?>"> <i class="icon-prepend fa fa-question-circle"></i></a><a href="javascript:void(0);" rel="popover" data-placement="top" data-original-title="<h4>Versions</h4>" data-content="None" data-html="true" id="p<?php echo $ts;?>">&nbsp;</a>
						<?php } ?>
						</td>
						<td><?php echo $lname; ?>
						<?php if($_SESSION["group_id"] == 1 || $_SESSION["group_id"] == 2){?>
						...
						<td><?php if($_SESSION["group_id"] == 1 || $_SESSION["group_id"] == 2){
							...
							<?php if($_SESSION["group_id"] == 1 || $_SESSION["group_id"] == 2){?>
							<?php --$ts;?><a href="javascript:void(0);" onclick="showversion('<?php echo $id; ?>','<?php echo 'user'; ?>','<?php echo 'usergroups_id'; ?>','<?php echo $ts;?>')" id="<?php echo $ts;?>"> <i class="icon-prepend fa fa-question-circle"></i></a><a href="javascript:void(0);" rel="popover" data-placement="top" data-original-title="<h4>Versions</h4>" data-content="None" data-html="true" id="p<?php echo $ts;?>">&nbsp;</a>
						<?php } ?>
						</td>
<?php if($_SESSION["group_id"] == 1 || $_SESSION["group_id"] == 2){?>
						<td><?php echo $Gender; ?>
...
<?php } ?>
						...
						<?php if($_SESSION["group_id"] == 1 || $_SESSION["group_id"] == 2){?>
							<?php --$ts;?><a href="javascript:void(0);" onclick="showversion('<?php echo $id; ?>','<?php echo 'user'; ?>','<?php echo 'status'; ?>','<?php echo $ts;?>')" id="<?php echo $ts;?>"> <i class="icon-prepend fa fa-question-circle"></i></a><a href="javascript:void(0);" rel="popover" data-placement="top" data-original-title="<h4>Versions</h4>" data-content="None" data-html="true" id="p<?php echo $ts;?>">&nbsp;</a>
							<?php } ?>
						</td>
						...
						<?php if(!$_SESSION["group_id"] == 1 and !$_SESSION["group_id"] == 2){?><button onclick="deleteuser(<?php echo $id; ?>,'<?php echo $Email; ?>')" title="Delete User" class="btn btn-xs btn-default"><i class="fa fa-times"></i></button><?php } ?>&nbsp;&nbsp;</td>
					</tr>
			<?php
			}
		}
