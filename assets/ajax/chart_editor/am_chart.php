...
//echo $_POST['chart_query'];
				...
				if (isset($_POST['chart_query']) || strlen($default_sql) > 1) {
					...
					// Get field information for all fields
					  ...
					  */
					  ...
					  if (($_GET['gid']=='col' || $_GET['gid']=='bar') and $_GET['cid']==9) {
						  $graph[1] .= '"type": "column",';
										  
						  $graph[2] .= '"bullet": "round",
									    "lineThickness": 2,';
					  }
					  */
					  ...
					  foreach ($fieldinfo as $key=>$val) {
						...
						/*
						if (isset($graph[$key])) {
							$graph_settings .= $graph[$key];
						}
						*/
						if ($key==3) { 
							if ( ($_GET['gid']=='col' || $_GET['gid']=='bar') and $_GET['cid']==4 ) {
								$graph_settings .= '"newStack": true,'; 
							}
						}
						if ($_GET['gid']=='line' and $_GET['cid']==13) {
							$graph_settings .= '"noStepRisers": true,';
						}
						...
						if(strpos($val->name,"_axes") !== false){
							...
							$graph_settings .= '"valueAxis": "ValueAxis-'.($key+1).'",';
								...
								*/
								...
								$secondary_vertical_axes .= '"id": "ValueAxis-'.($key+1).'",';
								
								if ( ($_GET['gid']=='col' and $_GET['cid']==15) || ($_GET['gid']=='bar' and $_GET['cid']==14) ) {
									$secondary_vertical_axes .= '"logarithmic": true,';
								}
								...
								if (isset($_POST['svv_min']) and !empty($_POST['svv_min'])) {
									$svv_min = $_POST['svv_min']; 
									$secondary_vertical_axes .= '"minimum": '.$svv_min.',';
								}
								if (isset($_POST['svv_max']) and !empty($_POST['svv_max'])) {
									$svv_max = $_POST['svv_max']; 
									$secondary_vertical_axes .= '"maximum": '.$svv_max.',';
								}
						...
						*/
						...
						if ($_GET['gid']=='col' and $_GET['cid']==14) {
							$graph_settings .= '"colorField": "color",';
							$graph_settings .= '"lineColorField": "color",';
						}
						
						if ($_GET['gid']=='bar' and $_GET['cid']==13) {
							$graph_settings .= '"alphaField": "fill alpha",';
							$graph_settings .= '"dashLengthField": "dash length",';
						}
					...
					if ($_GET['gid']=='col' and $_GET['cid']==14) {
						//var_dump($db_data_provider_arr);
						$db_data_provider_arr = addcustomcolor($db_data_provider_arr);
						//var_dump($db_data_provider_arr);
					}
					
					if ($_GET['gid']=='bar' and $_GET['cid']==13) {
						$db_data_provider_arr = adddashstroke($db_data_provider_arr);
				...
				/*
				----------category axes (vertical) settings---------------
				*/
				$cat_axes_setings = '';
				...
				//if ( $pcount==0 || ( isset($_POST['phh_label_enable']) and $_POST['phh_label_enable']==1 ) ) {
					...
					/*
					if (isset($_POST['phh_min']) and !empty($_POST['phh_min'])) {
						$phh_min = $_POST['svv_min']; 
						$cat_axes_setings .= '"minimum": '.$phh_min.',';
					}
					if (isset($_POST['phh_max']) and !empty($_POST['phh_max'])) {
						$phh_max = $_POST['phh_max']; 
						$cat_axes_setings .= '"maximum": '.$phh_max.',';
					}
					
					if (isset($_POST['phh_min_date']) and !empty($_POST['phh_min_date'])) {
						$phh_min_date = $_POST['phh_min_date']; 
						$cat_axes_setings .= '"minimumDate": "'.$phh_min_date.'",';
					}
					if (isset($_POST['phh_max_date']) and !empty($_POST['phh_max_date'])) {
						$phh_max_date = $_POST['phh_max_date']; 
						$cat_axes_setings .= '"maximumDate": "'.$phh_max_date.'",';
					}
					*/
					
					//"maximumDate": "2022-03-13",
				...
				*/
				...
				/*
				----------primary_vertical_axes (value axes) settings---------------
				*/
				$primary_vertical_axes = '{';
					...
					$primary_vertical_axes .= '"id": "ValueAxis-'.$key.'",';
					
					if ( ($_GET['gid']=='col' and $_GET['cid']==15) || ($_GET['gid']=='bar' and $_GET['cid']==14) ) {
						$primary_vertical_axes .= '"logarithmic": true,';
					}
					//"logarithmic": true,
					...
					if (isset($_POST['pvv_min']) and (int) $_POST['pvv_min'] > 0) {
						$pvv_min = $_POST['pvv_min'];
						$primary_vertical_axes .= '"minimum": '.$pvv_min.',';
					}
					
					if (isset($_POST['pvv_max']) and (int) $_POST['pvv_max'] > 0) {
						$pvv_max = $_POST['pvv_max'];
						$primary_vertical_axes .= '"maximum": '.$pvv_max.',';
					}
				...
				*/
				...
				if (isset($_POST['lpalignment']) and !empty($_POST['lpalignment'])) {
					$lpalignment = $_POST['lpalignment'];
					$format_legend .= '"align":"'.$lpalignment.'",';
				}
...
<html>
	<head>
		...
		<script type="text/javascript" src="/assets/js/amchart/amcharts.js"></script>
		<script type="text/javascript" src="/assets/js/amchart/serial.js"></script>
		<script type="text/javascript" src="/assets/js/amchart/pie.js"></script>
		<script type="text/javascript" src="/assets/js/amchart/funnel.js"></script>
		<script type="text/javascript" src="/assets/js/amchart/radar.js"></script>
		<?php if ( isset($_POST['theme']) and strlen($_POST['theme'])> 1 ) { ?>
		<script type="text/javascript" src="/assets/js/amchart/themes/<?php echo $_POST['theme'];?>.js"></script>
		<?php } ?>
...
*/
	...
	<body>
		...
		<style>
			...
			div.amcharts-chart-div a[title="JavaScript charts"]
			{
...
//--------------------------------------------------------------
function addcustomcolor ($db_data_provider_arr) {
	$last_record = [];
	
	$last_record_1 = end($db_data_provider_arr);
	
	$last_record_2 = [ "color" => "#d0c398" ];
	...
	//$last_key = key($db_data_provider_arr);
	//$last_record[] = ""=>"";
	//$last_record[0] += [ "color" => "#d0c398" ];
	$last_record[0] = $last_record_1 + $last_record_2;
	
	//var_dump($last_record);
	//die();
	
	array_pop($db_data_provider_arr); 
	
	//$return = $db_data_provider_arr;
	
	//$return = $db_data_provider_arr + $last_record;
	
	$return = array_merge($db_data_provider_arr,$last_record);
	//var_dump($return);
	
	
	return $return;
	
	//$db_data_provider_arr[$last_key] = $last_record;
...
//--------------------------------------------------------------
function adddashstroke ($db_data_provider_arr) {
	$last_record = [];
	
	$last_record_1 = end($db_data_provider_arr);
	
	$last_record_2 = [ "dash length" => 5, "fill alpha" => 0.2 ];
