...
if(isset($_GET['sgid']) and isset($_GET['action']) and $_GET['action'] == "details"){
	if(isset($_GET['sgid']) and @trim($_GET['sgid']) != "" and $_GET['sgid'] > 0)
		$sgid=$mysqli->real_escape_string(@trim($_GET['sgid']));
	else
...
#wid-id--77 .nopadds{padding:0 !important;}
	...
	<div class="row ssse" id="<?php echo $sgid; ?>">
		<!-- NEW WIDGET START -->
		<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
			<div class="jarviswidget jarviswidget-color-blueDark" id="wid-id--77" data-widget-editbutton="false">
				<header>
					<span class="widget-icon"> <i class="fa fa-table"></i> </span>
					<h2>ID: <?php echo $sgid; ?></h2>
					<span class="widget-icon" style="float: right;padding-right: 24px;cursor:pointer;" onclick="clearme(<?php echo $sgid; ?>,12,true,0)"> <i class="fa fa-times"></i> </span>
				</header>
				...
				if ($vestmt = $mysqli->prepare('Select ID,service_group_id,service_group,capturis_service_group_id,capturis_service_group,importDate From service_group Where ID='.$sgid.' LIMIT 1')) {

				$vestmt->execute();
				$vestmt->store_result();
				if ($vestmt->num_rows > 0) {
					$vestmt->bind_result($sgid,$service_group_id,$service_group,$capturis_service_group_id,$capturis_service_group,$importDate);
					$vestmt->fetch();

					$ts=$sgid.rand(650,900);
					...
					if($importDate=="0000-00-00 00:00:00") $importDate="";
						?>
						<table id="cmacctable<?php echo $sgid; ?>" class="table table-striped table-bordered table-hover" style="clear: both">
							<tr>
								<th width="14%">Service Group ID:</th>
								<td><input type="text" value="<?php echo $service_group_id; ?>" class="sginputautosave" saveme="service_group_id"></td>
								<th width="15%">Service Group:</th>
								<td><input type="text" value="<?php echo $service_group; ?>" class="sginputautosave" saveme="service_group"></td>
								<th width="14%">Capturis Service Group ID:</th>
								<td><input type="text" value="<?php echo $capturis_service_group_id; ?>" class="sginputautosave" saveme="capturis_service_group_id"></td>
							</tr>

							<tr>
								<th width="14%">Capturis Service Group:</th>
								<td><input type="text" value="<?php echo $capturis_service_group; ?>" class="sginputautosave" saveme="capturis_service_group"></td>
								<th width="15%">ImportDate:</th>
								<td><input type="text" value="<?php echo $importDate; ?>" class="sginputautosave" saveme="importDate"></td>
								<th width="14%"></th>
								<td></td>
							</tr>
<?php if(1==2){ ?>
							<tr>
								...
								<td><input type="text" value="<?php echo $entity_name; ?>" class="sginputautosave" saveme="entity_name" <?php echo $disabled; ?>>
								<?php echo checkversionavailability($mysqli,"startstop_status",$ssid,"entity_name",$disabled); ?></td>
								<th width="15%">Tax ID:</th>
								<td><input type="text" value="<?php echo $federal_tax_id; ?>" class="sginputautosave" saveme="federal_tax_id" <?php echo $disabled; ?>>
								<?php echo checkversionavailability($mysqli,"startstop_status",$ssid,"federal_tax_id",$disabled); ?></td>
								<th width="14%">Site Number:</th>
								<td><input type="text" value="<?php echo $site_number; ?>" class="sginputautosave" saveme="site_number" <?php echo $disabled; ?>>
							...
							<tr>
								...
								<td><input type="text" value="<?php echo $utility_service_type; ?>" class="sginputautosave" saveme="utility_service_type" <?php echo $disabled; ?>>
								...
								<td><input type="text" value="<?php echo $vendor_name; ?>" class="sginputautosave" saveme="vendor_name" <?php echo $disabled; ?>>
								<?php echo checkversionavailability($mysqli,"startstop_status",$ssid,"vendor_name",$disabled); ?></td>
								<th width="14%">Site Name:</th>
								<?php ++$ts; ?>
								<td><input type="text" value="<?php echo $site_name; ?>" class="sginputautosave" saveme="site_name" <?php echo $disabled; ?>>
							...
							<tr>
								...
								<td><input type="text" value="<?php echo $date_requested; ?>" class="sginputautosave" saveme="date_requested" <?php echo $disabled; ?>>
								<?php echo checkversionavailability($mysqli,"startstop_status",$ssid,"date_requested",$disabled); ?></td>
								<th width="15%">Date Contacted:</th>
								<td><input type="text" value="<?php echo $date_contacted; ?>" class="sginputautosave" saveme="date_contacted" <?php echo $disabled; ?>>
								<?php echo checkversionavailability($mysqli,"startstop_status",$ssid,"date_contacted",$disabled); ?></td>
								<th width="14%">Contacted Method:</th>
								<td><input type="text" value="<?php echo $contacted_method; ?>" class="sginputautosave" saveme="contacted_method" <?php echo $disabled; ?>>
							...
							<tr>
								...
								<td><input type="text" value="<?php echo $previous_account_number; ?>" class="sginputautosave" saveme="previous_account_number" <?php echo $disabled; ?>>
								...
								<td><input type="text" value="<?php echo $account_number; ?>" class="sginputautosave" saveme="account_number" <?php echo $disabled; ?>>
								<?php echo checkversionavailability($mysqli,"startstop_status",$ssid,"account_number",$disabled); ?></td>
								<th width="14%">Meter:</th>
								<td><input type="text" value="<?php echo $meter; ?>" class="sginputautosave" saveme="meter" <?php echo $disabled; ?>>
...
<?php } ?>
		...
		echo showlogs($sgid,'service_group','sgdetails','assets/ajax/service-group-pedit.php?action=details&sgid='.$sgid,$disabled);
		?>
	</div>
...
//$('.datetimepicker').datetimepicker();
  $('.sginputautosave').blur(function() {
	 autosave($(this).attr("saveme"),$(this).val());
  });

  $('.sgselectautosave').change(function() {
	 autosave($(this).attr("saveme"),$(this).val());
  });
  ...
  function autosave(savename,saveval){
	...
	formData.append('sgauto', <?php echo $sgid; ?>);
	formData.append('sgsavename', savename);
	formData.append('sgvalue', saveval);

	$.ajax({
		type: 'post',
		url: 'assets/includes/servicegroup.inc.php',
		data: formData,
		processData: false,
		contentType: false,
...
$("#logshow").load("assets/ajax/showlogs.php?pkey=<?php echo $sgid; ?>&tname=service_group&load=true&disb=<?php echo @trim($disabled); ?>&tuid=sgdetails&tuurl=<?php echo urlencode('assets/ajax/service-group-pedit.php?action=details&sgid='.$sgid); ?>&ct=<?php echo rand(0,100); ?>");
					parent.$('#sgtable').load("assets/ajax/service-group-pedit.php?load=true&ct=<?php mt_rand(2,77); ?>");
...
<table id="datatable_fixed_column" class="table table-striped table-bordered table-hover" width="100%">
	<thead>
			...
			<th class="hasinput">
				<input type="text" class="form-control" placeholder="Filter Capturis Service Group ID" />
			</th>
			<th class="hasinput">
				<input type="text" class="form-control" placeholder="Filter Capturis Service Group" />
			</th>
			<th class="hasinput">
		...
		<tr>
			...
			<th>Service Group</th>
			<th data-hide="phone">Capturis Service Group ID</th>
			<th data-hide="phone,tablet">Capturis Service Group</th>
			<th>Import Date</th>
		</tr>
	</thead>
	<tbody>
	...
	var pagefunction = function() {
		...
		*/
		...
		/*"columnDefs": [
			...
			"autoWidth" : true,
			"ajax": "assets/ajax/server_processing_servicegroup.php"/*,
...
function sgload_details(sgid) {
	...
	$("#sgdetails").load('assets/ajax/service-group-pedit.php?ct=<?php echo rand(0,100); ?>&action=details&sgid='+sgid);
	//otable.pageLength(6) ;
	//$("#ss_datatable_fixed_column").DataTable().page.len(6).draw();
}
function loadsgmenu(sgid) {
	sgload_details(sgid)
