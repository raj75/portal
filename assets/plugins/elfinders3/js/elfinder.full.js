...
elFinder.prototype.commands.quicklook.plugins = [
	...
	function(ql) {
				...
				preview.on(ql.evUpdate, function(e) {
					var file = e.file,
						mimess = file.mime;
					//if ($.inArray(mimess, mime) !== -1) {
						//e.preventDefault();

						e.stopImmediatePropagation();


					$.post("php/filepermission.php",
					{
					  url: fm.openUrl(file.hash),
					  role: "admin"
					},
					function(rurl){
					  if(rurl !=false){ previewPop(rurl)};					
					});
					//}
				});
			...
			/*preview.on(ql.evUpdate, function(e) {
				...
				if (active && file.mime === mime && ql.dispInlineRegex.test(file.mime)) {
					...
					function(rurl){
					  if(rurl !=false){ url=rurl;}					
						
						ql.cover.addClass('elfinder-quicklook-coverbg');
						$('<object class="elfinder-quicklook-preview-pdf" data="'+url+'" type="application/pdf" />')
							.on('error', function(e) {
								active = false;
...
function previewPop(filename){
      ...
      var exts = ['jpg','jpeg','gif','png','bmp','ico'];
      ...
      if ( file ) {
        ...
        if ( $.inArray ( get_ext[0].toLowerCase(), exts ) > -1 ){
          discode='<img src="'+filename+'" width="100%" class="img-responsive" style="height:100% !important;" />';
		  parent.$("#dialog").html('');
		  parent.$("#dialog").html(discode);
		  parent.$("#dialog").dialog("open");
		  parent.$("#d-download").attr('href', filename);
        } else {
          discode='<iframe src="https://docs.google.com/viewer?embedded=true&url='+filename+'" frameborder="0" width="100%" height="100%" id="googleload"></iframe><div class="noshow"></div>';
		  parent.$("#dialog").html('');
		  parent.$("#dialog").html(discode);
		  parent.$("#dialog").dialog("open");
		  parent.$("#d-download").attr('href', filename);
