...
$(function(){
	...
	$.get('../../../uploads/resources/Clients/.scans.php?docname=<?php echo $docname; ?>', function(data) {
		...
		function render(data) {
			...
			if(scannedFolders.length) {
				scannedFolders.forEach(function(f) {
					...
					var itemsLength = f.items.length,
						...
						icon = '<span class="icon folder-list"></span>';
			...
			if(scannedFiles.length) {
				...
				scannedFiles.forEach(function(f) {
					...
					//var file = $('<li class="files"><a href="javascript:void(0);" title="'+ f.path +'" class="files Popup" onclick="previewPop(\''+f.path+'\');">'+icon+'<span class="name">'+ name +'</span> <span class="details">'+fileSize+'</span></a></li>');
			...
			if(filemanager.hasClass('searching')){
				...
				//url += '<a href="<?php //echo $_COOKIE["appurl"]; ?>/assets/plugins/cute-file-browsers/<?php //echo $_COOKIE["uname"]."(".$_COOKIE["uid"].")";?>"><span class="folderName">Home</span></a> <span class="arrow">→</span> ';
...
function previewPop(filename){
      ...
      if ( file ) {
        ...
        if ( $.inArray ( get_ext[0].toLowerCase(), exts ) > -1 ){
          discode='<img src="http://<?php echo $_SERVER['HTTP_HOST']; ?>/uploads/resources/Clients/'+filename+'" width="100%" style="height:auto;max-height:100%;" />';
		  parent.$("#dialog").html('');
		  parent.$("#dialog").html(discode);
		  parent.$("#dialog").dialog("open");
		  parent.$("#d-download").attr('href', "http://<?php echo $_SERVER['HTTP_HOST']; ?>/uploads/resources/Clients/"+filename);
        } else {
          discode='<iframe src="https://docs.google.com/viewer?embedded=true&url=https://<?php echo $_SERVER['HTTP_HOST']; ?>/uploads/resources/Clients/'+filename+'" frameborder="0" width="100%" height="100%" id="googleload"></iframe><a href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/uploads/resources/Clients/'+filename+'" download><div class="noshow"></div></a>';
		  parent.$("#dialog").html('');
		  parent.$("#dialog").html(discode);
		  parent.$("#dialog").dialog("open");
		  parent.$("#d-download").attr('href', "http://<?php echo $_SERVER['HTTP_HOST']; ?>/uploads/resources/Clients/"+filename);
