    ...
    OptionCache.prototype = {
        ...
        _createElement: function(optElement, optGroup) {
            ...
            if (this._widget.options.selectionMode.indexOf('d&d') > -1) {
                var that = this;
				//by amir
				var overlay_ar = jQuery('<div id="overlay_ar"> </div>');
                e.draggable({
                    addClasses: false,
                    ...
                    start: function(evt, ui) {
						...
						//by amir
						overlay_ar.appendTo('.multiselect-available-list .multiselect-element-wrapper');
                    },
                    stop: function(evt, ui) {
                        $(this).removeClass('ui-state-disabled ui-state-active');
						setTimeout(function() {
						  $(".multiselect-available-list .multiselect-element-wrapper #overlay_ar").remove();
						}, 500); // milisec
                    },
                    helper: 'clone',
                    revert: 'invalid',
